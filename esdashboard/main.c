...
{
	/* Quit application */
	esdashboard_application_quit_forced(NULL);
...
static gboolean _restart(EsdashboardApplication *inApplication)
{
	GMainLoop			*mainLoop;
	guint				dbusWatcherID;
	...
	g_return_val_if_fail(ESDASHBOARD_IS_APPLICATION(inApplication), FALSE);
...
{
	EsdashboardApplication		*app=NULL;
	gint						status;
	 ...
	 * the ESDASHBOARD_BACKEND environment variable. If this environment variable
	 * is set, enforce this backend.
	 *
	 * This function must be called before any API function call at libesdashboard
	 * or other library API function like the one of Clutter, GTK+ etc.
	 */
	backend=g_getenv("ESDASHBOARD_BACKEND");
	if(backend)
	{
		esdashboard_window_tracker_backend_set_backend(backend);
	...
	if(status==ESDASHBOARD_APPLICATION_ERROR_RESTART &&
