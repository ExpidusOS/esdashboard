...
struct _EsdashboardStageInterfacePrivate
{
	/* Properties related */
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardStageInterface,
							esdashboard_stage_interface,
...
static GParamSpec* EsdashboardStageInterfaceProperties[PROP_LAST]={ 0, };


/* IMPLEMENTATION: Private variables and methods */

/* Monitor size changed */
static void _esdashboard_stage_interface_on_geometry_changed(EsdashboardStageInterface *self, gpointer inUserData)
{
	EsdashboardStageInterfacePrivate	*priv;
	gint								x, y, w, h;

	g_return_if_fail(ESDASHBOARD_IS_STAGE_INTERFACE(self));
	...
	ESDASHBOARD_DEBUG(self, ACTOR,
						...
						x, y,
						w, h,
						esdashboard_window_tracker_monitor_is_primary(priv->monitor) ? "primary" : "non-primary",
						esdashboard_window_tracker_monitor_get_number(priv->monitor));
}

/* Monitor changed primary state */
static void _esdashboard_stage_interface_on_primary_changed(EsdashboardStageInterface *self, gpointer inUserData)
{
	EsdashboardStageInterfacePrivate	*priv;
	gboolean							isPrimary;
	...
	isPrimary=esdashboard_window_tracker_monitor_is_primary(priv->monitor);

	/* Depending on primary state set CSS class */
	if(isPrimary) esdashboard_stylable_add_class(ESDASHBOARD_STYLABLE(self), "primary-monitor");
		else esdashboard_stylable_remove_class(ESDASHBOARD_STYLABLE(self), "primary-monitor");
...
static void esdashboard_stage_interface_parent_set(ClutterActor *inActor, ClutterActor *inOldParent)
{
	EsdashboardStageInterface			*self;
	EsdashboardStageInterfacePrivate	*priv;
	ClutterActorClass					*parentClass;
	ClutterActor						*newParent;

	g_return_if_fail(ESDASHBOARD_IS_STAGE_INTERFACE(inActor));

	self=ESDASHBOARD_STAGE_INTERFACE(inActor);
	priv=self->priv;

	/* Call parent's virtual function */
	parentClass=CLUTTER_ACTOR_CLASS(esdashboard_stage_interface_parent_class);
	...
	if(newParent && ESDASHBOARD_IS_STAGE(newParent))
	{
...
static void _esdashboard_stage_interface_get_preferred_height(ClutterActor *inActor,
																gfloat inForWidth,
																gfloat *outMinHeight,
																gfloat *outNaturalHeight)
{
	EsdashboardStageInterface			*self=ESDASHBOARD_STAGE_INTERFACE(inActor);
	EsdashboardStageInterfacePrivate	*priv=self->priv;
	...
	if(priv->monitor)
	{
		esdashboard_window_tracker_monitor_get_geometry(priv->monitor, NULL, NULL, NULL, &h);
...
static void _esdashboard_stage_interface_get_preferred_width(ClutterActor *inActor,
																gfloat inForHeight,
																gfloat *outMinWidth,
																gfloat *outNaturalWidth)
{
	...
	if(priv->monitor)
	{
		esdashboard_window_tracker_monitor_get_geometry(priv->monitor, NULL, NULL, &w, NULL);
...
/* Dispose this object */
static void _esdashboard_stage_interface_dispose(GObject *inObject)
{
	EsdashboardStageInterface			*self=ESDASHBOARD_STAGE_INTERFACE(inObject);
	EsdashboardStageInterfacePrivate	*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_stage_interface_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_stage_interface_set_property(GObject *inObject,
														guint inPropID,
														...
														GParamSpec *inSpec)
{
	...
	{
		case PROP_MONITOR:
			esdashboard_stage_interface_set_monitor(self, ESDASHBOARD_WINDOW_TRACKER_MONITOR(g_value_get_object(inValue)));
			break;

		case PROP_BACKGROUND_IMAGE_TYPE:
			esdashboard_stage_interface_set_background_image_type(self, g_value_get_enum(inValue));
			break;

		case PROP_BACKGROUND_COLOR:
			esdashboard_stage_interface_set_background_color(self, clutter_value_get_color(inValue));
			break;
...
static void _esdashboard_stage_interface_get_property(GObject *inObject,
														guint inPropID,
														GValue *outValue,
														GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_stage_interface_class_init(EsdashboardStageInterfaceClass *klass)
{
	...
	/* Override functions */
	clutterActorClass->parent_set=esdashboard_stage_interface_parent_set;
	clutterActorClass->get_preferred_width=_esdashboard_stage_interface_get_preferred_width;
	clutterActorClass->get_preferred_height=_esdashboard_stage_interface_get_preferred_height;

	gobjectClass->dispose=_esdashboard_stage_interface_dispose;
	gobjectClass->set_property=_esdashboard_stage_interface_set_property;
	gobjectClass->get_property=_esdashboard_stage_interface_get_property;

	/* Define properties */
	EsdashboardStageInterfaceProperties[PROP_MONITOR]=
	...
	EsdashboardStageInterfaceProperties[PROP_BACKGROUND_IMAGE_TYPE]=
	...
	EsdashboardStageInterfaceProperties[PROP_BACKGROUND_COLOR]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardStageInterfaceProperties);

	/* Define stylable properties */
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardStageInterfaceProperties[PROP_BACKGROUND_IMAGE_TYPE]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardStageInterfaceProperties[PROP_BACKGROUND_COLOR]);
}

/* Object initialization
 * Create private structure and set up default values
 */
static void esdashboard_stage_interface_init(EsdashboardStageInterface *self)
{
	EsdashboardStageInterfacePrivate	*priv;

	priv=self->priv=esdashboard_stage_interface_get_instance_private(self);
...
{
	return(CLUTTER_ACTOR(g_object_new(ESDASHBOARD_TYPE_STAGE_INTERFACE, NULL)));
}

/* Get/set monitor */
EsdashboardWindowTrackerMonitor* esdashboard_stage_interface_get_monitor(EsdashboardStageInterface *self)
{
	g_return_val_if_fail(ESDASHBOARD_IS_STAGE_INTERFACE(self), NULL);
...
void esdashboard_stage_interface_set_monitor(EsdashboardStageInterface *self, EsdashboardWindowTrackerMonitor *inMonitor)
{
	...
	if(priv->monitor!=inMonitor)
	{
		...
		priv->monitor=ESDASHBOARD_WINDOW_TRACKER_MONITOR(g_object_ref(inMonitor));

		/* Connect signals */
		priv->geometryChangedID=g_signal_connect_swapped(priv->monitor,
															"geometry-changed",
															G_CALLBACK(_esdashboard_stage_interface_on_geometry_changed),
															self);
		priv->primaryChangedID=g_signal_connect_swapped(priv->monitor,
															"primary-changed",
															G_CALLBACK(_esdashboard_stage_interface_on_primary_changed),
															self);

		/* Resize actor to new monitor */
		_esdashboard_stage_interface_on_geometry_changed(self, priv->monitor);

		/* Update actor from primary state */
		_esdashboard_stage_interface_on_primary_changed(self, priv->monitor);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardStageInterfaceProperties[PROP_MONITOR]);
	}
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_STAGE_INTERFACE(self), ESDASHBOARD_STAGE_BACKGROUND_IMAGE_TYPE_NONE);
...
void esdashboard_stage_interface_set_background_image_type(EsdashboardStageInterface *self, EsdashboardStageBackgroundImageType inType)
{
	...
	if(priv->backgroundType!=inType)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardStageInterfaceProperties[PROP_BACKGROUND_IMAGE_TYPE]);
	}
}
...
void esdashboard_stage_interface_set_background_color(EsdashboardStageInterface *self, const ClutterColor *inColor)
{
		...
		(inColor && clutter_color_equal(inColor, priv->backgroundColor)==FALSE))
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardStageInterfaceProperties[PROP_BACKGROUND_COLOR]);
	}
}
