...
static void _esdashboard_desktop_app_info_gappinfo_iface_init(GAppInfoIface *iface);

struct _EsdashboardDesktopAppInfoPrivate
{
	/* Properties related */
	gchar				*desktopID;
	GFile				*file;
...
G_DEFINE_TYPE_WITH_CODE(EsdashboardDesktopAppInfo,
						esdashboard_desktop_app_info,
						G_TYPE_OBJECT,
						G_ADD_PRIVATE(EsdashboardDesktopAppInfo)
						G_IMPLEMENT_INTERFACE(G_TYPE_APP_INFO, _esdashboard_desktop_app_info_gappinfo_iface_init))
...
static GParamSpec* EsdashboardDesktopAppInfoProperties[PROP_LAST]={ 0, };
...
static guint EsdashboardDesktopAppInfoSignals[SIGNAL_LAST]={ 0, };
...
typedef struct
{
	...
	gchar	*desktopFile;
} EsdashboardDesktopAppInfoChildSetupData;
...
static gboolean _esdashboard_desktop_app_info_load_secondary_source(EsdashboardDesktopAppInfo *self)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(self), FALSE);
...
static void _esdashboard_desktop_app_info_update_binary_executable(EsdashboardDesktopAppInfo *self)
{
	EsdashboardDesktopAppInfoPrivate		*priv;

	g_return_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(self));
...
static void _esdashboard_desktop_app_info_update_actions(EsdashboardDesktopAppInfo *self)
{
	EsdashboardDesktopAppInfoPrivate		*priv;
	 ...
	 */
	if(_esdashboard_desktop_app_info_load_secondary_source(self))
	{
		gchar								**itemActions;
		gchar								**iter;
...
/* (Re-)Load keywords */
static void _esdashboard_desktop_app_info_update_keywords(EsdashboardDesktopAppInfo *self)
{
	EsdashboardDesktopAppInfoPrivate		*priv;
...
static void _esdashboard_desktop_app_info_on_item_changed(EsdashboardDesktopAppInfo *self,
															gpointer inUserData)
{
	g_return_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(self));

	/* Emit 'changed' signal for this desktop app info */
	g_signal_emit(self, EsdashboardDesktopAppInfoSignals[SIGNAL_CHANGED], 0);
}

/* Set desktop ID */
static void _esdashboard_desktop_app_info_set_desktop_id(EsdashboardDesktopAppInfo *self,
															const gchar *inDesktopID)
{
	...
	if(g_strcmp0(priv->desktopID, inDesktopID))
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDesktopAppInfoProperties[PROP_DESKTOP_ID]);
	}
}

/* Set desktop file */
static void _esdashboard_desktop_app_info_set_file(EsdashboardDesktopAppInfo *self,
													GFile *inFile)
{
	EsdashboardDesktopAppInfoPrivate	*priv;
	...
	if(!(priv->file && inFile && g_file_equal(priv->file, inFile)))
	{
		...
		if(priv->item)
		{
			priv->itemChangedID=g_signal_connect_swapped(priv->item,
															"changed",
															G_CALLBACK(_esdashboard_desktop_app_info_on_item_changed),
															self);
		}

		/* Get path to executable file for this application */
		_esdashboard_desktop_app_info_update_binary_executable(self);
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDesktopAppInfoProperties[PROP_FILE]);
		...
		if(priv->isValid!=valid)
		{
			...
			/* Notify about property change */
			g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDesktopAppInfoProperties[PROP_VALID]);
		}
		...
		else if(priv->inited && priv->file && inFile && g_file_equal(priv->file, inFile))
		{
			...
			/* Reload desktop file */
			valid=esdashboard_desktop_app_info_reload(self);
...
#if !LIBEXPIDUS1UTIL_CHECK_VERSION(4, 15, 2)

/* Launch application with URIs in given context */
static void _esdashboard_desktop_app_info_expand_macros_add_file(const gchar *inURI, GString *ioExpanded)
{
	GFile		*file;
	gchar		*path;
	gchar		*quotedPath;
...
static void _esdashboard_desktop_app_info_expand_macros_add_uri(const gchar *inURI, GString *ioExpanded)
{
	gchar		*quotedURI;
...
static gboolean _esdashboard_desktop_app_info_expand_macros(EsdashboardDesktopAppInfo *self,
															...
															GString *ioExpanded)
{
	...
	while(*inCommand)
	{
		...
		if(*inCommand=='%')
		{
			...
			{
				case 'f':
					if(inURIs) _esdashboard_desktop_app_info_expand_macros_add_file(inURIs->data, ioExpanded);
					filesOrUriAdded=TRUE;
					break;

				case 'F':
					g_list_foreach(inURIs, (GFunc)_esdashboard_desktop_app_info_expand_macros_add_file, ioExpanded);
					filesOrUriAdded=TRUE;
					break;

				case 'u':
					if(inURIs) _esdashboard_desktop_app_info_expand_macros_add_uri(inURIs->data, ioExpanded);
					filesOrUriAdded=TRUE;
					break;

				case 'U':
					g_list_foreach(inURIs, (GFunc)_esdashboard_desktop_app_info_expand_macros_add_uri, ioExpanded);
					filesOrUriAdded=TRUE;
					break;
	...
	{
		g_string_append_c(ioExpanded, ' ');
		 _esdashboard_desktop_app_info_expand_macros_add_file(inURIs->data, ioExpanded);
	}
 ...
 */
static void _esdashboard_desktop_app_info_on_child_spawned(gpointer inUserData)
{
	EsdashboardDesktopAppInfoChildSetupData		*data=(EsdashboardDesktopAppInfoChildSetupData*)inUserData;
...
static gboolean _esdashboard_desktop_app_info_launch_appinfo_internal(EsdashboardDesktopAppInfo *self,
																		const gchar *inCommand,
																		GList *inURIs,
																		...
																		GError **outError)
{
	...
	gchar										**argv;
	GError										*error;
	EsdashboardDesktopAppInfoChildSetupData		childSetup;
	...
	if(!expanded ||
		!_esdashboard_desktop_app_info_expand_macros(self, inCommand, inURIs, expanded))
	{
		/* Set error */
	...
	success=g_spawn_async(workingDirectory,
							...
							NULL,
							G_SPAWN_SEARCH_PATH,
							_esdashboard_desktop_app_info_on_child_spawned,
							&childSetup,
							&launchedPID,
							&error);
	if(success)
																		...
																		GError **outError)
{
  ...
  g_free(uri);
	...
	if(inURIs && !g_regex_match_simple("%[fu]", inCommand, G_REGEX_CASELESS, 0))
	{
		...
		expidus_append_quoted(string, filename);
		g_free(filename);
...
static GAppInfo* _esdashboard_desktop_app_info_gappinfo_dup(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;

	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(inAppInfo), NULL);

	self=ESDASHBOARD_DESKTOP_APP_INFO(inAppInfo);
	priv=self->priv;

	/* Create and return a newly allocated copy */
	return(G_APP_INFO(g_object_new(ESDASHBOARD_TYPE_DESKTOP_APP_INFO,
...
static gboolean _esdashboard_desktop_app_info_gappinfo_equal(GAppInfo *inLeft, GAppInfo *inRight)
{
	EsdashboardDesktopAppInfo			*left;
	EsdashboardDesktopAppInfo			*right;

	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(inLeft), FALSE);
	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(inRight), FALSE);

	/* Get object instance for this class of both GAppInfos */
	left=ESDASHBOARD_DESKTOP_APP_INFO(inLeft);
	right=ESDASHBOARD_DESKTOP_APP_INFO(inRight);
...
/* Get ID of GAppInfo */
static const gchar* _esdashboard_desktop_app_info_gappinfo_get_id(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static const gchar* _esdashboard_desktop_app_info_gappinfo_get_name(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static const gchar* _esdashboard_desktop_app_info_gappinfo_get_description(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static const gchar* _esdashboard_desktop_app_info_gappinfo_get_executable(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static GIcon* _esdashboard_desktop_app_info_gappinfo_get_icon(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
	GIcon								*icon;
...
static gboolean _esdashboard_desktop_app_info_gappinfo_supports_uris(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
	gboolean							result;
	...
	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(inAppInfo), FALSE);
...
static gboolean _esdashboard_desktop_app_info_gappinfo_supports_files(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
	gboolean							result;
...
static gboolean _esdashboard_desktop_app_info_gappinfo_launch(GAppInfo *inAppInfo,
																GList *inFiles,
																GAppLaunchContext *inContext,
																GError **outError)
{
...
static gboolean _esdashboard_desktop_app_info_gappinfo_launch_uris(GAppInfo *inAppInfo,
																	GList *inURIs,
																	GAppLaunchContext *inContext,
																	GError **outError)
{
...
static gboolean _esdashboard_desktop_app_info_gappinfo_should_show(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static const gchar* _esdashboard_desktop_app_info_gappinfo_get_commandline(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static const gchar* _esdashboard_desktop_app_info_gappinfo_get_display_name(GAppInfo *inAppInfo)
{
	EsdashboardDesktopAppInfo			*self;
	EsdashboardDesktopAppInfoPrivate	*priv;
...
static void _esdashboard_desktop_app_info_gappinfo_iface_init(GAppInfoIface *iface)
{
	iface->dup=_esdashboard_desktop_app_info_gappinfo_dup;
	iface->equal=_esdashboard_desktop_app_info_gappinfo_equal;
	iface->get_id=_esdashboard_desktop_app_info_gappinfo_get_id;
	iface->get_name=_esdashboard_desktop_app_info_gappinfo_get_name;
	iface->get_description=_esdashboard_desktop_app_info_gappinfo_get_description;
	iface->get_executable=_esdashboard_desktop_app_info_gappinfo_get_executable;
	iface->get_icon=_esdashboard_desktop_app_info_gappinfo_get_icon;
	iface->launch=_esdashboard_desktop_app_info_gappinfo_launch;
	iface->supports_uris=_esdashboard_desktop_app_info_gappinfo_supports_uris;
	iface->supports_files=_esdashboard_desktop_app_info_gappinfo_supports_files;
	iface->launch_uris=_esdashboard_desktop_app_info_gappinfo_launch_uris;
	iface->should_show=_esdashboard_desktop_app_info_gappinfo_should_show;
	iface->get_commandline=_esdashboard_desktop_app_info_gappinfo_get_commandline;
	iface->get_display_name=_esdashboard_desktop_app_info_gappinfo_get_display_name;
}

/* IMPLEMENTATION: GObject */

/* Dispose this object */
static void _esdashboard_desktop_app_info_dispose(GObject *inObject)
{
	EsdashboardDesktopAppInfo			*self=ESDASHBOARD_DESKTOP_APP_INFO(inObject);
	EsdashboardDesktopAppInfoPrivate	*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_desktop_app_info_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_desktop_app_info_set_property(GObject *inObject,
														guint inPropID,
														...
														GParamSpec *inSpec)
{
	...
	switch(inPropID)
	{
		case PROP_DESKTOP_ID:
			_esdashboard_desktop_app_info_set_desktop_id(self, g_value_get_string(inValue));
			break;

		case PROP_FILE:
			_esdashboard_desktop_app_info_set_file(self, G_FILE(g_value_get_object(inValue)));
			break;
...
static void _esdashboard_desktop_app_info_get_property(GObject *inObject,
														guint inPropID,
														GValue *outValue,
														GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_desktop_app_info_class_init(EsdashboardDesktopAppInfoClass *klass)
{
	GObjectClass					*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->dispose=_esdashboard_desktop_app_info_dispose;
	gobjectClass->set_property=_esdashboard_desktop_app_info_set_property;
	gobjectClass->get_property=_esdashboard_desktop_app_info_get_property;

	/* Define properties */
	EsdashboardDesktopAppInfoProperties[PROP_VALID]=
	...
	EsdashboardDesktopAppInfoProperties[PROP_DESKTOP_ID]=
	...
	EsdashboardDesktopAppInfoProperties[PROP_FILE]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardDesktopAppInfoProperties);

	/* Define signals */
	EsdashboardDesktopAppInfoSignals[SIGNAL_CHANGED]=
		g_signal_new("changed",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_FIRST,
						G_STRUCT_OFFSET(EsdashboardDesktopAppInfoClass, changed),
						NULL,
						NULL,
 ...
 */
static void esdashboard_desktop_app_info_init(EsdashboardDesktopAppInfo *self)
{
	EsdashboardDesktopAppInfoPrivate	*priv;

	priv=self->priv=esdashboard_desktop_app_info_get_instance_private(self);
...
GAppInfo* esdashboard_desktop_app_info_new_from_desktop_id(const gchar *inDesktopID)
{
	...
	desktopFilename=esdashboard_application_database_get_file_from_desktop_id(inDesktopID);
	if(!desktopFilename)
	{
	...
	instance=ESDASHBOARD_DESKTOP_APP_INFO(g_object_new(ESDASHBOARD_TYPE_DESKTOP_APP_INFO,
														"desktop-id", inDesktopID,
														"file", file,
														NULL));
	ESDASHBOARD_DEBUG(instance, APPLICATIONS,
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(self), NULL);
...
GList* esdashboard_desktop_app_info_get_actions(EsdashboardDesktopAppInfo *self)
{
	...
	_esdashboard_desktop_app_info_update_actions(self);
													...
													GError **outError)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO(self), FALSE);
	g_return_val_if_fail(ESDASHBOARD_IS_DESKTOP_APP_INFO_ACTION(inAction), FALSE);
	g_return_val_if_fail(!inContext || G_IS_APP_LAUNCH_CONTEXT(inContext), FALSE);
	g_return_val_if_fail(outError && *outError==NULL, FALSE);
	 ...
	 */
	actionName=esdashboard_desktop_app_info_action_get_name(inAction);
	success=esdashboard_desktop_app_info_launch_action_by_name(self,
																actionName,
																inContext,
																outError);
															...
															GError **outError)
{
	...
	{
		EsdashboardDesktopAppInfoAction	*iterAction;

		iterAction=ESDASHBOARD_DESKTOP_APP_INFO_ACTION(iter->data);
		if(!iterAction) continue;

		if(g_strcmp0(esdashboard_desktop_app_info_action_get_name(iterAction), inActionName)==0)
		{
			action=iterAction;
		}
	}
	...
	success=_esdashboard_desktop_app_info_launch_appinfo_internal(self,
																	esdashboard_desktop_app_info_action_get_command(action),
																	NULL,
																	inContext,
																	outError);
	if(!success)
...
GList* esdashboard_desktop_app_info_get_keywords(EsdashboardDesktopAppInfo *self)
{
	...
	_esdashboard_desktop_app_info_update_keywords(self);
