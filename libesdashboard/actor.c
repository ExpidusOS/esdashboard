...
static gpointer				esdashboard_actor_parent_class=NULL;
static gint					esdashboard_actor_private_offset=0;

static gpointer esdashboard_actor_get_instance_private(EsdashboardActor *self);
void esdashboard_actor_class_init(EsdashboardActorClass *klass);
void esdashboard_actor_init(EsdashboardActor *self);
void esdashboard_actor_base_class_finalize(EsdashboardActorClass *klass);

struct _EsdashboardActorPrivate
{
	/* Properties related */
	gboolean						canFocus;
	gchar							*effects;
	...
	EsdashboardAnimation			*firstTimeMappedAnimation;
	...
	EsdashboardAnimation			*allocationAnimation;
...
enum
{
	...
	/* Overriden properties of interface: EsdashboardStylable */
	PROP_STYLE_CLASSES,
...
static GParamSpec* EsdashboardActorProperties[PROP_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */
#define ALLOCATION_ANIMATION_SIGNAL		"move-resize"

typedef struct _EsdashboardActorAnimationEntry		EsdashboardActorAnimationEntry;
struct _EsdashboardActorAnimationEntry
{
	gboolean									inDestruction;
	gchar										*signal;
...
#define ESDASHBOARD_ACTOR_PARAM_SPEC_REF		(_esdashboard_actor_param_spec_ref_quark())

static GParamSpecPool		*_esdashboard_actor_stylable_properties_pool=NULL;


/* Quark declarations */
static GQuark _esdashboard_actor_param_spec_ref_quark(void)
{
	return(g_quark_from_static_string("esdashboard-actor-param-spec-ref-quark"));
}


/* Free an animation entry */
static void _esdashboard_actor_animation_entry_free(EsdashboardActorAnimationEntry *inData)
{
...
static void _esdashboard_actor_invalidate_recursive(ClutterActor *inActor)
{
	ClutterActor			*child;
	...
	{
		esdashboard_stylable_invalidate(ESDASHBOARD_STYLABLE(inActor));
	}
	...
	while(clutter_actor_iter_next(&actorIter, &child))
	{
		 ...
		 * be invalidated.
		 */
		_esdashboard_actor_invalidate_recursive(child);
	}
}
 ...
 */
static void _esdashboard_actor_hashtable_get_all_stylable_param_specs(GHashTable *ioHashtable, GObjectClass *inClass, gboolean inRecursive)
{
	GList						*paramSpecs, *entry;
	GObjectClass				*parentClass;

	/* Get list of all parameter specification registered for class */
	paramSpecs=g_param_spec_pool_list_owned(_esdashboard_actor_stylable_properties_pool, G_OBJECT_CLASS_TYPE(inClass));
	...
	if(inRecursive && parentClass) _esdashboard_actor_hashtable_get_all_stylable_param_specs(ioHashtable, parentClass, inRecursive);
}

/* Remove entries from hashtable whose key is a duplicate
 * in other hashtable (given in user data)
 */
static gboolean _esdashboard_actor_hashtable_is_duplicate_key(gpointer inKey,
																gpointer inValue,
																gpointer inUserData)
{
...
static void _esdashboard_actor_first_time_created_animation_done(EsdashboardAnimation *inAnimation,
																	gpointer inUserData)
{
	EsdashboardActor				*self;
	EsdashboardActorPrivate			*priv;

	g_return_if_fail(ESDASHBOARD_IS_ANIMATION(inAnimation));
	g_return_if_fail(ESDASHBOARD_IS_ACTOR(inUserData));

	self=ESDASHBOARD_ACTOR(inUserData);
	priv=self->priv;
...
static void _esdashboard_actor_on_mapped_changed(GObject *inObject,
													GParamSpec *inSpec,
													gpointer inUserData)
{
	EsdashboardActor					*self;
	...
	g_return_if_fail(ESDASHBOARD_IS_ACTOR(inObject));

	self=ESDASHBOARD_ACTOR(inObject);
	priv=self->priv;
	...
	if(clutter_actor_is_mapped(CLUTTER_ACTOR(self)))
	{
		...
		if(!priv->firstTimeMapped)
		{
			...
			if(esdashboard_animation_has_animation(self, "created"))
			{
				/* Set up default initial values for animation */
				initials=esdashboard_animation_defaults_new(4,
															"x", G_TYPE_FLOAT, 0.0f,
															"y", G_TYPE_FLOAT, 0.0f,
				...
				esdashboard_actor_get_allocation_box(self, &finalBox);
				finals=esdashboard_animation_defaults_new(4,
				...
				priv->firstTimeMappedAnimation=esdashboard_animation_new_with_values(self, "created", initials, finals);
				...
				if(!priv->firstTimeMappedAnimation ||
					!esdashboard_animation_get_id(priv->firstTimeMappedAnimation) ||
					esdashboard_animation_is_empty(priv->firstTimeMappedAnimation))
				{
				...
				/* Start animation */
				g_signal_connect_after(priv->firstTimeMappedAnimation, "animation-done", G_CALLBACK(_esdashboard_actor_first_time_created_animation_done), self);
				esdashboard_animation_run(priv->firstTimeMappedAnimation);
...
/* Actor was (re)named */
static void _esdashboard_actor_on_name_changed(GObject *inObject,
												GParamSpec *inSpec,
												gpointer inUserData)
{
	EsdashboardActor			*self;
	 ...
	 */
	_esdashboard_actor_invalidate_recursive(CLUTTER_ACTOR(self));
}

/* Actor's reactive state changed */
static void _esdashboard_actor_on_reactive_changed(GObject *inObject,
													GParamSpec *inSpec,
													gpointer inUserData)
{
	EsdashboardActor			*self;
	...
	{
		esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(self), "insensitive");
	}
		else
		{
			esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(self), "insensitive");
		}
...
static void _esdashboard_actor_update_effects(EsdashboardActor *self, const gchar *inEffects)
{
	EsdashboardActorPrivate		*priv;
	EsdashboardTheme			*theme;
	EsdashboardThemeEffects		*themeEffects;
	gchar						**effectIDs;
	gchar						**iter;
	gchar						*effectsList;

	g_return_if_fail(ESDASHBOARD_IS_ACTOR(self));
	...
	themeEffects=esdashboard_theme_get_effects(theme);
	g_object_ref(themeEffects);

	/* Get array of effect ID to create */
	if(inEffects) effectIDs=esdashboard_split_string(inEffects, " \t\r\n");
	...
	while(iter && *iter)
	{
		 ...
		 * of effect IDs.
		 */
		effect=esdashboard_theme_effects_create_effect(themeEffects, *iter);
		if(effect)
		{
...
static gboolean _esdashboard_actor_focusable_can_focus(EsdashboardFocusable *inFocusable)
{
	EsdashboardActor			*self;
	EsdashboardActorPrivate		*priv;

	g_return_val_if_fail(ESDASHBOARD_IS_FOCUSABLE(inFocusable), FALSE);
	g_return_val_if_fail(ESDASHBOARD_IS_ACTOR(inFocusable), FALSE);

	self=ESDASHBOARD_ACTOR(inFocusable);
	priv=self->priv;
 ...
 */
static void _esdashboard_actor_focusable_iface_init(EsdashboardFocusableInterface *iface)
{
	iface->can_focus=_esdashboard_actor_focusable_can_focus;
}

/* IMPLEMENTATION: Interface EsdashboardStylable */

/* Get stylable properties of actor */
static void _esdashboard_actor_stylable_get_stylable_properties(EsdashboardStylable *inStylable,
																		GHashTable *ioStylableProperties)
{
	...
	_esdashboard_actor_hashtable_get_all_stylable_param_specs(ioStylableProperties,
																G_OBJECT_CLASS(inStylable),
																TRUE);
}

/* Get stylable name of actor */
static const gchar* _esdashboard_actor_stylable_get_name(EsdashboardStylable *inStylable)
{
...
static EsdashboardStylable* _esdashboard_actor_stylable_get_parent(EsdashboardStylable *inStylable)
{
	ClutterActor			*self;
	ClutterActor			*parent;
	...
	if(!ESDASHBOARD_IS_STYLABLE(parent)) return(NULL);

	/* Return stylable parent */
	return(ESDASHBOARD_STYLABLE(parent));
}

/* Get/set style classes of actor */
static const gchar* _esdashboard_actor_stylable_get_classes(EsdashboardStylable *inStylable)
{
	EsdashboardActor			*self;

	g_return_val_if_fail(ESDASHBOARD_IS_ACTOR(inStylable), NULL);

	self=ESDASHBOARD_ACTOR(inStylable);

	return(self->priv->styleClasses);
}

static void _esdashboard_actor_stylable_set_classes(EsdashboardStylable *inStylable, const gchar *inStyleClasses)
{
	EsdashboardActor			*self;
	EsdashboardActorPrivate		*priv;

	g_return_if_fail(ESDASHBOARD_IS_ACTOR(inStylable));
...
static const gchar* _esdashboard_actor_stylable_get_pseudo_classes(EsdashboardStylable *inStylable)
{
	EsdashboardActor			*self;
...
static void _esdashboard_actor_stylable_set_pseudo_classes(EsdashboardStylable *inStylable, const gchar *inStylePseudoClasses)
{
	EsdashboardActor			*self;
	EsdashboardActorPrivate		*priv;
...
static void _esdashboard_actor_stylable_invalidate(EsdashboardStylable *inStylable)
{
	EsdashboardActor			*self;
	EsdashboardActorPrivate		*priv;
	EsdashboardActorClass		*klass;
	EsdashboardTheme			*theme;
	EsdashboardThemeCSS			*themeCSS;
	GHashTable					*possibleStyleSet;
	GParamSpec					*paramSpec;
	GHashTableIter				hashIter;
	GHashTable					*themeStyleSet;
	gchar						*styleName;
	EsdashboardThemeCSSValue	*styleValue;
	gboolean					didChange;
#ifdef DEBUG
	...
	self=ESDASHBOARD_ACTOR(inStylable);
	priv=self->priv;
	klass=ESDASHBOARD_ACTOR_GET_CLASS(self);
	didChange=FALSE;
	...
	/* Get theme CSS */
	themeCSS=esdashboard_theme_get_css(theme);
	 ...
	 */
	possibleStyleSet=esdashboard_actor_get_stylable_properties_full(klass);
	...
	if(doDebug)
	{
		...
		{
			realParamSpec=(GParamSpec*)g_param_spec_get_qdata(paramSpec, ESDASHBOARD_ACTOR_PARAM_SPEC_REF);
		...
		ESDASHBOARD_DEBUG(self, STYLE, "End of param specs");
	}
#endif

	/* Get style information from theme */
	themeStyleSet=esdashboard_theme_css_get_properties(themeCSS, ESDASHBOARD_STYLABLE(self));
	...
	if(doDebug)
	{
		...
		ESDASHBOARD_DEBUG(self, STYLE, "End of styles from theme");
	}
#endif
	...
	if(priv->lastThemeStyleSet)
	{
		...
		g_hash_table_foreach_remove(priv->lastThemeStyleSet, (GHRFunc)_esdashboard_actor_hashtable_is_duplicate_key, themeStyleSet);
...
static void _esdashboard_actor_animation_done(EsdashboardAnimation *inAnimation,
												gpointer inUserData)
{
	EsdashboardActor				*self;
	EsdashboardActorPrivate			*priv;
	GSList							*iter;
	EsdashboardActorAnimationEntry	*data;
	...
	for(iter=priv->animations; iter; iter=g_slist_next(iter))
	{
		...
		data=(EsdashboardActorAnimationEntry*)iter->data;
		if(!data) continue;
		...
		if(data->animation==inAnimation)
		{
			ESDASHBOARD_DEBUG(self, ANIMATION,
									...
									esdashboard_animation_get_id(data->animation));
			...
			data->animation=NULL;
			_esdashboard_actor_animation_entry_free(data);
			g_slist_free_1(iter);
		}
	}
}
...
static void _esdashboard_actor_remove_animation(EsdashboardActor *self,
												const gchar *inAnimationSignal)
{
	...
	for(iter=priv->animations; iter; iter=g_slist_next(iter))
	{
		...
		if(g_strcmp0(data->signal, inAnimationSignal)==0)
		{
			 ...
			 * done callback _esdashboard_actor_animation_done() of animation.
			 */
...
static EsdashboardAnimation* _esdashboard_actor_add_animation(EsdashboardActor *self,
																const gchar *inAnimationSignal)
{
	EsdashboardActorPrivate			*priv;
	EsdashboardAnimation			*animation;
	...
	g_return_val_if_fail(ESDASHBOARD_IS_ACTOR(self), NULL);
	 ...
	 */
	animation=esdashboard_animation_new(ESDASHBOARD_ACTOR(self), inAnimationSignal);
	if(!animation ||
		esdashboard_animation_is_empty(animation))
	{
	...
	if(clutter_actor_get_transition(CLUTTER_ACTOR(self), esdashboard_animation_get_id(animation)))
	{
		ESDASHBOARD_DEBUG(self, ANIMATION,
	...
	/* Start animation */
	g_signal_connect_after(animation, "animation-done", G_CALLBACK(_esdashboard_actor_animation_done), self);
	esdashboard_animation_run(animation);
	ESDASHBOARD_DEBUG(self, ANIMATION,
...
static void _esdashboard_actor_stylable_class_added(EsdashboardStylable *inStylable, const gchar *inClass)
{
	...
	_esdashboard_actor_remove_animation(ESDASHBOARD_ACTOR(inStylable), animationSignal);
	g_free(animationSignal);

	/* Create animation for this class added */
	animationSignal=g_strdup_printf("class-added:%s", inClass);
	_esdashboard_actor_add_animation(ESDASHBOARD_ACTOR(inStylable), animationSignal);
	g_free(animationSignal);
}

/* Signal handler for "class-removed" signal of stylable interface */
static void _esdashboard_actor_stylable_class_removed(EsdashboardStylable *inStylable, const gchar *inClass)
{
	gchar			*animationSignal;
...
static void _esdashboard_actor_stylable_pseudo_class_added(EsdashboardStylable *inStylable, const gchar *inClass)
{
	gchar			*animationSignal;
...
static void _esdashboard_actor_stylable_pseudo_class_removed(EsdashboardStylable *inStylable, const gchar *inClass)
{
	gchar			*animationSignal;
 ...
 * Set up default functions
 */
static void _esdashboard_actor_stylable_iface_init(EsdashboardStylableInterface *iface)
{
	iface->get_stylable_properties=_esdashboard_actor_stylable_get_stylable_properties;
	iface->get_name=_esdashboard_actor_stylable_get_name;
	iface->get_parent=_esdashboard_actor_stylable_get_parent;
	iface->get_classes=_esdashboard_actor_stylable_get_classes;
	iface->set_classes=_esdashboard_actor_stylable_set_classes;
	iface->class_added=_esdashboard_actor_stylable_class_added;
	iface->class_removed=_esdashboard_actor_stylable_class_removed;
	iface->get_pseudo_classes=_esdashboard_actor_stylable_get_pseudo_classes;
	iface->set_pseudo_classes=_esdashboard_actor_stylable_set_pseudo_classes;
	iface->pseudo_class_added=_esdashboard_actor_stylable_pseudo_class_added;
	iface->pseudo_class_removed=_esdashboard_actor_stylable_pseudo_class_removed;
	iface->invalidate=_esdashboard_actor_stylable_invalidate;
}

/* IMPLEMENTATION: ClutterActor */

/* Allocation animation ended */
static void _esdashboard_actor_on_allocation_animation_done(EsdashboardAnimation *inAnimation,
															gpointer inUserData)
{
	EsdashboardActor				*self;
	EsdashboardActorPrivate			*priv;
...
static void _esdashboard_actor_on_allocation_changed(ClutterActor *inActor,
														...
														gpointer inUserData)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_ACTOR(inActor));
	g_return_if_fail(inAllocationBox);

	self=ESDASHBOARD_ACTOR(inActor);
	priv=self->priv;
		...
		!priv->disallowAllocationAnimation)
	{
		...
		animation=esdashboard_animation_new_with_values(self,
		...
		if(animation)
		{
			...
			if(!esdashboard_animation_is_empty(animation))
			{
				...
				g_signal_connect_after(animation,
										"animation-done",
										G_CALLBACK(_esdashboard_actor_on_allocation_animation_done),
										inActor);
				esdashboard_animation_run(priv->allocationAnimation);
...
/* Pointer left actor */
static gboolean _esdashboard_actor_leave_event(ClutterActor *inActor, ClutterCrossingEvent *inEvent)
{
	EsdashboardActor		*self;
	ClutterActorClass		*parentClass;

	g_return_val_if_fail(ESDASHBOARD_IS_ACTOR(inActor), CLUTTER_EVENT_PROPAGATE);

	self=ESDASHBOARD_ACTOR(inActor);

	/* Call parent's virtual function */
	parentClass=CLUTTER_ACTOR_CLASS(esdashboard_actor_parent_class);
	...
	esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(self), "hover");

	return(CLUTTER_EVENT_PROPAGATE);
}

/* Pointer entered actor */
static gboolean _esdashboard_actor_enter_event(ClutterActor *inActor, ClutterCrossingEvent *inEvent)
{
	EsdashboardActor		*self;
	ClutterActorClass		*parentClass;
	...
	esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(self), "hover");

	return(CLUTTER_EVENT_PROPAGATE);
}

/* Actor was (re)parented */
static void _esdashboard_actor_parent_set(ClutterActor *inActor, ClutterActor *inOldParent)
{
	EsdashboardActor				*self;
	EsdashboardActorPrivate			*priv;
	ClutterActorClass				*parentClass;
...
/* Actor will be shown */
static EsdashboardAnimation* _esdashboard_actor_replace_animation(EsdashboardActor *self,
																	const gchar *inOldSignal,
																	const gchar *inNewSignal)
{
	EsdashboardActorPrivate				*priv;
	EsdashboardAnimation				*oldAnimation;
	EsdashboardAnimation				*newAnimation;
	GSList								*iter;
	EsdashboardActorAnimationEntry		*data;
	...
	newAnimation=_esdashboard_actor_add_animation(self, inNewSignal);
	...
	if(oldAnimation)
	{
		 ...
		 */
		if(!newAnimation ||
			!esdashboard_animation_get_id(newAnimation))
		{
			esdashboard_animation_ensure_complete(oldAnimation);
		}
...
static void _esdashboard_actor_show(ClutterActor *inActor)
{
	EsdashboardActor		*self;
	ClutterActorClass		*parentClass;
	...
	_esdashboard_actor_replace_animation(self, "hide", "show");
}

/* Actor will be hidden */
static void _esdashboard_actor_hide_on_animation_done(EsdashboardActor *inActor, gpointer inUserData)
{
	ClutterActorClass		*parentClass;
...
static void _esdashboard_actor_hide(ClutterActor *inActor)
{
	EsdashboardActor		*self;
	EsdashboardAnimation	*animation;
	 ...
	 */
	animation=_esdashboard_actor_replace_animation(self, "show", "hide");
	if(animation) g_signal_connect_swapped(animation, "animation-done", G_CALLBACK(_esdashboard_actor_hide_on_animation_done), self);
		else _esdashboard_actor_hide_on_animation_done(self, NULL);
}
...
/* Dispose this object */
static void _esdashboard_actor_dispose(GObject *inObject)
{
	EsdashboardActor			*self=ESDASHBOARD_ACTOR(inObject);
	EsdashboardActorPrivate		*priv=self->priv;
	...
	if(priv->animations)
	{
		g_slist_free_full(priv->animations, (GDestroyNotify)_esdashboard_actor_animation_entry_free);
	...
	G_OBJECT_CLASS(esdashboard_actor_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_actor_set_property(GObject *inObject,
											guint inPropID,
											...
											GParamSpec *inSpec)
{
	...
	{
		case PROP_CAN_FOCUS:
			esdashboard_actor_set_can_focus(self, g_value_get_boolean(inValue));
			break;

		case PROP_EFFECTS:
			esdashboard_actor_set_effects(self, g_value_get_string(inValue));
			break;

		case PROP_STYLE_CLASSES:
			_esdashboard_actor_stylable_set_classes(ESDASHBOARD_STYLABLE(self),
													g_value_get_string(inValue));
			break;

		case PROP_STYLE_PSEUDO_CLASSES:
			_esdashboard_actor_stylable_set_pseudo_classes(ESDASHBOARD_STYLABLE(self),
															g_value_get_string(inValue));
			break;
...
static void _esdashboard_actor_get_property(GObject *inObject,
											guint inPropID,
											GValue *outValue,
											GParamSpec *inSpec)
{
...
static inline gpointer esdashboard_actor_get_instance_private(EsdashboardActor *self)
{
	return(G_STRUCT_MEMBER_P(self, esdashboard_actor_private_offset));
}
...
void esdashboard_actor_class_init(EsdashboardActorClass *klass)
{
	...
	/* Get parent class */
	esdashboard_actor_parent_class=g_type_class_peek_parent(klass);

	/* Adjust offset to instance private structure */
	if(esdashboard_actor_private_offset!=0)
	{
		g_type_class_adjust_private_offset(klass, &esdashboard_actor_private_offset);
	}

	/* Override functions */
	gobjectClass->dispose=_esdashboard_actor_dispose;
	gobjectClass->set_property=_esdashboard_actor_set_property;
	gobjectClass->get_property=_esdashboard_actor_get_property;

	clutterActorClass->parent_set=_esdashboard_actor_parent_set;
	clutterActorClass->enter_event=_esdashboard_actor_enter_event;
	clutterActorClass->leave_event=_esdashboard_actor_leave_event;
	clutterActorClass->show=_esdashboard_actor_show;
	clutterActorClass->hide=_esdashboard_actor_hide;

	/* Create param-spec pool for themable properties */
	g_assert(_esdashboard_actor_stylable_properties_pool==NULL);
	_esdashboard_actor_stylable_properties_pool=g_param_spec_pool_new(FALSE);

	/* Define properties */
	EsdashboardActorProperties[PROP_CAN_FOCUS]=
		g_param_spec_boolean("can-focus",
								"Can focus",
								"This flag indicates if this actor can be focused",
								FALSE,
								G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
	g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_CAN_FOCUS, EsdashboardActorProperties[PROP_CAN_FOCUS]);

	EsdashboardActorProperties[PROP_EFFECTS]=
		g_param_spec_string("effects",
								"Effects",
								"List of space-separated strings with IDs of effects set at this actor",
								NULL,
								G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
	g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_EFFECTS, EsdashboardActorProperties[PROP_EFFECTS]);
	...
	esdashboard_actor_install_stylable_property_by_name(klass, "effects");
	esdashboard_actor_install_stylable_property_by_name(klass, "x-expand");
	esdashboard_actor_install_stylable_property_by_name(klass, "y-expand");
	esdashboard_actor_install_stylable_property_by_name(klass, "x-align");
	esdashboard_actor_install_stylable_property_by_name(klass, "y-align");
	esdashboard_actor_install_stylable_property_by_name(klass, "margin-top");
	esdashboard_actor_install_stylable_property_by_name(klass, "margin-bottom");
	esdashboard_actor_install_stylable_property_by_name(klass, "margin-left");
	esdashboard_actor_install_stylable_property_by_name(klass, "margin-right");
}
...
void esdashboard_actor_init(EsdashboardActor *self)
{
	...
	priv=self->priv=esdashboard_actor_get_instance_private(self);
	...
	/* Connect signals */
	g_signal_connect(self, "notify::mapped", G_CALLBACK(_esdashboard_actor_on_mapped_changed), NULL);
	g_signal_connect(self, "notify::name", G_CALLBACK(_esdashboard_actor_on_name_changed), NULL);
	g_signal_connect(self, "notify::reactive", G_CALLBACK(_esdashboard_actor_on_reactive_changed), NULL);
	g_signal_connect(self, "allocation-changed", G_CALLBACK(_esdashboard_actor_on_allocation_changed), NULL);
}
...
void esdashboard_actor_base_class_finalize(EsdashboardActorClass *klass)
{
	...
	paramSpecs=g_param_spec_pool_list_owned(_esdashboard_actor_stylable_properties_pool, G_OBJECT_CLASS_TYPE(klass));
	for(entry=paramSpecs; entry; entry=g_list_next(entry))
	{
		GParamSpec		*paramSpec=G_PARAM_SPEC(entry->data);
		...
		if(paramSpec)
		{
			g_param_spec_pool_remove(_esdashboard_actor_stylable_properties_pool, paramSpec);

			ESDASHBOARD_DEBUG(NULL, STYLE,
...
GType esdashboard_actor_get_type(void)
{
	...
	if(G_UNLIKELY(actorType==0))
	{
		...
		{
			sizeof(EsdashboardActorClass),
			NULL,
			(GBaseFinalizeFunc)(void*)esdashboard_actor_base_class_finalize,
			(GClassInitFunc)(void*)esdashboard_actor_class_init,
			NULL, /* class_finalize */
			NULL, /* class_data */
			sizeof(EsdashboardActor),
			0,    /* n_preallocs */
			(GInstanceInitFunc)(void*)esdashboard_actor_init,
			NULL, /* value_table */
		};
		...
		{
			(GInterfaceInitFunc)(void*)_esdashboard_actor_stylable_iface_init,
			NULL,
			NULL
		};
		...
		{
			(GInterfaceInitFunc)(void*)_esdashboard_actor_focusable_iface_init,
			NULL,
			NULL
		};
		...
		actorType=g_type_register_static(CLUTTER_TYPE_ACTOR,
											g_intern_static_string("EsdashboardActor"),
											&actorInfo,
											0);

		/* Add private structure */
		esdashboard_actor_private_offset=g_type_add_instance_private(actorType, sizeof(EsdashboardActorPrivate));

		/* Add implemented interfaces */
		g_type_add_interface_static(actorType,
									ESDASHBOARD_TYPE_STYLABLE,
...
{
	return(CLUTTER_ACTOR(g_object_new(ESDASHBOARD_TYPE_ACTOR, NULL)));
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_ACTOR(self), FALSE);
...
void esdashboard_actor_set_can_focus(EsdashboardActor *self, gboolean inCanFous)
{
	...
	if(priv->canFocus!=inCanFous)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardActorProperties[PROP_CAN_FOCUS]);
	}
}
...
void esdashboard_actor_set_effects(EsdashboardActor *self, const gchar *inEffects)
{
	...
	{
		/* Set value */
		_esdashboard_actor_update_effects(self, inEffects);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardActorProperties[PROP_EFFECTS]);
	}
}
...
void esdashboard_actor_install_stylable_property(EsdashboardActorClass *klass, GParamSpec *inParamSpec)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_ACTOR_CLASS(klass));
	...
	if(g_param_spec_pool_lookup(_esdashboard_actor_stylable_properties_pool, g_param_spec_get_name(inParamSpec), G_OBJECT_CLASS_TYPE(klass), FALSE))
	{
	...
	g_param_spec_set_qdata_full(stylableParamSpec,
									ESDASHBOARD_ACTOR_PARAM_SPEC_REF,
									g_param_spec_ref(inParamSpec),
									(GDestroyNotify)g_param_spec_unref);
	g_param_spec_pool_insert(_esdashboard_actor_stylable_properties_pool, stylableParamSpec, G_OBJECT_CLASS_TYPE(klass));
	ESDASHBOARD_DEBUG(NULL, STYLE,
...
void esdashboard_actor_install_stylable_property_by_name(EsdashboardActorClass *klass, const gchar *inParamName)
{
	...
	if(paramSpec) esdashboard_actor_install_stylable_property(klass, paramSpec);
		else
		{
...
GHashTable* esdashboard_actor_get_stylable_properties(EsdashboardActorClass *klass)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_ACTOR_CLASS(klass), NULL);
	...
	stylableProps=g_hash_table_new_full(g_str_hash,
											...
											g_free,
											(GDestroyNotify)g_param_spec_unref);
	_esdashboard_actor_hashtable_get_all_stylable_param_specs(stylableProps, G_OBJECT_CLASS(klass), FALSE);
...
GHashTable* esdashboard_actor_get_stylable_properties_full(EsdashboardActorClass *klass)
{
	...
	stylableProps=g_hash_table_new_full(g_str_hash,
											...
											g_free,
											(GDestroyNotify)g_param_spec_unref);
	_esdashboard_actor_hashtable_get_all_stylable_param_specs(stylableProps, G_OBJECT_CLASS(klass), TRUE);
 ...
 */
static void _esdashboard_actor_on_destroy_animation_done(EsdashboardAnimation *inAnimation,
															gpointer inUserData)
{
	ClutterActor				*self;
...
gboolean esdashboard_actor_destroy(ClutterActor *self)
{
	...
	/* Check if an animation exists but only for actors derived from EsdashboardActor */
	if(ESDASHBOARD_IS_ACTOR(self))
	{
		animation=esdashboard_animation_new(ESDASHBOARD_ACTOR(self), "destroy");
	}
		...
		!esdashboard_animation_is_empty(animation))
	{
		...
		g_signal_connect_after(animation,
								"animation-done",
								G_CALLBACK(_esdashboard_actor_on_destroy_animation_done),
								self);

		/* Set CSS pseudo-class that this actor is going to be destroyed */
		esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(self), "destroying");
...
void esdashboard_actor_destroy_all_children(ClutterActor *self)
{
	...
	{
		esdashboard_actor_iter_destroy(&iter);
	}
