...
struct _EsdashboardDragActionPrivate
{
	/* Properties related */
	ClutterActor			*source;
	ClutterActor			*actor;
	...
	GSList					*targets;
	EsdashboardDropAction	*lastDropTarget;
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardDragAction,
							esdashboard_drag_action,
...
GParamSpec* EsdashboardDragActionProperties[PROP_LAST]={ 0, };
...
guint EsdashboardDragActionSignals[SIGNAL_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Sort drop action targets */
static gint _esdashboard_drag_action_sort_targets_callback(gconstpointer inLeft, gconstpointer inRight)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_DROP_ACTION(inLeft) && ESDASHBOARD_IS_DROP_ACTION(inRight), 0);
...
static void _esdashboard_drag_action_sort_targets(EsdashboardDragAction *self)
{
	EsdashboardDragActionPrivate	*priv;

	g_return_if_fail(ESDASHBOARD_IS_DRAG_ACTION(self));

	priv=self->priv;

	/* Sort targets */
	priv->targets=g_slist_sort(priv->targets, _esdashboard_drag_action_sort_targets_callback);
}

/* Transform stage coordinates to drop action's target actor coordinates */
static void _esdashboard_drag_action_transform_stage_point(EsdashboardDropAction *inDropTarget,
															gfloat inStageX,
															gfloat inStageY,
															gfloat *outActorX,
															gfloat *outActorY)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_DROP_ACTION(inDropTarget));
...
static EsdashboardDropAction* _esdashboard_drag_action_find_drop_traget_at_coord(EsdashboardDragAction *self,
																					gfloat inStageX,
																					gfloat inStageY)
{
	EsdashboardDragActionPrivate	*priv;
	...
	g_return_val_if_fail(ESDASHBOARD_IS_DRAG_ACTION(self), NULL);
	...
	for(list=priv->targets; list; list=g_slist_next(list))
	{
		...
		{
			return(ESDASHBOARD_DROP_ACTION(actorMeta));
		}
	}
...
static void _esdashboard_drag_on_drop_target_allocation_changed(EsdashboardDragAction *self,
																...
																gpointer inUserData)
{
	...
	_esdashboard_drag_action_sort_targets(self);
}

/* Set source actor */
static void _esdashboard_drag_action_set_source(EsdashboardDragAction *self, ClutterActor *inSource)
{
	EsdashboardDragActionPrivate	*priv;
	...
	{
		/* Unset style */
		if(ESDASHBOARD_IS_ACTOR(priv->source))
		{
			esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(priv->source), "current-drag-source");
		}
...
static void _esdashboard_drag_action_on_dragged_actor_destroyed(EsdashboardDragAction *self,
																gpointer inUserData)
{
	EsdashboardDragActionPrivate	*priv;
	gfloat							x, y;
 ...
 */
static void _esdashboard_drag_action_on_motion_actor_destroyed(gpointer inActor,
																gpointer inUserData)
{
	EsdashboardDragAction			*self;
	EsdashboardDragActionPrivate	*priv;
	ClutterActor					*actor;

	g_return_if_fail(CLUTTER_IS_ACTOR(inActor));
	g_return_if_fail(ESDASHBOARD_IS_DRAG_ACTION(inUserData));

	actor=CLUTTER_ACTOR(inActor);
	self=ESDASHBOARD_DRAG_ACTION(inUserData);
	priv=self->priv;

	/* Disconnect signal */
	g_signal_handlers_disconnect_by_func(actor, G_CALLBACK(_esdashboard_drag_action_on_motion_actor_destroyed), self);
...
static void _esdashboard_drag_action_on_drag_handle_changed(EsdashboardDragAction *self,
															GParamSpec *inSpec,
															gpointer inUserData)
{
	EsdashboardDragActionPrivate	*priv;
	...
	if(priv->dragHandle &&
		ESDASHBOARD_IS_ACTOR(priv->dragHandle))
	{
		/* Unset style */
		if(priv->source)
		{
			styleClass=g_strdup_printf("drag-source-%s", G_OBJECT_TYPE_NAME(priv->source));
			esdashboard_stylable_remove_class(ESDASHBOARD_STYLABLE(priv->dragHandle), styleClass);
			g_free(styleClass);
		}
		...
		esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(priv->dragHandle), "drag-handle");
		...
		ESDASHBOARD_IS_ACTOR(priv->dragHandle))
	{
		...
		{
			styleClass=g_strdup_printf("drag-source-%s", G_OBJECT_TYPE_NAME(priv->source));
			esdashboard_stylable_add_class(ESDASHBOARD_STYLABLE(priv->dragHandle), styleClass);
			g_free(styleClass);
		}
		...
		esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(priv->dragHandle), "drag-handle");
	}
...
static void _esdashboard_drag_action_drag_begin(ClutterDragAction *inAction,
												...
												ClutterModifierType inModifiers)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_DRAG_ACTION(inAction));

	self=ESDASHBOARD_DRAG_ACTION(inAction);
	priv=self->priv;
	dragActionClass=CLUTTER_DRAG_ACTION_CLASS(esdashboard_drag_action_parent_class);
	...
	priv->actor=inActor;
	priv->actorDestroySignalID=g_signal_connect_swapped(priv->actor, "destroy", G_CALLBACK(_esdashboard_drag_action_on_dragged_actor_destroyed), self);

	/* Get list of drop targets. It is a new list with all current
	 * drop targets already reffed. So the drop targets will be valid
	 * while dragging
	 */
	priv->targets=esdashboard_drop_action_get_targets();
	...
	while(list)
	{
		EsdashboardDropAction			*dropTarget=ESDASHBOARD_DROP_ACTION(list->data);
	...
	for(list=priv->targets; list; list=g_slist_next(list))
	{
		...
		g_signal_connect_swapped(actor, "allocation-changed", G_CALLBACK(_esdashboard_drag_on_drop_target_allocation_changed), self);
	}
	...
	/* Set styles */
	if(priv->source && ESDASHBOARD_IS_ACTOR(priv->source))
	{
		esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(priv->source), "drag-source");
	}

	if(ESDASHBOARD_IS_ACTOR(priv->actor))
	{
		esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(priv->actor), "dragged");
	}
		...
		ESDASHBOARD_IS_ACTOR(priv->dragHandle))
	{
		...
		priv->dragHandleChangedID=g_signal_connect(self, "notify::drag-handle", G_CALLBACK(_esdashboard_drag_action_on_drag_handle_changed), NULL);
	}
}

/* Dragged actor moved */
static void _esdashboard_drag_action_drag_motion(ClutterDragAction *inAction,
													ClutterActor *inActor,
													gfloat inDeltaX,
													gfloat inDeltaY)
{
	...
	EsdashboardDropAction				*dropTarget;
	gfloat								dropX, dropY;
	...
	dropTarget=_esdashboard_drag_action_find_drop_traget_at_coord(self, stageX, stageY);
	...
	{
		dropX=dropY=0.0f;
		_esdashboard_drag_action_transform_stage_point(priv->lastDropTarget,
														stageX, stageY,
														&dropX, &dropY);
			...
			if(motionActor && newMotionActor)
			{
				while(motionActor)
				{
					...
					if(!g_slist_find(priv->lastMotionActors, motionActor))
					{
						 ...
						 * end of drag.
						 */
						g_signal_connect(motionActor, "destroy", G_CALLBACK(_esdashboard_drag_action_on_motion_actor_destroyed), self);
					}
...
static void _esdashboard_drag_action_drag_end(ClutterDragAction *inAction,
												ClutterActor *inActor,
												gfloat inStageX,
												gfloat inStageY,
												ClutterModifierType inModifiers)
{
	...
	if(priv->source && ESDASHBOARD_IS_ACTOR(priv->source))
	{
		esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(priv->source), "drag-source");
	}

	if(ESDASHBOARD_IS_ACTOR(priv->actor))
	{
		esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(priv->actor), "dragged");
	}
	...
	for(list=priv->targets; list; list=g_slist_next(list))
	{
		EsdashboardDropAction			*target=ESDASHBOARD_DROP_ACTION(list->data);
		ClutterActorMeta				*actorMeta=CLUTTER_ACTOR_META(target);
		ClutterActor					*actor=clutter_actor_meta_get_actor(actorMeta);

		g_signal_handlers_disconnect_by_func(actor, G_CALLBACK(_esdashboard_drag_on_drop_target_allocation_changed), self);
	}

	/* Find drop target at stage coordinate if dragged actor was not destroyed */
	if(!priv->dragCancelled) dropTarget=_esdashboard_drag_action_find_drop_traget_at_coord(self, inStageX, inStageY);

	/* If drop target was found check if we are allowed to drop on it. */
	if(dropTarget)
	{
		/* Transform event coordinates relative to drop target */
		_esdashboard_drag_action_transform_stage_point(dropTarget,
														inStageX, inStageY,
														&dropX, &dropY);
	...
	*/
	...
	{
		g_slist_foreach(priv->lastMotionActors, _esdashboard_drag_action_on_motion_actor_destroyed, self);
...
/* Dispose this object */
static void _esdashboard_drag_action_dispose(GObject *inObject)
{
	EsdashboardDragAction				*self=ESDASHBOARD_DRAG_ACTION(inObject);
	EsdashboardDragActionPrivate		*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_drag_action_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_drag_action_set_property(GObject *inObject,
													guint inPropID,
													...
													GParamSpec *inSpec)
{
	...
	{
		case PROP_SOURCE:
			_esdashboard_drag_action_set_source(self, g_value_get_object(inValue));
			break;
...
static void _esdashboard_drag_action_get_property(GObject *inObject,
													guint inPropID,
													GValue *outValue,
													GParamSpec *inSpec)
{
 ...
 * and signals
 */
void esdashboard_drag_action_class_init(EsdashboardDragActionClass *klass)
{
	ClutterDragActionClass	*dragActionClass=CLUTTER_DRAG_ACTION_CLASS(klass);
	GObjectClass			*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->set_property=_esdashboard_drag_action_set_property;
	gobjectClass->get_property=_esdashboard_drag_action_get_property;
	gobjectClass->dispose=_esdashboard_drag_action_dispose;

	dragActionClass->drag_begin=_esdashboard_drag_action_drag_begin;
	dragActionClass->drag_motion=_esdashboard_drag_action_drag_motion;
	dragActionClass->drag_end=_esdashboard_drag_action_drag_end;

	/* Define properties */
	EsdashboardDragActionProperties[PROP_SOURCE]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardDragActionProperties);

	/* Define signals */
	EsdashboardDragActionSignals[SIGNAL_DRAG_CANCEL]=
		g_signal_new("drag-cancel",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardDragActionClass, drag_cancel),
						NULL,
						NULL,
						_esdashboard_marshal_VOID__OBJECT_FLOAT_FLOAT,
						G_TYPE_NONE,
						3,
						CLUTTER_TYPE_ACTOR,
						G_TYPE_FLOAT,
 ...
 */
void esdashboard_drag_action_init(EsdashboardDragAction *self)
{
	EsdashboardDragActionPrivate	*priv;

	priv=self->priv=esdashboard_drag_action_get_instance_private(self);
...
/* Create new action */
ClutterAction* esdashboard_drag_action_new(void)
{
	return(g_object_new(ESDASHBOARD_TYPE_DRAG_ACTION, NULL));
}
...
ClutterAction* esdashboard_drag_action_new_with_source(ClutterActor *inSource)
{
	...
	return(g_object_new(ESDASHBOARD_TYPE_DRAG_ACTION,
						"source", inSource,
						NULL));
}
