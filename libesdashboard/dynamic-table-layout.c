...
static void _esdashboard_dynamic_table_layout_stylable_iface_init(EsdashboardStylableInterface *inInterface);

struct _EsdashboardDynamicTableLayoutPrivate
{
	/* Properties related */
	gfloat				rowSpacing;
...
G_DEFINE_TYPE_WITH_CODE(EsdashboardDynamicTableLayout,
						esdashboard_dynamic_table_layout,
						CLUTTER_TYPE_LAYOUT_MANAGER,
						G_ADD_PRIVATE(EsdashboardDynamicTableLayout)
						G_IMPLEMENT_INTERFACE(ESDASHBOARD_TYPE_STYLABLE, _esdashboard_dynamic_table_layout_stylable_iface_init));
...
static GParamSpec* EsdashboardDynamicTableLayoutProperties[PROP_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Updates data needed for layout in dynamic mode (dynamic number of columns) */
static void _esdashboard_dynamic_table_layout_update_layout_data_dynamic(EsdashboardDynamicTableLayout *self,
																			...
																			gfloat inHeight)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_DYNAMIC_TABLE_LAYOUT(self));
	...
	if(numberChildren!=priv->numberChildren)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDynamicTableLayoutProperties[PROP_NUMBER_CHILDREN]);
	}
	...
	{
		priv->rows=rows;
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDynamicTableLayoutProperties[PROP_ROWS]);
	}

	if(columns!=priv->columns)
	{
		priv->columns=columns;
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDynamicTableLayoutProperties[PROP_COLUMNS]);
	}
...
static void _esdashboard_dynamic_table_layout_update_layout_data_fixed(EsdashboardDynamicTableLayout *self,
																		ClutterContainer *inContainer,
																		gfloat inWidth,
																		gfloat inHeight)
{
...
static void _esdashboard_dynamic_table_layout_update_layout_data(EsdashboardDynamicTableLayout *self,
																	ClutterContainer *inContainer,
																	gfloat inWidth,
																	gfloat inHeight)
{
	...
	{
		_esdashboard_dynamic_table_layout_update_layout_data_fixed(self,
																	inContainer,
																	inWidth,
																	inHeight);
	}
		...
		{
			_esdashboard_dynamic_table_layout_update_layout_data_dynamic(self,
																			inContainer,
																			inWidth,
																			inHeight);
		}
...
static void _esdashboard_dynamic_table_layout_on_style_revalidated(EsdashboardDynamicTableLayout *self,
																	gpointer inUserData)
{
	g_return_if_fail(ESDASHBOARD_IS_DYNAMIC_TABLE_LAYOUT(self));
...
static void _esdashboard_dynamic_table_layout_get_preferred_width(ClutterLayoutManager *self,
																	ClutterContainer *inContainer,
																	gfloat inForHeight,
																	gfloat *outMinWidth,
																	gfloat *outNaturalWidth)
{
	...
	priv=ESDASHBOARD_DYNAMIC_TABLE_LAYOUT(self)->priv;
	...
	_esdashboard_dynamic_table_layout_update_layout_data(ESDASHBOARD_DYNAMIC_TABLE_LAYOUT(self), inContainer, -1.0f, inForHeight);
...
static void _esdashboard_dynamic_table_layout_get_preferred_height(ClutterLayoutManager *self,
																	ClutterContainer *inContainer,
																	gfloat inForWidth,
																	gfloat *outMinHeight,
																	gfloat *outNaturalHeight)
{
	...
	/* Update data needed for layout */
	_esdashboard_dynamic_table_layout_update_layout_data(ESDASHBOARD_DYNAMIC_TABLE_LAYOUT(self), inContainer, inForWidth, -1.0f);
...
static void _esdashboard_dynamic_table_layout_allocate(ClutterLayoutManager *self,
														ClutterContainer *inContainer,
														...
														ClutterAllocationFlags inFlags)
{
	...
	/* Update data needed for layout */
	_esdashboard_dynamic_table_layout_update_layout_data(ESDASHBOARD_DYNAMIC_TABLE_LAYOUT(self),
															inContainer,
															width,
															height);
...
static void _esdashboard_dynamic_table_layout_set_container(ClutterLayoutManager *self,
															ClutterContainer *inContainer)
{
	EsdashboardDynamicTableLayoutPrivate	*priv;
	...
	if(inContainer)
	{
		...
		priv->styleRevalidationSignalID=g_signal_connect_swapped(priv->container,
																	"style-revalidated",
																	G_CALLBACK(_esdashboard_dynamic_table_layout_on_style_revalidated),
																	self);
	}
...
static void _esdashboard_dynamic_table_layout_stylable_get_stylable_properties(EsdashboardStylable *inStylable,
																				GHashTable *ioStylableProperties)
{
	g_return_if_fail(ESDASHBOARD_IS_STYLABLE(inStylable));

	/* Add stylable properties to hashtable */
	esdashboard_stylable_add_stylable_property(inStylable, ioStylableProperties, "fixed-columns");
}

/* Get parent stylable actor of this stylable object */
static EsdashboardStylable* _esdashboard_dynamic_table_layout_stylable_get_parent(EsdashboardStylable *inStylable)
{
	EsdashboardDynamicTableLayoutPrivate	*priv;

	g_return_val_if_fail(ESDASHBOARD_IS_STYLABLE(inStylable), NULL);
	g_return_val_if_fail(ESDASHBOARD_IS_DYNAMIC_TABLE_LAYOUT(inStylable), NULL);

	priv=ESDASHBOARD_DYNAMIC_TABLE_LAYOUT(inStylable)->priv;

	/* Check if container set in this layout manager is stylable and return the container as parent */
	if(priv->container &&
		ESDASHBOARD_IS_STYLABLE(priv->container))
	{
		return(ESDASHBOARD_STYLABLE(priv->container));
	}
...
static const gchar* _esdashboard_dynamic_table_layout_stylable_get_classes(EsdashboardStylable *inStylable)
{
	/* Not implemented */
	return(NULL);
}

static void _esdashboard_dynamic_table_layout_stylable_set_classes(EsdashboardStylable *inStylable, const gchar *inStyleClasses)
{
	/* Not implemented */
}

/* Get/set style pseudo-classes of this stylable object */
static const gchar* _esdashboard_dynamic_table_layout_stylable_get_pseudo_classes(EsdashboardStylable *inStylable)
{
	/* Not implemented */
	return(NULL);
}

static void _esdashboard_dynamic_table_layout_stylable_set_pseudo_classes(EsdashboardStylable *inStylable, const gchar *inStylePseudoClasses)
{
	/* Not implemented */
}
 ...
 */
void _esdashboard_dynamic_table_layout_stylable_iface_init(EsdashboardStylableInterface *iface)
{
	iface->get_stylable_properties=_esdashboard_dynamic_table_layout_stylable_get_stylable_properties;
	iface->get_parent=_esdashboard_dynamic_table_layout_stylable_get_parent;
	iface->get_classes=_esdashboard_dynamic_table_layout_stylable_get_classes;
	iface->set_classes=_esdashboard_dynamic_table_layout_stylable_set_classes;
	iface->get_pseudo_classes=_esdashboard_dynamic_table_layout_stylable_get_pseudo_classes;
	iface->set_pseudo_classes=_esdashboard_dynamic_table_layout_stylable_set_pseudo_classes;
}
...
/* Dispose this object */
static void _esdashboard_dynamic_table_layout_dispose(GObject *inObject)
{
	EsdashboardDynamicTableLayout			*self=ESDASHBOARD_DYNAMIC_TABLE_LAYOUT(inObject);
	EsdashboardDynamicTableLayoutPrivate	*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_dynamic_table_layout_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_dynamic_table_layout_set_property(GObject *inObject,
															guint inPropID,
															...
															GParamSpec *inSpec)
{
	...
	switch(inPropID)
	{
		case PROP_ROW_SPACING:
			esdashboard_dynamic_table_layout_set_row_spacing(self, g_value_get_float(inValue));
			break;

		case PROP_COLUMN_SPACING:
			esdashboard_dynamic_table_layout_set_column_spacing(self, g_value_get_float(inValue));
			break;

		case PROP_FIXED_COLUMNS:
			esdashboard_dynamic_table_layout_set_fixed_columns(self, g_value_get_int(inValue));
			break;
...
static void _esdashboard_dynamic_table_layout_get_property(GObject *inObject,
															guint inPropID,
															GValue *outValue,
															GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_dynamic_table_layout_class_init(EsdashboardDynamicTableLayoutClass *klass)
{
	...
	/* Override functions */
	layoutClass->set_container=_esdashboard_dynamic_table_layout_set_container;
	layoutClass->get_preferred_width=_esdashboard_dynamic_table_layout_get_preferred_width;
	layoutClass->get_preferred_height=_esdashboard_dynamic_table_layout_get_preferred_height;
	layoutClass->allocate=_esdashboard_dynamic_table_layout_allocate;

	gobjectClass->dispose=_esdashboard_dynamic_table_layout_dispose;
	gobjectClass->set_property=_esdashboard_dynamic_table_layout_set_property;
	gobjectClass->get_property=_esdashboard_dynamic_table_layout_get_property;

	stylableIface=g_type_default_interface_ref(ESDASHBOARD_TYPE_STYLABLE);

	/* Define properties */
	EsdashboardDynamicTableLayoutProperties[PROP_ROW_SPACING]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_COLUMN_SPACING]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_NUMBER_CHILDREN]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_ROWS]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_COLUMNS]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_FIXED_COLUMNS]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_STYLE_CLASSES]=
	...
	EsdashboardDynamicTableLayoutProperties[PROP_STYLE_PSEUDO_CLASSES]=
		g_param_spec_override("style-pseudo-classes", paramSpec);

	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardDynamicTableLayoutProperties);
 ...
 */
static void esdashboard_dynamic_table_layout_init(EsdashboardDynamicTableLayout *self)
{
	EsdashboardDynamicTableLayoutPrivate	*priv;

	priv=self->priv=esdashboard_dynamic_table_layout_get_instance_private(self);
...
{
	return(CLUTTER_LAYOUT_MANAGER(g_object_new(ESDASHBOARD_TYPE_DYNAMIC_TABLE_LAYOUT, NULL)));
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_DYNAMIC_TABLE_LAYOUT(self), 0);
...
void esdashboard_dynamic_table_layout_set_spacing(EsdashboardDynamicTableLayout *self, gfloat inSpacing)
{
	...
	if(priv->rowSpacing!=inSpacing || priv->columnSpacing!=inSpacing)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDynamicTableLayoutProperties[PROP_ROW_SPACING]);

		priv->columnSpacing=inSpacing;
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDynamicTableLayoutProperties[PROP_COLUMN_SPACING]);
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_DYNAMIC_TABLE_LAYOUT(self), 0.0f);
...
void esdashboard_dynamic_table_layout_set_fixed_columns(EsdashboardDynamicTableLayout *self, gint inColumns)
{
	...
	if(priv->fixedColumns!=inColumns)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardDynamicTableLayoutProperties[PROP_FIXED_COLUMNS]);
