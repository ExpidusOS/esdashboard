...
struct _EsdashboardThemeCSSPrivate
{
	/* Instance related */
	gchar		*themePath;
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardThemeCSS,
							esdashboard_theme_css,
							G_TYPE_OBJECT)
...
static GParamSpec* EsdashboardThemeCSSProperties[PROP_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */
typedef enum /*< skip,prefix=ESDASHBOARD_THEME_CSS_SELECTOR_TYPE >*/
{
	ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_NONE=0,
	ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_SELECTOR,
	ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_CONSTANT
} EsdashboardThemeCSSSelectorType;

typedef struct _EsdashboardThemeCSSSelector			EsdashboardThemeCSSSelector;
struct _EsdashboardThemeCSSSelector
{
	EsdashboardThemeCSSSelectorType	type;
	EsdashboardCssSelector			*selector;
	GHashTable						*style;
};

typedef struct _EsdashboardThemeCSSSelectorMatch	EsdashboardThemeCSSSelectorMatch;
struct _EsdashboardThemeCSSSelectorMatch
{
	EsdashboardThemeCSSSelector		*selector;
	gint							score;
};

typedef struct _EsdashboardThemeCSSTableCopyData	EsdashboardThemeCSSTableCopyData;
struct _EsdashboardThemeCSSTableCopyData
{
	GHashTable						*table;
	const gchar						*name;
};

#define ESDASHBOARD_THEME_CSS_FUNCTION_CALLBACK(f)	((EsdashboardThemeCSSFunctionCallback)(f))
typedef gboolean (*EsdashboardThemeCSSFunctionCallback)(EsdashboardThemeCSS *self,
														const gchar *inName,
														GList *inArguments,
														GValue *outResult,
														GError **outError);
...
static void _esdashboard_theme_css_set_error(EsdashboardThemeCSS *self,
												GError **outError,
												EsdashboardThemeCSSErrorEnum inCode,
												const gchar *inFormat,
												...) G_GNUC_PRINTF (4, 5);

static gchar* _esdashboard_theme_css_resolve_at_identifier_internal(EsdashboardThemeCSS *self,
																	GScanner *inScanner,
																	GScanner *inScopeScanner,
																	GList *inScopeSelectors,
																	const gchar *inRecursionIdentifier);

static gchar* _esdashboard_theme_css_resolve_at_identifier_by_string(EsdashboardThemeCSS *self,
																		const gchar *inValue,
																		GScanner *inScopeScanner,
																		GList *inScopeSelectors,
																		const gchar *inRecursionIdentifier);
												...
												...)
{
	...
	/* Create error object */
	tempError=g_error_new_literal(ESDASHBOARD_THEME_CSS_ERROR, inCode, message);
...
static EsdashboardThemeCSSValue* _esdashboard_theme_css_value_new(void)
{
	return(g_slice_new0(EsdashboardThemeCSSValue));
}

/* Destroy css value instance */
static void _esdashboard_theme_css_value_free(EsdashboardThemeCSSValue *self)
{
	g_slice_free(EsdashboardThemeCSSValue, self);
}


/* Copy a property */
static void _esdashboard_theme_css_copy_table(gpointer *inKey, gpointer *inValue, EsdashboardThemeCSSTableCopyData *inData)
{
	EsdashboardThemeCSSValue		*value;

	value=_esdashboard_theme_css_value_new();
...
/* Free selector match */
static void _esdashboard_themes_css_selector_match_free(EsdashboardThemeCSSSelectorMatch *inData)
{
	g_slice_free(EsdashboardThemeCSSSelectorMatch, inData);
}

/* Append strings or characters to string in-place */
static gchar* _esdashboard_theme_css_append_string(gchar *ioString1, const gchar *inString2)
{
	gchar	*tmp;
...
static gchar* _esdashboard_theme_css_append_char(gchar *ioString, gchar inChar)
{
	gchar	*tmp;
	gint	length;
...
/* Destroy selector */
static void _esdashboard_theme_css_selector_free(EsdashboardThemeCSSSelector *inSelector)
{
	...
	g_slice_free(EsdashboardThemeCSSSelector, inSelector);
}

/* Create selector */
static EsdashboardThemeCSSSelector* _esdashboard_theme_css_selector_new(const gchar *inName)
{
	EsdashboardThemeCSSSelector		*selector;

	selector=g_slice_new0(EsdashboardThemeCSSSelector);
	selector->type=ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_NONE;
 ...
 */
static GValue* _esdashboard_theme_css_get_argument(EsdashboardThemeCSS *self,
													GList *inArguments,
													...
													GError **outError)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_THEME_CSS(self), NULL);
	...
	{
		_esdashboard_theme_css_set_error(self,
											outError,
 ...
 */
static gboolean _esdashboard_theme_css_parse_string_to_color_component(EsdashboardThemeCSS *self,
																		...
																		GError **outError)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_THEME_CSS(self), FALSE);
 ...
 */
static gboolean _esdashboard_theme_css_function_try_icons(EsdashboardThemeCSS *self,
															const gchar *inName,
															GList *inArguments,
															GValue *outResult,
															GError **outError)
 ...
 */
static gboolean _esdashboard_theme_css_function_lighter_darker(EsdashboardThemeCSS *self,
																const gchar *inName,
																GList *inArguments,
																GValue *outResult,
																GError **outError)
{
	...
	value=_esdashboard_theme_css_get_argument(self, inArguments, 0, CLUTTER_TYPE_COLOR, &error);
	if(!value)
	{
 ...
 */
static gboolean _esdashboard_theme_css_function_alpha(EsdashboardThemeCSS *self,
														const gchar *inName,
														GList *inArguments,
														GValue *outResult,
														GError **outError)
{
	...
	value=_esdashboard_theme_css_get_argument(self, inArguments, 1, G_TYPE_DOUBLE, &error);
	if(!value)
	{
		/* Release allocated resources */
 ...
 */
static gboolean _esdashboard_theme_css_function_shade(EsdashboardThemeCSS *self,
														const gchar *inName,
														GList *inArguments,
														GValue *outResult,
														GError **outError)
 ...
 * of color1 to color2.
 */
static gboolean _esdashboard_theme_css_function_mix(EsdashboardThemeCSS *self,
													const gchar *inName,
													GList *inArguments,
													GValue *outResult,
													GError **outError)
{
	...
	value=_esdashboard_theme_css_get_argument(self, inArguments, 1, CLUTTER_TYPE_COLOR, &error);
	if(!value)
	{
		/* Release allocated resources */
	...
	value=_esdashboard_theme_css_get_argument(self, inArguments, 2, G_TYPE_DOUBLE, &error);
	if(!value)
	{
		/* Release allocated resources */
 ...
 */
static gboolean _esdashboard_theme_css_function_rgb_rgba(EsdashboardThemeCSS *self,
															const gchar *inName,
															GList *inArguments,
															GValue *outResult,
															GError **outError)
{
	...
	{
		/* Get argument */
		value=_esdashboard_theme_css_get_argument(self, inArguments, i, G_TYPE_STRING, &error);
		if(!value)
		{
			g_propagate_error(outError, error);
			return(FALSE);
		...
		if(!_esdashboard_theme_css_parse_string_to_color_component(self,
	...
	if(isRGBA)
	{
		...
		/* Get alpha factor */
		value=_esdashboard_theme_css_get_argument(self, inArguments, 3, G_TYPE_DOUBLE, &error);
		if(!value)
		{
			/* Propagate error message and return failure result */
			g_propagate_error(outError, error);
...
static void _esdashboard_theme_css_register_function(EsdashboardThemeCSS *self,
														const gchar *inName,
														EsdashboardThemeCSSFunctionCallback inCallback)
{
	EsdashboardThemeCSSPrivate		*priv;

	g_return_if_fail(ESDASHBOARD_IS_THEME_CSS(self));
 ...
 */
static gchar* _esdashboard_theme_css_parse_at_identifier(EsdashboardThemeCSS *self,
															GScanner *ioScanner,
															GScanner *inScopeScanner,
															GList *inScopeSelectors,
															const gchar *inRecursionIdentifier)
{
	...
	gpointer								value;
	gchar									*errorMessage;
	EsdashboardThemeCSSFunctionCallback		functionCallback;
				...
				switch((gint)token)
				{
					case '@':
						...
						if(token==G_TOKEN_IDENTIFIER)
						{
							...
							/* Resolve '@'-identifier */
							resolvedValue=_esdashboard_theme_css_parse_at_identifier(self,
																						ioScanner,
																						inScopeScanner,
																						inScopeSelectors,
							...
							if(resolvedValue)
							{
								...
								finalResolvedValue=_esdashboard_theme_css_resolve_at_identifier_by_string(self,
																											resolvedValue,
																											inScopeScanner,
																											inScopeSelectors,
																											inRecursionIdentifier);
							...
							if(resolvedValue)
							{
								arg=_esdashboard_theme_css_append_string(arg, resolvedValue);
								g_free(resolvedValue);
							}
								else
								{
					...
					case G_TOKEN_IDENTIFIER:
						arg=_esdashboard_theme_css_append_string(arg, ioScanner->value.v_identifier);
						break;

					case G_TOKEN_STRING:
						arg=_esdashboard_theme_css_append_string(arg, ioScanner->value.v_string);
						break;
					...
					default:
						...
						{
							arg=_esdashboard_theme_css_append_char(arg, (gchar)token);
						}
							else
							{
								error=TRUE;
	...
	{
		selector=(EsdashboardThemeCSSSelector*)iter->data;

		/* Handle only constants selectors */
		if(selector->type==ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_CONSTANT)
		{
																	...
																	const gchar *inRecursionIdentifier)
{
	...
	while(token!=G_TOKEN_EOF)
	{
		...
		{
			case G_TOKEN_IDENTIFIER:
				value=_esdashboard_theme_css_append_string(value, inScanner->value.v_identifier);
				break;

			case G_TOKEN_STRING:
				value=_esdashboard_theme_css_append_string(value, inScanner->value.v_string);
				break;
			...
			case G_TOKEN_CHAR:
				...
				if(inScanner->value.v_char=='@')
				{
					...
					constantValue=_esdashboard_theme_css_parse_at_identifier(self, inScanner, inScopeScanner, inScopeSelectors, inRecursionIdentifier);
					if(!constantValue)
					{
						g_free(value);
						return(NULL);
					...
					value=_esdashboard_theme_css_append_string(value, constantValue);

					/* Release allocated resources */
					g_free(constantValue);
				}
					/* ... otherwise just add character to value */
					else value=_esdashboard_theme_css_append_char(value, inScanner->value.v_char);
				break;
	...
	if(haveResolvedAtIdentifier)
	{
		...
		/* Call ourselve recursive (via _esdashboard_theme_css_resolve_at_identifier_by_string)
		...
		resolvedValue=_esdashboard_theme_css_resolve_at_identifier_by_string(self,
																				value,
																				inScopeScanner,
																				inScopeSelectors,
																				haveResolvedAtIdentifier);
																		...
																		const gchar *inRecursionIdentifier)
{
	...
	/* Resolve '@' identifier */
	value=_esdashboard_theme_css_resolve_at_identifier_internal(self,
																scanner,
																inScopeScanner,
																inScopeSelectors,
																inRecursionIdentifier);
...
static GTokenType _esdashboard_theme_css_parse_css_key_value(EsdashboardThemeCSS *self,
																GScanner *inScanner,
																GList *inScopeSelectors,
																...
																gchar **outValue)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_THEME_CSS(self), G_TOKEN_ERROR);
			...
			inScanner->next_value.v_char!=';')
	{
		...
		{
			case G_TOKEN_IDENTIFIER:
				*outValue=_esdashboard_theme_css_append_string(*outValue, inScanner->value.v_identifier);
				break;

			case G_TOKEN_CHAR:
				*outValue=_esdashboard_theme_css_append_char(*outValue, inScanner->value.v_char);
				break;
			...
			case G_TOKEN_STRING:
				 ...
				 */
				if(!inDoResolveAt) *outValue=_esdashboard_theme_css_append_char(*outValue, '\'');
				*outValue=_esdashboard_theme_css_append_string(*outValue, inScanner->value.v_string);
				if(!inDoResolveAt) *outValue=_esdashboard_theme_css_append_char(*outValue, '\'');
				break;
...
static GTokenType _esdashboard_theme_css_parse_css_styles(EsdashboardThemeCSS *self,
															GScanner *inScanner,
															GList *inScopeSelectors,
															gboolean inDoResolveAt,
															GHashTable *ioHashtable)
{
	...
	while(token!=G_TOKEN_RIGHT_CURLY)
	{
		...
		/* Parse key and value */
		token=_esdashboard_theme_css_parse_css_key_value(self,
															inScanner,
															inScopeSelectors,
															inDoResolveAt,
															&key,
...
static GTokenType _esdashboard_theme_css_command_import(EsdashboardThemeCSS *self,
														GScanner *inScanner,
														GList **ioSelectors)
{
	EsdashboardThemeCSSPrivate	*priv;
			...
			token==G_TOKEN_STRING)
	{
		...
		{
			case G_TOKEN_CHAR:
				filename=_esdashboard_theme_css_append_char(filename, inScanner->value.v_char);
				break;

			case G_TOKEN_STRING:
				filename=_esdashboard_theme_css_append_string(filename, inScanner->value.v_string);
				break;

			case G_TOKEN_IDENTIFIER:
				filename=_esdashboard_theme_css_append_string(filename, inScanner->value.v_identifier);
				break;
	...
	if(!esdashboard_theme_css_add_file(self, filename, GPOINTER_TO_INT(inScanner->user_data), &error))
	{
		gchar					*errorMessage;
...
static gboolean _esdashboard_theme_css_parse_css_ruleset_finish(EsdashboardCssSelector *inSelector,
																GScanner *inScanner,
																GTokenType inPeekNextToken,
																gpointer inUserData)
{
	g_return_val_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(inSelector), ESDASHBOARD_CSS_SELECTOR_PARSE_FINISH_BAD_STATE);
	g_return_val_if_fail(inScanner, ESDASHBOARD_CSS_SELECTOR_PARSE_FINISH_BAD_STATE);
	g_return_val_if_fail(ESDASHBOARD_IS_THEME_CSS(inUserData), ESDASHBOARD_CSS_SELECTOR_PARSE_FINISH_BAD_STATE);

	/* Scanner is in good state if current token in stream is a comma
	 * or a left curly bracket.
	 */
	if(inPeekNextToken=='{') return(ESDASHBOARD_CSS_SELECTOR_PARSE_FINISH_OK);
	...
	if(inPeekNextToken==',')
	{
		...
		return(ESDASHBOARD_CSS_SELECTOR_PARSE_FINISH_OK);
	}

	/* If we get here scanner is in bad state now */
	return(ESDASHBOARD_CSS_SELECTOR_PARSE_FINISH_BAD_STATE);
}

static GTokenType _esdashboard_theme_css_parse_css_ruleset(EsdashboardThemeCSS *self,
															GScanner *inScanner,
															GList **ioSelectors)
{
	EsdashboardThemeCSSPrivate		*priv;
	...
	while(token!=G_TOKEN_LEFT_CURLY)
	{
		...
		switch((guint)token)
		{
			...
			case ',':
				...
				selector=_esdashboard_theme_css_selector_new(inScanner->input_name);
				*ioSelectors=g_list_prepend(*ioSelectors, selector);

				/* Parse selector */
				selector->selector=esdashboard_css_selector_new_from_scanner_with_priority(inScanner,
																							GPOINTER_TO_INT(inScanner->user_data),
																							(EsdashboardCssSelectorParseFinishCallback)_esdashboard_theme_css_parse_css_ruleset_finish,
																							self);
				if(!selector->selector) return(G_TOKEN_ERROR);

				esdashboard_css_selector_adjust_to_offset(selector->selector, priv->offsetLine, 0);

				/* If we get here selector could be parse so set type */
				selector->type=ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_SELECTOR;
				break;
			...
			case '@':
				...
				if(g_strcmp0(inScanner->value.v_identifier, "constants")==0)
				{
					...
					selector=_esdashboard_theme_css_selector_new(inScanner->input_name);
					selector->type=ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_CONSTANT;
					...
					*ioSelectors=g_list_prepend(*ioSelectors, selector);
					...
					{
						token=_esdashboard_theme_css_command_import(self,
																	inScanner,
																	ioSelectors);
						if(token!=G_TOKEN_NONE) return(token);
					}
...
static GTokenType _esdashboard_theme_css_parse_css_block(EsdashboardThemeCSS *self,
															GScanner *inScanner,
															GList **ioSelectors,
															GList **ioStyles)
{
	...
	token=_esdashboard_theme_css_parse_css_ruleset(self, inScanner, &selectors);
	if(token!=G_TOKEN_NONE)
	{
		g_list_free_full(selectors, (GDestroyNotify)_esdashboard_theme_css_selector_free);
	...
	token=_esdashboard_theme_css_parse_css_styles(self,
													inScanner,
													*ioSelectors,
													doResolveAt,
													styles);
...
static gboolean _esdashboard_theme_css_parse_css(EsdashboardThemeCSS *self,
													...
													GError **outError)
{
	...
	while(token!=G_TOKEN_EOF)
	{
		token=_esdashboard_theme_css_parse_css_block(self, scanner, &selectors, &styles);
...
static gint _esdashboard_theme_css_sort_by_score(EsdashboardThemeCSSSelectorMatch *inLeft,
													EsdashboardThemeCSSSelectorMatch *inRight)
{
	gint							priority;
	guint							line;
	guint							position;
	gint							score;
	EsdashboardCssSelectorRule		*leftRule;
	EsdashboardCssSelectorRule		*rightRule;
	...
	leftRule=esdashboard_css_selector_get_rule(inLeft->selector->selector);
	rightRule=esdashboard_css_selector_get_rule(inRight->selector->selector);

	/* Compare CSS selector rules */
	priority=esdashboard_css_selector_rule_get_priority(leftRule);
	priority-=esdashboard_css_selector_rule_get_priority(rightRule);
	if(priority!=0) return(priority);

	line=esdashboard_css_selector_rule_get_line(leftRule);
	line-=esdashboard_css_selector_rule_get_line(rightRule);
	if(line!=0) return(line);

	position=esdashboard_css_selector_rule_get_position(leftRule);
	position-=esdashboard_css_selector_rule_get_position(rightRule);
...
/* Dispose this object */
static void _esdashboard_theme_css_dispose(GObject *inObject)
{
	EsdashboardThemeCSS				*self=ESDASHBOARD_THEME_CSS(inObject);
	EsdashboardThemeCSSPrivate		*priv=self->priv;
	...
	if(priv->selectors)
	{
		g_list_free_full(priv->selectors, (GDestroyNotify)_esdashboard_theme_css_selector_free);
		priv->selectors=NULL;
	}
	...
	G_OBJECT_CLASS(esdashboard_theme_css_parent_class)->dispose(inObject);
}

/* Set properties */
static void _esdashboard_theme_css_set_property(GObject *inObject,
												guint inPropID,
 ...
 * and signals
 */
void esdashboard_theme_css_class_init(EsdashboardThemeCSSClass *klass)
{
	GObjectClass		*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->dispose=_esdashboard_theme_css_dispose;
	gobjectClass->set_property=_esdashboard_theme_css_set_property;

	/* Define properties */
	EsdashboardThemeCSSProperties[PROP_THEME_PATH]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardThemeCSSProperties);
}

/* Object initialization
 * Create private structure and set up default values
 */
void esdashboard_theme_css_init(EsdashboardThemeCSS *self)
{
	EsdashboardThemeCSSPrivate		*priv;

	priv=self->priv=esdashboard_theme_css_get_instance_private(self);
...
#define REGISTER_CSS_FUNC(name, callback) \
	_esdashboard_theme_css_register_function(self, \
												name, \
												ESDASHBOARD_THEME_CSS_FUNCTION_CALLBACK(callback));

	REGISTER_CSS_FUNC("rgb", _esdashboard_theme_css_function_rgb_rgba);
	REGISTER_CSS_FUNC("rgba", _esdashboard_theme_css_function_rgb_rgba);
	REGISTER_CSS_FUNC("mix", _esdashboard_theme_css_function_mix);
	REGISTER_CSS_FUNC("lighter", _esdashboard_theme_css_function_lighter_darker);
	REGISTER_CSS_FUNC("darker", _esdashboard_theme_css_function_lighter_darker);
	REGISTER_CSS_FUNC("shade", _esdashboard_theme_css_function_shade);
	REGISTER_CSS_FUNC("alpha", _esdashboard_theme_css_function_alpha);
	REGISTER_CSS_FUNC("try_icons", _esdashboard_theme_css_function_try_icons)
...
{
	return(g_quark_from_static_string("esdashboard-theme-css-error-quark"));
}
...
{
	return(ESDASHBOARD_THEME_CSS(g_object_new(ESDASHBOARD_TYPE_THEME_CSS,
											...
											GError **outError)
{
	...
	_esdashboard_theme_css_parse_css(self,
													...
													EsdashboardStylable *inStylable)
{
	EsdashboardThemeCSSPrivate			*priv;
	GList								*entry, *matches;
	EsdashboardThemeCSSSelectorMatch	*match;
	GHashTable							*result;
#ifdef DEBUG
	GTimer								*timer=NULL;
	const gchar							*styleID;
	...
	g_return_val_if_fail(ESDASHBOARD_IS_THEME_CSS(self), NULL);
	g_return_val_if_fail(ESDASHBOARD_IS_STYLABLE(inStylable), NULL);
...
#ifdef DEBUG
	styleID=esdashboard_stylable_get_name(inStylable);
	styleClasses=esdashboard_stylable_get_classes(inStylable);
	stylePseudoClasses=esdashboard_stylable_get_pseudo_classes(inStylable);
	...
	for(entry=priv->selectors; entry; entry=g_list_next(entry))
	{
		...
		selector=(EsdashboardThemeCSSSelector*)entry->data;
		if(selector->type==ESDASHBOARD_THEME_CSS_SELECTOR_TYPE_SELECTOR)
		{
			score=esdashboard_css_selector_score(selector->selector, inStylable);
			if(score>=0)
			{
				match=g_slice_new(EsdashboardThemeCSSSelectorMatch);
	...
	matches=g_list_sort(matches,
						(GCompareFunc)_esdashboard_theme_css_sort_by_score);

	/* Get properties from matching selectors' styles */
	result=g_hash_table_new_full(g_str_hash,
									g_str_equal,
									NULL,
									(GDestroyNotify)_esdashboard_theme_css_value_free);
	for(entry=matches; entry; entry=g_list_next(entry))
	{
		EsdashboardThemeCSSTableCopyData	copyData;

		/* Get selector */
		match=(EsdashboardThemeCSSSelectorMatch*)entry->data;

		/* Copy selector properties to result set */
		copyData.name=esdashboard_css_selector_rule_get_source(esdashboard_css_selector_get_rule(match->selector->selector));
		copyData.table=result;

		g_hash_table_foreach(match->selector->style,
								(GHFunc)_esdashboard_theme_css_copy_table,
								&copyData);
	}

	g_list_free_full(matches, (GDestroyNotify)_esdashboard_themes_css_selector_match_free);
