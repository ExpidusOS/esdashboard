...
static void _esdashboard_window_tracker_backend_gdk_window_tracker_backend_iface_init(EsdashboardWindowTrackerBackendInterface *iface);

struct _EsdashboardWindowTrackerBackendGDKPrivate
{
	/* Instance related */
	EsdashboardWindowTrackerX11		*windowTracker;
};

G_DEFINE_TYPE_WITH_CODE(EsdashboardWindowTrackerBackendGDK,
						esdashboard_window_tracker_backend_gdk,
						G_TYPE_OBJECT,
						G_ADD_PRIVATE(EsdashboardWindowTrackerBackendGDK)
						G_IMPLEMENT_INTERFACE(ESDASHBOARD_TYPE_WINDOW_TRACKER_BACKEND, _esdashboard_window_tracker_backend_gdk_window_tracker_backend_iface_init))

/* IMPLEMENTATION: Private variables and methods */

/* State of stage window changed */
static void _esdashboard_window_tracker_backend_gdk_on_stage_state_changed(WnckWindow *inWindow,
...
static void _esdashboard_window_tracker_backend_gdk_on_stage_active_window_changed(WnckScreen *inScreen,
...
static void _esdashboard_window_tracker_backend_gdk_on_stage_screen_size_changed(EsdashboardWindowTracker *inWindowTracker,
																					gint inWidth,
																					gint inHeight,
																					gpointer inUserData)
{
	 ...
	 * point on.
	 */
	stageWindow=esdashboard_window_tracker_window_gdk_get_window(realStageWindow);
...
/* Get name of backend */
static const gchar* _esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_name(EsdashboardWindowTrackerBackend *inBackend)
{
	g_return_val_if_fail(ESDASHBOARD_IS_WINDOW_TRACKER_BACKEND_GDK(inBackend), NULL);
...
static EsdashboardWindowTracker* _esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_window_tracker(EsdashboardWindowTrackerBackend *inBackend)
{
	EsdashboardWindowTrackerBackendGDK				*self;
	EsdashboardWindowTrackerBackendGDKPrivate		*priv;

	g_return_val_if_fail(ESDASHBOARD_IS_WINDOW_TRACKER_BACKEND_GDK(inBackend), NULL);

	self=ESDASHBOARD_WINDOW_TRACKER_BACKEND_GDK(inBackend);
	priv=self->priv;
...
/* Get window of stage */
static EsdashboardWindowTrackerWindow* _esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_window_for_stage(EsdashboardWindowTrackerBackend *inBackend,
...
static ClutterStage* _esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_stage_from_window(EsdashboardWindowTrackerBackend *inBackend,
...
static void _esdashboard_window_tracker_backend_gdk_window_tracker_backend_show_stage_window(EsdashboardWindowTrackerBackend *inBackend,
																								EsdashboardWindowTrackerWindow *inStageWindow)
{
	EsdashboardWindowTrackerBackendGDK			*self;
	EsdashboardWindowTrackerBackendGDKPrivate	*priv;
	...
	g_return_if_fail(ESDASHBOARD_IS_WINDOW_TRACKER_BACKEND_GDK(inBackend));
	...
	stageGdkWindow=clutter_gdk_get_stage_window(esdashboard_window_tracker_window_get_stage(inStageWindow));
	if(!stageGdkWindow)
	{
	...
	handlerID=g_signal_handler_find(stageWnckWindow,
									...
									0,
									NULL,
									G_CALLBACK(_esdashboard_window_tracker_backend_gdk_on_stage_state_changed),
									stageWindow);
	if(!handlerID)
	{
	...
	handlerID=g_signal_handler_find(screen,
									...
									0,
									NULL,
									G_CALLBACK(_esdashboard_window_tracker_backend_gdk_on_stage_active_window_changed),
									stageWindow);
	if(!handlerID)
	{
	...
	handlerID=g_signal_handler_find(priv->windowTracker,
									...
									0,
									NULL,
									G_CALLBACK(_esdashboard_window_tracker_backend_gdk_on_stage_screen_size_changed),
									stageWindow);
	if(!handlerID)
	{
	...
	_esdashboard_window_tracker_backend_gdk_on_stage_screen_size_changed(ESDASHBOARD_WINDOW_TRACKER(priv->windowTracker),
																			width,
																			height,
																			inStageWindow);
...
static void _esdashboard_window_tracker_backend_gdk_window_tracker_backend_hide_stage_window(EsdashboardWindowTrackerBackend *inBackend,
																								EsdashboardWindowTrackerWindow *inStageWindow)
{
	EsdashboardWindowTrackerBackendGDK			*self;
	EsdashboardWindowTrackerBackendGDKPrivate	*priv;
...
static void _esdashboard_window_tracker_backend_gdk_window_tracker_backend_iface_init(EsdashboardWindowTrackerBackendInterface *iface)
{
	iface->get_name=_esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_name;

	iface->get_window_tracker=_esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_window_tracker;

	iface->get_window_for_stage=_esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_window_for_stage;
	iface->get_stage_from_window=_esdashboard_window_tracker_backend_gdk_window_tracker_backend_get_stage_from_window;
	iface->show_stage_window=_esdashboard_window_tracker_backend_gdk_window_tracker_backend_show_stage_window;
	iface->hide_stage_window=_esdashboard_window_tracker_backend_gdk_window_tracker_backend_hide_stage_window;
}
...
/* Dispose this object */
static void _esdashboard_window_tracker_backend_gdk_dispose(GObject *inObject)
{
	EsdashboardWindowTrackerBackendGDK				*self=ESDASHBOARD_WINDOW_TRACKER_BACKEND_GDK(inObject);
	EsdashboardWindowTrackerBackendGDKPrivate		*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_window_tracker_backend_gdk_parent_class)->dispose(inObject);
}
 ...
 * and signals
 */
void esdashboard_window_tracker_backend_gdk_class_init(EsdashboardWindowTrackerBackendGDKClass *klass)
{
	GObjectClass						*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->dispose=_esdashboard_window_tracker_backend_gdk_dispose;
}

/* Object initialization
 * Create private structure and set up default values
 */
void esdashboard_window_tracker_backend_gdk_init(EsdashboardWindowTrackerBackendGDK *self)
{
	EsdashboardWindowTrackerBackendGDKPrivate		*priv;

	priv=self->priv=esdashboard_window_tracker_backend_gdk_get_instance_private(self);

	ESDASHBOARD_DEBUG(self, WINDOWS, "Initializing GDK-X11 window tracker backend");
...
/**
 * esdashboard_window_tracker_backend_gdk_new:
 *
 * Creates a new #EsdashboardWindowTrackerBackendGDK backend for use with
...
EsdashboardWindowTrackerBackend* esdashboard_window_tracker_backend_gdk_new(void)
{
	...
	{
		ESDASHBOARD_DEBUG(NULL, WINDOWS, "GDK windowing system is X11");
		supported=TRUE;
	}
#endif
	...
	if(!supported)
	{
		ESDASHBOARD_DEBUG(NULL, WINDOWS, "Cannot create GDK window tracker backend as no supported backend found");
		return(NULL);
	}

	/* Create window tracker backend and return it */
	return(g_object_new(ESDASHBOARD_TYPE_WINDOW_TRACKER_BACKEND_GDK, NULL));
}
