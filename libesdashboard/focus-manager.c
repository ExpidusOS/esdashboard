...
struct _EsdashboardFocusManagerPrivate
{
	/* Instance related */
	GList					*registeredFocusables;
	EsdashboardFocusable	*currentFocus;
};

G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardFocusManager,
							esdashboard_focus_manager,
							G_TYPE_OBJECT)
...
static guint EsdashboardFocusManagerSignals[SIGNAL_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Single instance of focus manager */
static EsdashboardFocusManager*		_esdashboard_focus_manager=NULL;

/* A registered focusable actor is going to be destroyed so unregister it */
static void _esdashboard_focus_manager_on_focusable_destroy(EsdashboardFocusManager *self,
															gpointer inUserData)
{
	EsdashboardFocusable			*focusable;

	g_return_if_fail(ESDASHBOARD_IS_FOCUS_MANAGER(self));
	g_return_if_fail(ESDASHBOARD_IS_FOCUSABLE(inUserData));

	focusable=ESDASHBOARD_FOCUSABLE(inUserData);

	/* Unregister going-to-be-destroyed focusable actor */
	esdashboard_focus_manager_unregister(self, focusable);
}

/* A registered focusable actor is going to be hidden or unrealized */
static void _esdashboard_focus_manager_on_focusable_hide(EsdashboardFocusManager *self,
															gpointer inUserData)
{
	EsdashboardFocusManagerPrivate	*priv;
	EsdashboardFocusable			*focusable;
	EsdashboardFocusable			*nextFocusable;
	 ...
	 */
	nextFocusable=esdashboard_focus_manager_get_next_focusable(self, priv->currentFocus);
	if(nextFocusable && nextFocusable!=priv->currentFocus) esdashboard_focus_manager_set_focus(self, nextFocusable);
		else
		{
			esdashboard_focusable_unset_focus(priv->currentFocus);
 ...
 */
static GSList* _esdashboard_focus_manager_get_targets_for_binding(EsdashboardFocusManager *self,
																	const EsdashboardBinding *inBinding)
{
	GSList							*targets;
	...
	g_return_val_if_fail(ESDASHBOARD_IS_FOCUS_MANAGER(self), NULL);
	g_return_val_if_fail(ESDASHBOARD_IS_BINDING(inBinding), NULL);

	targets=NULL;
	mustBeFocusable=TRUE;

	/* Get list of possible targets */
	targets=esdashboard_focus_manager_get_targets(self, esdashboard_binding_get_target(inBinding));

	/* Determine if unfocusable targets should be included */
	if(esdashboard_binding_get_flags(inBinding) & ESDASHBOARD_BINDING_FLAGS_ALLOW_UNFOCUSABLE_TARGET)
	{
	...
	if(mustBeFocusable)
	{
		...
		{
			/* Get focusable actor */
			if(!ESDASHBOARD_IS_FOCUSABLE(iter->data)) continue;
			focusable=ESDASHBOARD_FOCUSABLE(iter->data);

			/* Check if focusable actor can be focused as it may be disabled */
			if(!esdashboard_focusable_can_focus(focusable))
			{
	...
	ESDASHBOARD_DEBUG(self, MISC,
						...
						esdashboard_binding_get_action(inBinding),
						esdashboard_binding_get_target(inBinding),
						g_slist_length(targets));
	return(targets);
}

/* Action signal to move focus to first focusable actor was emitted */
static gboolean _esdashboard_focus_manager_move_focus_first(EsdashboardFocusManager *self,
															EsdashboardFocusable *inSource,
															const gchar *inAction,
															ClutterEvent *inEvent)
{
	EsdashboardFocusManagerPrivate	*priv;
	EsdashboardFocusable			*currentFocusable;
	EsdashboardFocusable			*newFocusable;
	GList							*iter;

	g_return_val_if_fail(ESDASHBOARD_IS_FOCUS_MANAGER(self), CLUTTER_EVENT_PROPAGATE);
	g_return_val_if_fail(inEvent, CLUTTER_EVENT_PROPAGATE);

	priv=self->priv;

	/* Get current focus */
	currentFocusable=esdashboard_focus_manager_get_focus(self);

	/* Iterate through registered focusable actor and find the first focusable one.
	 * We do not use esdashboard_focus_manager_get_next_focusable(self, NULL) as
	...
	{
		newFocusable=(EsdashboardFocusable*)iter->data;
		...
		if(esdashboard_focusable_can_focus(newFocusable))
		{
			esdashboard_focus_manager_set_focus(self, newFocusable);
...
static gboolean _esdashboard_focus_manager_move_focus_last(EsdashboardFocusManager *self,
															EsdashboardFocusable *inSource,
															const gchar *inAction,
															ClutterEvent *inEvent)
{
	 ...
	 * one. We do not use esdashboard_focus_manager_get_previous_focusable(self, NULL)
...
static gboolean _esdashboard_focus_manager_move_focus_next(EsdashboardFocusManager *self,
															EsdashboardFocusable *inSource,
															const gchar *inAction,
															ClutterEvent *inEvent)
{
	...
	newFocusable=esdashboard_focus_manager_get_next_focusable(self, currentFocusable);
	if(newFocusable) esdashboard_focus_manager_set_focus(self, newFocusable);

	return(CLUTTER_EVENT_STOP);
}

/* Action signal to move focus to previous focusable actor was emitted */
static gboolean _esdashboard_focus_manager_move_focus_previous(EsdashboardFocusManager *self,
																EsdashboardFocusable *inSource,
																const gchar *inAction,
																ClutterEvent *inEvent)
{
	...
	/* Get next focusable actor to focus */
	newFocusable=esdashboard_focus_manager_get_previous_focusable(self, currentFocusable);
	if(newFocusable) esdashboard_focus_manager_set_focus(self, newFocusable);
...
static GObject* _esdashboard_focus_manager_constructor(GType inType,
														guint inNumberConstructParams,
														GObjectConstructParam *inConstructParams)
{
	GObject									*object;

	if(!_esdashboard_focus_manager)
	{
		object=G_OBJECT_CLASS(esdashboard_focus_manager_parent_class)->constructor(inType, inNumberConstructParams, inConstructParams);
		_esdashboard_focus_manager=ESDASHBOARD_FOCUS_MANAGER(object);
	}
		else
		{
			object=g_object_ref(G_OBJECT(_esdashboard_focus_manager));
		}
...
/* Dispose this object */
static void _esdashboard_focus_manager_dispose_unregister_focusable(gpointer inData, gpointer inUserData)
{
	EsdashboardFocusManager			*self;
	EsdashboardFocusable			*focusable;

	g_return_if_fail(ESDASHBOARD_IS_FOCUS_MANAGER(inUserData));
	g_return_if_fail(ESDASHBOARD_IS_FOCUSABLE(inData));

	self=ESDASHBOARD_FOCUS_MANAGER(inUserData);
	focusable=ESDASHBOARD_FOCUSABLE(inData);
	...
	g_signal_handlers_disconnect_by_func(focusable,
											G_CALLBACK(_esdashboard_focus_manager_on_focusable_destroy),
											self);
	g_signal_handlers_disconnect_by_func(focusable,
											G_CALLBACK(_esdashboard_focus_manager_on_focusable_hide),
											self);

	g_signal_emit(self, EsdashboardFocusManagerSignals[SIGNAL_UNREGISTERED], 0, focusable);
}

static void _esdashboard_focus_manager_dispose(GObject *inObject)
{
	EsdashboardFocusManager			*self=ESDASHBOARD_FOCUS_MANAGER(inObject);
	EsdashboardFocusManagerPrivate	*priv=self->priv;

	/* Release allocated resouces */
	if(priv->registeredFocusables)
	{
		g_list_foreach(priv->registeredFocusables, _esdashboard_focus_manager_dispose_unregister_focusable, self);
		g_list_free(priv->registeredFocusables);
		priv->registeredFocusables=NULL;
	}

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_focus_manager_parent_class)->dispose(inObject);
}

/* Finalize this object */
static void _esdashboard_focus_manager_finalize(GObject *inObject)
{
	/* Release allocated resources finally, e.g. unset singleton */
	if(G_LIKELY(G_OBJECT(_esdashboard_focus_manager)==inObject))
	{
		_esdashboard_focus_manager=NULL;
	}

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_focus_manager_parent_class)->finalize(inObject);
}
 ...
 * and signals
 */
static void esdashboard_focus_manager_class_init(EsdashboardFocusManagerClass *klass)
{
	GObjectClass		*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->constructor=_esdashboard_focus_manager_constructor;
	gobjectClass->dispose=_esdashboard_focus_manager_dispose;
	gobjectClass->finalize=_esdashboard_focus_manager_finalize;

	klass->focus_move_first=_esdashboard_focus_manager_move_focus_first;
	klass->focus_move_last=_esdashboard_focus_manager_move_focus_last;
	klass->focus_move_next=_esdashboard_focus_manager_move_focus_next;
	klass->focus_move_previous=_esdashboard_focus_manager_move_focus_previous;

	/* Define signals */
	EsdashboardFocusManagerSignals[SIGNAL_REGISTERED]=
		g_signal_new("registered",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, registered),
						NULL,
						NULL,
	...
	EsdashboardFocusManagerSignals[SIGNAL_UNREGISTERED]=
		g_signal_new("unregistered",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, unregistered),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
	...
	EsdashboardFocusManagerSignals[SIGNAL_CHANGED]=
		g_signal_new("changed",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, changed),
						NULL,
						NULL,
	...
	EsdashboardFocusManagerSignals[ACTION_FOCUS_MOVE_FIRST]=
		g_signal_new("focus-move-first",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, focus_move_first),
	...
	EsdashboardFocusManagerSignals[ACTION_FOCUS_MOVE_LAST]=
		g_signal_new("focus-move-last",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, focus_move_last),
						g_signal_accumulator_true_handled,
						NULL,
						_esdashboard_marshal_BOOLEAN__OBJECT_STRING_BOXED,
						G_TYPE_BOOLEAN,
	...
	EsdashboardFocusManagerSignals[ACTION_FOCUS_MOVE_NEXT]=
		g_signal_new("focus-move-next",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, focus_move_next),
						g_signal_accumulator_true_handled,
						NULL,
						_esdashboard_marshal_BOOLEAN__OBJECT_STRING_BOXED,
						G_TYPE_BOOLEAN,
	...
	EsdashboardFocusManagerSignals[ACTION_FOCUS_MOVE_PREVIOUS]=
		g_signal_new("focus-move-previous",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
						G_STRUCT_OFFSET(EsdashboardFocusManagerClass, focus_move_previous),
						g_signal_accumulator_true_handled,
						NULL,
						_esdashboard_marshal_BOOLEAN__OBJECT_STRING_BOXED,
						G_TYPE_BOOLEAN,
 ...
 */
static void esdashboard_focus_manager_init(EsdashboardFocusManager *self)
{
	EsdashboardFocusManagerPrivate	*priv;

	priv=self->priv=esdashboard_focus_manager_get_instance_private(self);
...
EsdashboardFocusManager* esdashboard_focus_manager_get_default(void)
{
	...
	singleton=g_object_new(ESDASHBOARD_TYPE_FOCUS_MANAGER, NULL);
	return(ESDASHBOARD_FOCUS_MANAGER(singleton));
}
...
void esdashboard_focus_manager_register(EsdashboardFocusManager *self, EsdashboardFocusable *inFocusable)
{
	...
	esdashboard_focus_manager_register_after(self, inFocusable, NULL);
}
...
void esdashboard_focus_manager_register_after(EsdashboardFocusManager *self, EsdashboardFocusable *inFocusable, EsdashboardFocusable *inAfterFocusable)
{
	...
	g_return_if_fail(!inAfterFocusable || ESDASHBOARD_IS_FOCUSABLE(inAfterFocusable));

	priv=self->priv;

	/* Check if given focusable actor is really focusable and stylable */
	if(!ESDASHBOARD_IS_FOCUSABLE(inFocusable))
	{
		g_warning("Object %s does not inherit %s and cannot be registered",
					G_OBJECT_TYPE_NAME(inFocusable),
					g_type_name(ESDASHBOARD_TYPE_FOCUSABLE));
		return;
	}

	if(!ESDASHBOARD_IS_STYLABLE(inFocusable))
	{
		g_warning("Object %s does not inherit %s and cannot be registered",
					G_OBJECT_TYPE_NAME(inFocusable),
					g_type_name(ESDASHBOARD_TYPE_STYLABLE));
		return;
	}
	...
	if(g_list_find(priv->registeredFocusables, inFocusable)==NULL)
	{
		...
		/* Emit signal */
		g_signal_emit(self, EsdashboardFocusManagerSignals[SIGNAL_REGISTERED], 0, inFocusable);
	}
}
...
void esdashboard_focus_manager_unregister(EsdashboardFocusManager *self, EsdashboardFocusable *inFocusable)
{
	...
	if(g_list_find(priv->registeredFocusables, inFocusable)!=NULL)
	{
		...
		if(inFocusable==priv->currentFocus)
		{
			...
			focusable=esdashboard_focus_manager_get_next_focusable(self, priv->currentFocus);
			if(focusable && focusable!=priv->currentFocus) esdashboard_focus_manager_set_focus(self, focusable);
				else
				{
					esdashboard_focusable_unset_focus(priv->currentFocus);
					priv->currentFocus=NULL;
		...
		/* Emit signal */
		g_signal_emit(self, EsdashboardFocusManagerSignals[SIGNAL_UNREGISTERED], 0, inFocusable);
	}
}
...
gboolean esdashboard_focus_manager_is_registered(EsdashboardFocusManager *self, EsdashboardFocusable *inFocusable)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_FOCUS_MANAGER(self), FALSE);
...
GSList* esdashboard_focus_manager_get_targets(EsdashboardFocusManager *self, const gchar *inTarget)
{
	...
	if(g_type_is_a(ESDASHBOARD_TYPE_APPLICATION, targetType))
	{
		targets=g_slist_append(targets, g_object_ref(esdashboard_application_get_default()));
	}
	...
	{
		focusable=(EsdashboardFocusable*)focusablesIter->data;
...
{
	EsdashboardFocusManagerPrivate	*priv;
	EsdashboardFocusable			*oldFocusable;
	 ...
	 */
	if(!esdashboard_focusable_can_focus(inFocusable))
	{
		EsdashboardFocusable		*newFocusable;

		newFocusable=esdashboard_focus_manager_get_next_focusable(self, inFocusable);
		if(!newFocusable)
		{
			ESDASHBOARD_DEBUG(self, MISC,
	...
	{
		ESDASHBOARD_DEBUG(self, MISC, "Current focused actor and new one are the same so do nothing.");
		return;
	}
	...
	esdashboard_focusable_set_focus(priv->currentFocus);
	ESDASHBOARD_DEBUG(self, MISC,
	...
	g_signal_emit(self, EsdashboardFocusManagerSignals[SIGNAL_CHANGED], 0, oldFocusable, priv->currentFocus);
}
																	...
																	EsdashboardFocusable *inBeginFocusable)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_FOCUS_MANAGER(self), NULL);
	g_return_val_if_fail(!inBeginFocusable || ESDASHBOARD_IS_FOCUSABLE(inBeginFocusable), NULL);
	...
	{
		focusable=(EsdashboardFocusable*)iter->data;

		/* If focusable can be focused then return it */
		if(esdashboard_focusable_can_focus(focusable)) return(focusable);
	}
																...
																const gchar **outAction)
{
	...
	const EsdashboardBinding		*binding;
	const gchar						*action;
	...
	g_return_val_if_fail(!inFocusable || ESDASHBOARD_IS_FOCUSABLE(inFocusable), FALSE);
	 ...
	 * for this event.
	 */
	bindings=esdashboard_bindings_pool_get_default();
	binding=esdashboard_bindings_pool_find_for_event(bindings, CLUTTER_ACTOR(inFocusable), inEvent);
	if(binding)
	{
		const gchar					*target;

		/* Get action of binding */
		action=esdashboard_binding_get_action(binding);
		 ...
		 */
		targetFocusables=NULL;
		target=esdashboard_binding_get_target(binding);
		if(target)
		{
			/* Target class name is specified so build up a list of targets */
			targetFocusables=_esdashboard_focus_manager_get_targets_for_binding(self, binding);
		}
			else
			{
													...
													EsdashboardFocusable *inFocusable)
{
	...
	g_return_val_if_fail(!inFocusable || ESDASHBOARD_IS_FOCUSABLE(inFocusable), CLUTTER_EVENT_PROPAGATE);
	...
	targetFocusables=NULL;
	action=NULL;
	if(esdashboard_focus_manager_get_event_targets_and_action(self, inEvent, inFocusable, &targetFocusables, &action))
	{
