...
struct _EsdashboardBackgroundPrivate
{
	/* Properties related */
	EsdashboardBackgroundType	type;

	ClutterColor				*fillColor;
	EsdashboardCorners			fillCorners;
	gfloat						fillCornersRadius;

	ClutterColor				*outlineColor;
	gfloat						outlineWidth;
	EsdashboardBorders			outlineBorders;
	EsdashboardCorners			outlineCorners;
	gfloat						outlineCornersRadius;

	/* Instance related */
	ClutterContent				*fillCanvas;
	EsdashboardOutlineEffect	*outline;
	ClutterImage				*image;
};

G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardBackground,
							esdashboard_background,
...
static GParamSpec* EsdashboardBackgroundProperties[PROP_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Rectangle canvas should be redrawn */
static gboolean _esdashboard_background_on_draw_fill_canvas(EsdashboardBackground *self,
																cairo_t *inContext,
																int inWidth,
																int inHeight,
																gpointer inUserData)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_BACKGROUND(self), TRUE);
	...
	if(!(priv->type & ESDASHBOARD_BACKGROUND_TYPE_FILL)) return(CLUTTER_EVENT_PROPAGATE);

	/* Determine if we should draw rounded corners */

	/* Draw rectangle with or without rounded corners */
	if((priv->type & ESDASHBOARD_BACKGROUND_TYPE_ROUNDED_CORNERS) &&
		(priv->fillCorners & ESDASHBOARD_CORNERS_ALL) &&
		priv->fillCornersRadius>0.0f)
	{
		...
		/* Top-left */
		if(priv->fillCorners & ESDASHBOARD_CORNERS_TOP_LEFT)
		{
		...
		/* Top-right */
		if(priv->fillCorners & ESDASHBOARD_CORNERS_TOP_RIGHT)
		{
		...
		/* Bottom-right */
		if(priv->fillCorners & ESDASHBOARD_CORNERS_BOTTOM_RIGHT)
		{
		...
		/* Bottom-left */
		if(priv->fillCorners & ESDASHBOARD_CORNERS_BOTTOM_LEFT)
		{
		...
		/* Close to top-left */
		if(priv->fillCorners & ESDASHBOARD_CORNERS_TOP_LEFT) cairo_line_to(inContext, 0, radius);
...
/* Paint actor */
static void _esdashboard_background_paint_node(ClutterActor *self,
												ClutterPaintNode *inRootNode)
{
	EsdashboardBackgroundPrivate	*priv=ESDASHBOARD_BACKGROUND(self)->priv;
	ClutterContentIface				*iface;

	/* First draw canvas for background */
	if(priv->type & ESDASHBOARD_BACKGROUND_TYPE_FILL)
	{
	...
	if(CLUTTER_ACTOR_CLASS(esdashboard_background_parent_class)->paint_node)
	{
		CLUTTER_ACTOR_CLASS(esdashboard_background_parent_class)->paint_node(self, inRootNode);
	}
}

/* Allocate position and size of actor and its children */
static void _esdashboard_background_allocate(ClutterActor *self,
												...
												ClutterAllocationFlags inFlags)
{
	...
	CLUTTER_ACTOR_CLASS(esdashboard_background_parent_class)->allocate(self, inBox, inFlags);
...
/* Dispose this object */
static void _esdashboard_background_dispose(GObject *inObject)
{
	/* Release allocated variables */
	EsdashboardBackgroundPrivate	*priv=ESDASHBOARD_BACKGROUND(inObject)->priv;
	...
	G_OBJECT_CLASS(esdashboard_background_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_background_set_property(GObject *inObject,
													guint inPropID,
													const GValue *inValue,
													GParamSpec *inSpec)
{
	EsdashboardBackground			*self=ESDASHBOARD_BACKGROUND(inObject);

	switch(inPropID)
	{
		case PROP_TYPE:
			esdashboard_background_set_background_type(self, g_value_get_flags(inValue));
			break;

		case PROP_CORNERS:
			esdashboard_background_set_corners(self, g_value_get_flags(inValue));
			break;

		case PROP_CORNERS_RADIUS:
			esdashboard_background_set_corner_radius(self, g_value_get_float(inValue));
			break;

		case PROP_FILL_COLOR:
			esdashboard_background_set_fill_color(self, clutter_value_get_color(inValue));
			break;

		case PROP_FILL_CORNERS:
			esdashboard_background_set_fill_corners(self, g_value_get_flags(inValue));
			break;

		case PROP_FILL_CORNERS_RADIUS:
			esdashboard_background_set_fill_corner_radius(self, g_value_get_float(inValue));
			break;

		case PROP_OUTLINE_COLOR:
			esdashboard_background_set_outline_color(self, clutter_value_get_color(inValue));
			break;

		case PROP_OUTLINE_WIDTH:
			esdashboard_background_set_outline_width(self, g_value_get_float(inValue));
			break;

		case PROP_OUTLINE_BORDERS:
			esdashboard_background_set_outline_borders(self, g_value_get_flags(inValue));
			break;

		case PROP_OUTLINE_CORNERS:
			esdashboard_background_set_outline_corners(self, g_value_get_flags(inValue));
			break;

		case PROP_OUTLINE_CORNERS_RADIUS:
			esdashboard_background_set_outline_corner_radius(self, g_value_get_float(inValue));
			break;

		case PROP_IMAGE:
			esdashboard_background_set_image(self, CLUTTER_IMAGE(g_value_get_object(inValue)));
			break;
...
static void _esdashboard_background_get_property(GObject *inObject,
													guint inPropID,
													GValue *outValue,
													GParamSpec *inSpec)
{
	EsdashboardBackground			*self=ESDASHBOARD_BACKGROUND(inObject);
	EsdashboardBackgroundPrivate	*priv=self->priv;
 ...
 * and signals
 */
static void esdashboard_background_class_init(EsdashboardBackgroundClass *klass)
{
	...
	/* Override functions */
	gobjectClass->dispose=_esdashboard_background_dispose;
	gobjectClass->set_property=_esdashboard_background_set_property;
	gobjectClass->get_property=_esdashboard_background_get_property;

	clutterActorClass->paint_node=_esdashboard_background_paint_node;
	clutterActorClass->allocate=_esdashboard_background_allocate;

	/* Define properties */
	EsdashboardBackgroundProperties[PROP_TYPE]=
		g_param_spec_flags("background-type",
							"Background type",
							"Background type",
							ESDASHBOARD_TYPE_BACKGROUND_TYPE,
							ESDASHBOARD_BACKGROUND_TYPE_NONE,
							G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

	EsdashboardBackgroundProperties[PROP_CORNERS]=
	...
	EsdashboardBackgroundProperties[PROP_CORNERS_RADIUS]=
	...
	EsdashboardBackgroundProperties[PROP_FILL_COLOR]=
	...
	EsdashboardBackgroundProperties[PROP_FILL_CORNERS]=
	...
	EsdashboardBackgroundProperties[PROP_FILL_CORNERS_RADIUS]=
	...
	EsdashboardBackgroundProperties[PROP_OUTLINE_COLOR]=
	...
	EsdashboardBackgroundProperties[PROP_OUTLINE_WIDTH]=
	...
	EsdashboardBackgroundProperties[PROP_OUTLINE_BORDERS]=
	...
	EsdashboardBackgroundProperties[PROP_OUTLINE_CORNERS]=
	...
	EsdashboardBackgroundProperties[PROP_OUTLINE_CORNERS_RADIUS]=
	...
	EsdashboardBackgroundProperties[PROP_IMAGE]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardBackgroundProperties);

	/* Define stylable properties */
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_TYPE]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_CORNERS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_CORNERS_RADIUS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_FILL_COLOR]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_FILL_CORNERS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_FILL_CORNERS_RADIUS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_OUTLINE_COLOR]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_OUTLINE_WIDTH]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_OUTLINE_BORDERS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_OUTLINE_CORNERS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_OUTLINE_CORNERS_RADIUS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardBackgroundProperties[PROP_IMAGE]);
}

/* Object initialization
 * Create private structure and set up default values
 */
static void esdashboard_background_init(EsdashboardBackground *self)
{
	EsdashboardBackgroundPrivate	*priv;

	priv=self->priv=esdashboard_background_get_instance_private(self);

	/* This actor reacts on events */
	clutter_actor_set_reactive(CLUTTER_ACTOR(self), TRUE);

	/* Set up default values */
	priv->type=ESDASHBOARD_BACKGROUND_TYPE_NONE;
	...
	priv->outline=ESDASHBOARD_OUTLINE_EFFECT(g_object_ref(esdashboard_outline_effect_new()));
	priv->outlineColor=clutter_color_copy(CLUTTER_COLOR_White);
	priv->outlineWidth=1.0f;
	priv->outlineCorners=ESDASHBOARD_CORNERS_ALL;
	priv->outlineBorders=ESDASHBOARD_BORDERS_ALL;
	...
	/* Connect signals */
	g_signal_connect_swapped(priv->fillCanvas, "draw", G_CALLBACK(_esdashboard_background_on_draw_fill_canvas), self);
}
...
{
	return(g_object_new(ESDASHBOARD_TYPE_BACKGROUND,
						"background-type", ESDASHBOARD_BACKGROUND_TYPE_NONE,
						NULL));
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_BACKGROUND(self), ESDASHBOARD_BACKGROUND_TYPE_NONE);
...
void esdashboard_background_set_background_type(EsdashboardBackground *self, const EsdashboardBackgroundType inType)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_BACKGROUND(self));
	...
	if(priv->type!=inType)
	{
		...
		if(priv->outline)
		{
			if(inType & ESDASHBOARD_BACKGROUND_TYPE_OUTLINE) enableOutline=TRUE;
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_TYPE]);
	}
}
...
void esdashboard_background_set_corners(EsdashboardBackground *self, EsdashboardCorners inCorners)
{
	...
	/* Set radius */
	esdashboard_background_set_fill_corners(self, inCorners);
	esdashboard_background_set_outline_corners(self, inCorners);
}
...
void esdashboard_background_set_corner_radius(EsdashboardBackground *self, const gfloat inRadius)
{
	...
	/* Set radius */
	esdashboard_background_set_fill_corner_radius(self, inRadius);
	esdashboard_background_set_outline_corner_radius(self, inRadius);
}

/* Get/set color to fill background with */
const ClutterColor* esdashboard_background_get_fill_color(EsdashboardBackground *self)
{
	g_return_val_if_fail(ESDASHBOARD_IS_BACKGROUND(self), NULL);
...
void esdashboard_background_set_fill_color(EsdashboardBackground *self, const ClutterColor *inColor)
{
	...
	if(priv->fillColor==NULL || clutter_color_equal(inColor, priv->fillColor)==FALSE)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_FILL_COLOR]);
	}
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_BACKGROUND(self), ESDASHBOARD_CORNERS_NONE);
...
void esdashboard_background_set_fill_corners(EsdashboardBackground *self, EsdashboardCorners inCorners)
{
	...
	if(priv->fillCorners!=inCorners)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_FILL_CORNERS]);
	}
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_BACKGROUND(self), 0.0f);
...
void esdashboard_background_set_fill_corner_radius(EsdashboardBackground *self, const gfloat inRadius)
{
	...
	if(priv->fillCornersRadius!=inRadius)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_FILL_CORNERS_RADIUS]);
	}
}
...
void esdashboard_background_set_outline_color(EsdashboardBackground *self, const ClutterColor *inColor)
{
	...
	if(priv->outlineColor==NULL || clutter_color_equal(inColor, priv->outlineColor)==FALSE)
	{
		...
		/* Update effect */
		if(priv->outline) esdashboard_outline_effect_set_color(priv->outline, inColor);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_OUTLINE_COLOR]);
	}
}
...
void esdashboard_background_set_outline_width(EsdashboardBackground *self, const gfloat inWidth)
{
	...
	if(priv->outlineWidth!=inWidth)
	{
		...
		/* Update effect */
		if(priv->outline) esdashboard_outline_effect_set_width(priv->outline, inWidth);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_OUTLINE_WIDTH]);
	}
}
...
void esdashboard_background_set_outline_borders(EsdashboardBackground *self, EsdashboardBorders inBorders)
{
	...
	if(priv->outlineBorders!=inBorders)
	{
		...
		/* Update effect */
		if(priv->outline) esdashboard_outline_effect_set_borders(priv->outline, inBorders);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_OUTLINE_BORDERS]);
	}
}
...
void esdashboard_background_set_outline_corners(EsdashboardBackground *self, EsdashboardCorners inCorners)
{
	...
	if(priv->outlineCorners!=inCorners)
	{
		...
		/* Update effect */
		if(priv->outline) esdashboard_outline_effect_set_corners(priv->outline, inCorners);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_OUTLINE_CORNERS]);
	}
}
...
void esdashboard_background_set_outline_corner_radius(EsdashboardBackground *self, const gfloat inRadius)
{
	...
	if(priv->outlineCornersRadius!=inRadius)
	{
		...
		/* Update effect */
		if(priv->outline) esdashboard_outline_effect_set_corner_radius(priv->outline, inRadius);

		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_OUTLINE_CORNERS_RADIUS]);
	}
}
...
void esdashboard_background_set_image(EsdashboardBackground *self, ClutterImage *inImage)
{
	...
	if(priv->image!=inImage)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardBackgroundProperties[PROP_IMAGE]);
	}
}
