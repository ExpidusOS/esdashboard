 ...
 * @include: esdashboard/transition-group.h
 *
 * The #EsdashboardTransitionGroup allows running multiple #ClutterTransition
 * instances concurrently.
 * 
 * The #EsdashboardTransitionGroup is a one-to-one copy of #ClutterTransitionGroup
 ...
 * #EsdashboardTransitionGroup will also applied to all transitions added to this
 * group.
 *
 * In addition it will also reset "min-width-set", "min-height-set" and other
 * "*-set" properties to their values before the #EsdashboardTransitionGroup
...
typedef struct _EsdashboardTransitionActorSetFlags			EsdashboardTransitionActorSetFlags;
struct _EsdashboardTransitionActorSetFlags
{
...
struct _EsdashboardTransitionGroupPrivate
{
	/* Properties related */
	gboolean							resetFlags;
	...
	EsdashboardTransitionActorSetFlags	flags;
};

G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardTransitionGroup,
							esdashboard_transition_group,
...
static GParamSpec* EsdashboardTransitionGroupProperties[PROP_LAST]={ 0, };


/* IMPLEMENTATION: ClutterTimeline */

/* Time at transition has elapsed, so advance all transitions as well */
static void _esdashboard_transition_group_timeline_new_frame(ClutterTimeline *inTimeline,
																gint inElapsed)
{
	EsdashboardTransitionGroup			*self;
	...
	g_return_if_fail(ESDASHBOARD_IS_TRANSITION_GROUP(inTimeline));

	self=ESDASHBOARD_TRANSITION_GROUP(inTimeline);
	priv=self->priv;
...
static void _esdashboard_transition_group_timeline_started(ClutterTimeline *inTimeline)
{
	EsdashboardTransitionGroup			*self;
	EsdashboardTransitionGroupPrivate	*priv;
	GHashTableIter						iter;
...
static void _esdashboard_transition_group_transition_attached(ClutterTransition *inTransition,
																ClutterAnimatable *inAnimatable)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_TRANSITION_GROUP(inTransition));
	g_return_if_fail(CLUTTER_IS_ANIMATABLE(inAnimatable));

	self=ESDASHBOARD_TRANSITION_GROUP(inTransition);
	priv=self->priv;
...
static void _esdashboard_transition_group_transition_detached(ClutterTransition *inTransition,
																ClutterAnimatable *inAnimatable)
{
	EsdashboardTransitionGroup				*self;
	EsdashboardTransitionGroupPrivate		*priv;
	...
	if(priv->resetFlags)
	{
		EsdashboardTransitionActorSetFlags	newFlags;
...
/* Dispose this object */
static void _esdashboard_transition_group_dispose(GObject *inObject)
{
	EsdashboardTransitionGroup			*self=ESDASHBOARD_TRANSITION_GROUP(inObject);
	ClutterAnimatable					*animatable;

	/* Restore "*-set" properties */
	animatable=clutter_transition_get_animatable(CLUTTER_TRANSITION(self));
	if(animatable) _esdashboard_transition_group_transition_detached(CLUTTER_TRANSITION(self), animatable);

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_transition_group_parent_class)->dispose(inObject);
}

/* Finalize this object */
static void _esdashboard_transition_group_finalize(GObject *inObject)
{
	EsdashboardTransitionGroup			*self=ESDASHBOARD_TRANSITION_GROUP(inObject);
	EsdashboardTransitionGroupPrivate	*priv=self->priv;
	...
	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_transition_group_parent_class)->finalize(inObject);
}

/* Set/get properties */
static void _esdashboard_transition_group_set_property(GObject *inObject,
														guint inPropID,
...
static void _esdashboard_transition_group_get_property(GObject *inObject,
														guint inPropID,
														GValue *outValue,
														GParamSpec *inSpec)
{
 ...
 * and signals
 */
void esdashboard_transition_group_class_init(EsdashboardTransitionGroupClass *klass)
{
	...
	/* Override functions */
	gobjectClass->set_property=_esdashboard_transition_group_set_property;
	gobjectClass->get_property=_esdashboard_transition_group_get_property;
	gobjectClass->dispose=_esdashboard_transition_group_dispose;
	gobjectClass->finalize=_esdashboard_transition_group_finalize;

	timelineClass->started=_esdashboard_transition_group_timeline_started;
	timelineClass->new_frame=_esdashboard_transition_group_timeline_new_frame;

	transitionClass->attached=_esdashboard_transition_group_transition_attached;
	transitionClass->detached=_esdashboard_transition_group_transition_detached;

	/* Define properties */
	EsdashboardTransitionGroupProperties[PROP_RESET_FLAGS]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardTransitionGroupProperties);
}

/* Object initialization
 * Create private structure and set up default values
 */
void esdashboard_transition_group_init(EsdashboardTransitionGroup *self)
{
	EsdashboardTransitionGroupPrivate		*priv;

	priv=self->priv=esdashboard_transition_group_get_instance_private(self);
...
/**
 * esdashboard_transition_group_new:
 *
 * Creates a new #EsdashboardTransitionGroup instance.
 *
 * Return value: The newly created #EsdashboardTransitionGroup. Use
 *   g_object_unref() when done to deallocate the resources it uses
 */
ClutterTransition* esdashboard_transition_group_new(void)
{
	return(CLUTTER_TRANSITION(g_object_new(ESDASHBOARD_TYPE_TRANSITION_GROUP, NULL)));
}

/**
 * esdashboard_transition_group_add_transition:
 * @self: A #EsdashboardTransitionGroup
 ...
 * when calling esdashboard_transition_group_remove_transition() or
 * esdashboard_transition_group_remove_all().
 */
													...
													ClutterTransition *inTransition)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_TRANSITION_GROUP(self));
...
/**
 * esdashboard_transition_group_remove_transition:
 * @self: A #EsdashboardTransitionGroup
 * @inTransition: A #ClutterTransition
 *
 ...
 * calling esdashboard_transition_group_add_transition().
 */
...
/**
 * esdashboard_transition_group_get_transitions:
 * @self: A #EsdashboardTransitionGroup
 *
...
GSList* esdashboard_transition_group_get_transitions(EsdashboardTransitionGroup *self)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_TRANSITION_GROUP(self), NULL);
