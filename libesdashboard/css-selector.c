...
struct _EsdashboardCssSelectorPrivate
{
	/* Properties related */
	gint							priority;

	/* Instance related */
	EsdashboardCssSelectorRule		*rule;
};

G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardCssSelector,
				esdashboard_css_selector,
				G_TYPE_OBJECT)
...
static GParamSpec* EsdashboardCssSelectorProperties[PROP_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */
typedef enum /*< skip,prefix=ESDASHBOARD_CSS_SELECTOR_RULE_MODE >*/
{
	ESDASHBOARD_CSS_SELECTOR_RULE_MODE_NONE=0,
	ESDASHBOARD_CSS_SELECTOR_RULE_MODE_PARENT,
	ESDASHBOARD_CSS_SELECTOR_RULE_MODE_ANCESTOR
} EsdashboardCssSelectorRuleMode;

struct _EsdashboardCssSelectorRule
{
	gchar							*type;
	gchar							*id;
	gchar							*classes;
	gchar							*pseudoClasses;
	EsdashboardCssSelectorRule		*parentRule;
	EsdashboardCssSelectorRuleMode	parentRuleMode;
...
/* Create rule */
static EsdashboardCssSelectorRule* _esdashboard_css_selector_rule_new(const gchar *inSource,
																		gint inPriority,
																		guint inLine,
																		guint inPosition)
{
	...
	rule=g_slice_new0(EsdashboardCssSelectorRule);
...
/* Destroy selector */
static void _esdashboard_css_selector_rule_free(EsdashboardCssSelectorRule *inRule)
{
	...
	if(inRule->parentRule) _esdashboard_css_selector_rule_free(inRule->parentRule);

	/* Free selector itself */
	g_slice_free(EsdashboardCssSelectorRule, inRule);
}

/* Get string for selector */
static gchar* _esdashboard_css_selector_rule_to_string(EsdashboardCssSelectorRule *inRule)
{
	...
	if(inRule->parentRule)
	{
		...
		{
			case ESDASHBOARD_CSS_SELECTOR_RULE_MODE_ANCESTOR:
			case ESDASHBOARD_CSS_SELECTOR_RULE_MODE_PARENT:
				temp=_esdashboard_css_selector_rule_to_string(inRule->parentRule);
				if(!temp)
				{
		...
		parentSelector=g_strdup_printf("%s%s ",
										temp,
										(inRule->parentRuleMode==ESDASHBOARD_CSS_SELECTOR_RULE_MODE_PARENT) ? " >" : "");
 ...
 */
static gboolean _esdashboard_css_selector_list_contains(const gchar *inNeedle,
														gint inNeedleLength,
 ...
 */
static gint _esdashboard_css_selector_score_node(EsdashboardCssSelectorRule *inRule,
															EsdashboardStylable *inStylable)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_STYLABLE(inStylable), -1);
	...
	id=esdashboard_stylable_get_name(ESDASHBOARD_STYLABLE(inStylable));
	classes=esdashboard_stylable_get_classes(ESDASHBOARD_STYLABLE(inStylable));
	pseudoClasses=esdashboard_stylable_get_pseudo_classes(ESDASHBOARD_STYLABLE(inStylable));
	...
	if(inRule->classes)
	{
		...
		for(needle=inRule->classes; needle; needle=strchr(needle, '.'))
		{
			 ...
			 * match
			 */
			if(!_esdashboard_css_selector_list_contains(needle, needleLength, classes, '.')) return(-1);
			numberMatches++;
		}
	...
	if(inRule->pseudoClasses)
	{
		...
		for(needle=inRule->pseudoClasses; needle; needle=strchr(needle, ':'))
		{
			 ...
			 * match
			 */
			if(!_esdashboard_css_selector_list_contains(needle, needleLength, pseudoClasses, ':')) return(-1);
			numberMatches++;
		}
	...
	{
		gint					parentScore;
		EsdashboardStylable		*parent;

		/* If node has no parent, no parent can match ;) so return immediately */
		parent=esdashboard_stylable_get_parent(inStylable);
		if(!parent || !ESDASHBOARD_IS_STYLABLE(parent)) return(-1);

		/* Check if there are matching parents. If not return immediately. */
		parentScore=_esdashboard_css_selector_score_node(inRule->parentRule, parent);
	...
	if(inRule->parentRule &&
		inRule->parentRuleMode==ESDASHBOARD_CSS_SELECTOR_RULE_MODE_ANCESTOR)
	{
		gint					ancestorScore;
		EsdashboardStylable		*ancestor;
		...
		do
		{
			ancestor=esdashboard_stylable_get_parent(ancestor);
		}
		while(ancestor && !ESDASHBOARD_IS_STYLABLE(ancestor));

		if(!ancestor || !ESDASHBOARD_IS_STYLABLE(ancestor)) return(-1);
		...
		while(ancestor)
		{
			 ...
			 */
			ancestorScore=_esdashboard_css_selector_score_node(inRule->parentRule, ancestor);
			if(ancestorScore>=0)
			{
				c+=ancestorScore;
				break;
			 ...
			 * the EsdashboardStylable interface
			 */
			do
			{
				ancestor=esdashboard_stylable_get_parent(ancestor);
...
/* Parse selector */
static GTokenType _esdashboard_css_selector_parse_css_simple_selector(EsdashboardCssSelector *self,
																		GScanner *inScanner,
																		EsdashboardCssSelectorRule *ioRule)
{
	GTokenType		token;

	g_return_val_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(self), G_TOKEN_ERROR);
...
static GTokenType _esdashboard_css_selector_parse_css_rule(EsdashboardCssSelector *self,
															GScanner *inScanner)
{
	EsdashboardCssSelectorPrivate	*priv;
	GTokenType						token;
	EsdashboardCssSelectorRule		*rule, *parentRule;
	...
	while(token!=G_TOKEN_EOF)
	{
		switch((guint)token)
		{
			...
			case ':':
				...
				/* Create new selector */
				rule=_esdashboard_css_selector_rule_new(inScanner->input_name,
														priv->priority,
														g_scanner_cur_line(inScanner),
														g_scanner_cur_position(inScanner));
				priv->rule=rule;
				...
				if(parentRule)
				{
					...
					rule->parentRuleMode=ESDASHBOARD_CSS_SELECTOR_RULE_MODE_ANCESTOR;
				}

				/* Parse selector */
				token=_esdashboard_css_selector_parse_css_simple_selector(self, inScanner, rule);
			...
			case '>':
				...
				rule->parentRule=parentRule;
				rule->parentRuleMode=ESDASHBOARD_CSS_SELECTOR_RULE_MODE_PARENT;
...
static gboolean _esdashboard_css_selector_parse(EsdashboardCssSelector *self, GScanner *ioScanner)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(self), FALSE);
	...
	{
		token=_esdashboard_css_selector_parse_css_rule(self, ioScanner);
...
/* Dispose this object */
static void _esdashboard_css_selector_dispose(GObject *inObject)
{
	EsdashboardCssSelector			*self=ESDASHBOARD_CSS_SELECTOR(inObject);
	EsdashboardCssSelectorPrivate	*priv=self->priv;

	/* Release allocated resources */
	if(priv->rule)
	{
		_esdashboard_css_selector_rule_free(priv->rule);
		priv->rule=NULL;
	}

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_css_selector_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_css_selector_set_property(GObject *inObject,
													guint inPropID,
...
static void _esdashboard_css_selector_get_property(GObject *inObject,
													guint inPropID,
													GValue *outValue,
													GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_css_selector_class_init(EsdashboardCssSelectorClass *klass)
{
	GObjectClass				*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->set_property=_esdashboard_css_selector_set_property;
	gobjectClass->get_property=_esdashboard_css_selector_get_property;
	gobjectClass->dispose=_esdashboard_css_selector_dispose;

	/* Define properties */
	EsdashboardCssSelectorProperties[PROP_PRIORITY]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardCssSelectorProperties);
}

/* Object initialization
 * Create private structure and set up default values
 */
static void esdashboard_css_selector_init(EsdashboardCssSelector *self)
{
	EsdashboardCssSelectorPrivate	*priv;

	priv=self->priv=esdashboard_css_selector_get_instance_private(self);
...
{
	return(esdashboard_css_selector_new_from_string_with_priority(inSelector, G_MININT));
}
...
EsdashboardCssSelector* esdashboard_css_selector_new_from_string_with_priority(const gchar *inSelector, gint inPriority)
{
	...
	selector=g_object_new(ESDASHBOARD_TYPE_CSS_SELECTOR,
							"priority", inPriority,
							NULL);
	if(!selector)
	{
	...
	if(!_esdashboard_css_selector_parse(ESDASHBOARD_CSS_SELECTOR(selector), scanner))
	{
	...
	return(ESDASHBOARD_CSS_SELECTOR(selector));
}
...
EsdashboardCssSelector* esdashboard_css_selector_new_from_scanner(GScanner *ioScanner,
																	...
																	gpointer inUserData)
{
	return(esdashboard_css_selector_new_from_scanner_with_priority(ioScanner, G_MININT, inFinishCallback, inUserData));
}
																				...
																				gpointer inUserData)
{
	...
	if(!_esdashboard_css_selector_parse(ESDASHBOARD_CSS_SELECTOR(selector), ioScanner))
	{
		g_object_unref(selector);
		return(NULL);
	}
	...
	if(inFinishCallback)
	{
		...
		goodState=(inFinishCallback)(ESDASHBOARD_CSS_SELECTOR(selector), ioScanner, g_scanner_peek_next_token(ioScanner), inUserData);
		if(!goodState)
		{
...
gchar* esdashboard_css_selector_to_string(EsdashboardCssSelector *self)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(self), NULL);
	...
	if(priv->rule)
	{
		selector=_esdashboard_css_selector_rule_to_string(priv->rule);
	}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(self), -1);
	g_return_val_if_fail(ESDASHBOARD_IS_STYLABLE(inStylable), -1);

	/* Check and score rules */
	return(_esdashboard_css_selector_score_node(self->priv->rule, inStylable));
}
...
void esdashboard_css_selector_adjust_to_offset(EsdashboardCssSelector *self, gint inLine, gint inPosition)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(self));
...
EsdashboardCssSelectorRule* esdashboard_css_selector_rule_get_parent(EsdashboardCssSelectorRule *inRule)
{
	...
	if(inRule->parentRuleMode!=ESDASHBOARD_CSS_SELECTOR_RULE_MODE_PARENT) return(NULL);
...
EsdashboardCssSelectorRule* esdashboard_css_selector_rule_get_ancestor(EsdashboardCssSelectorRule *inRule)
{
	...
	if(inRule->parentRuleMode!=ESDASHBOARD_CSS_SELECTOR_RULE_MODE_ANCESTOR) return(NULL);
	return(inRule->parentRule);
}
