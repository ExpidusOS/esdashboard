...
struct _EsdashboardApplicationTrackerPrivate
{
	/* Instance related */
	GList							*runningApps;
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardApplicationTracker,
				esdashboard_application_tracker,
				G_TYPE_OBJECT)
...
static guint EsdashboardApplicationTrackerSignals[SIGNAL_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Single instance of application database */
static EsdashboardApplicationTracker*		_esdashboard_application_tracker=NULL;

typedef struct _EsdashboardApplicationTrackerItem	EsdashboardApplicationTrackerItem;
struct _EsdashboardApplicationTrackerItem
{
	GPid				pid;
...
static gboolean _esdashboard_application_tracker_item_remove_window(EsdashboardApplicationTrackerItem *inItem,
																	EsdashboardWindowTrackerWindow *inWindow)
{
	...
	{
		/* Get window */
		window=(EsdashboardWindowTrackerWindow*)iter->data;
		if(!window) continue;
...
static gboolean _esdashboard_application_tracker_item_add_window(EsdashboardApplicationTrackerItem *inItem,
																EsdashboardWindowTrackerWindow *inWindow)
{
	GList									*iter;
	EsdashboardWindowTrackerWindow			*window;
...
static void _esdashboard_application_tracker_item_free(EsdashboardApplicationTrackerItem *inItem)
{
...
static EsdashboardApplicationTrackerItem* _esdashboard_application_tracker_item_new(GAppInfo *inAppInfo,
																					EsdashboardWindowTrackerWindow *inWindow)
{
	EsdashboardApplicationTrackerItem		*item;
	...
	item=g_new0(EsdashboardApplicationTrackerItem, 1);
	item->pid=esdashboard_window_tracker_window_get_pid(inWindow);
...
static EsdashboardApplicationTrackerItem* _esdashboard_application_tracker_find_item_by_desktop_id(EsdashboardApplicationTracker *self,
																									const gchar *inDesktopID)
{
	EsdashboardApplicationTrackerPrivate		*priv;
	GList										*iter;

	g_return_val_if_fail(ESDASHBOARD_IS_APPLICATION_TRACKER(self), NULL);
	...
	for(iter=priv->runningApps; iter; iter=g_list_next(iter))
	{
		...
		item=(EsdashboardApplicationTrackerItem*)iter->data;
		if(!item) continue;
...
static EsdashboardApplicationTrackerItem* _esdashboard_application_tracker_find_item_by_app_info(EsdashboardApplicationTracker *self,
																									GAppInfo *inAppInfo)
{
	g_return_val_if_fail(ESDASHBOARD_IS_APPLICATION_TRACKER(self), NULL);
	g_return_val_if_fail(G_IS_APP_INFO(inAppInfo), NULL);

	return(_esdashboard_application_tracker_find_item_by_desktop_id(self, g_app_info_get_id(inAppInfo)));
}

/* Find application tracker item by window */
static EsdashboardApplicationTrackerItem* _esdashboard_application_tracker_find_item_by_window(EsdashboardApplicationTracker *self,
																								EsdashboardWindowTrackerWindow *inWindow)
{
	EsdashboardApplicationTrackerPrivate		*priv;
	GList										*appIter;
	...
	for(appIter=priv->runningApps; appIter; appIter=g_list_next(appIter))
	{
		...
		/* Get application tracker item */
		item=(EsdashboardApplicationTrackerItem*)appIter->data;
		if(!item) continue;
		...
		{
			/* Get window */
			window=(EsdashboardWindowTrackerWindow*)windowIter->data;
			if(!window) continue;
 ...
 */
static GHashTable* _esdashboard_application_tracker_get_environment_from_pid(gint inPID)
{
	...
	{
		ESDASHBOARD_DEBUG(_esdashboard_application_tracker, APPLICATIONS,
 ...
 */
static GAppInfo* _esdashboard_application_tracker_get_desktop_id_from_environment(EsdashboardApplicationTracker *self,
																					EsdashboardWindowTrackerWindow *inWindow)
{
	EsdashboardApplicationTrackerPrivate	*priv;
	...
	windowPID=esdashboard_window_tracker_window_get_pid(inWindow);
	if(windowPID<=0)
	{
	...
	environments=_esdashboard_application_tracker_get_environment_from_pid(windowPID);
	if(!environments)
	{
		ESDASHBOARD_DEBUG(self, APPLICATIONS,
	...
	foundAppInfo=esdashboard_application_database_lookup_desktop_id(priv->appDatabase, value);
	if(!foundAppInfo)
	{
 ...
 * Callee is responsible to free result with g_object_unref().
 */
static GAppInfo* _esdashboard_application_tracker_get_desktop_id_from_window_names(EsdashboardApplicationTracker *self,
																					EsdashboardWindowTrackerWindow *inWindow)
{
	EsdashboardApplicationTrackerPrivate	*priv;
	GAppInfo								*foundAppInfo;
	...
	apps=esdashboard_application_database_get_all_applications(priv->appDatabase);

	/* Get window's names */
	names=esdashboard_window_tracker_window_get_instance_names(inWindow);
	...
	while(iter && *iter)
	{
		...
		appInfo=esdashboard_application_database_lookup_desktop_id(priv->appDatabase, iterName);
		...
		if(!appInfo)
		{
			...
			appInfo=esdashboard_application_database_lookup_desktop_id(priv->appDatabase, iterNameLowerCase);
		}
...
static void _esdashboard_application_tracker_on_window_opened(EsdashboardApplicationTracker *self,
																EsdashboardWindowTrackerWindow *inWindow,
																gpointer inUserData)
{
	EsdashboardApplicationTrackerPrivate	*priv;
	...
	g_return_if_fail(ESDASHBOARD_IS_APPLICATION_TRACKER(self));
	...
	appInfo=_esdashboard_application_tracker_get_desktop_id_from_environment(self, inWindow);
	if(!appInfo) appInfo=_esdashboard_application_tracker_get_desktop_id_from_window_names(self, inWindow);
	...
	item= _esdashboard_application_tracker_find_item_by_app_info(self, appInfo);
	if(!item)
	{
		/* Create application tracker item */
		item=_esdashboard_application_tracker_item_new(appInfo, inWindow);
		...
		ESDASHBOARD_DEBUG(self, APPLICATIONS,
							...
							item->desktopID);
		g_signal_emit(self, EsdashboardApplicationTrackerSignals[SIGNAL_STATE_CHANGED], g_quark_from_string(item->desktopID), item->desktopID, TRUE);
	}
		/* ... otherwise add window to existing one */
		else
		{
			/* Add window to list of window for known running application */
			_esdashboard_application_tracker_item_add_window(item, inWindow);
		}
...
/* A window was closed */
static void _esdashboard_application_tracker_on_window_closed(EsdashboardApplicationTracker *self,
																EsdashboardWindowTrackerWindow *inWindow,
																gpointer inUserData)
{
	EsdashboardApplicationTrackerPrivate	*priv;
	 ...
	 */
	item= _esdashboard_application_tracker_find_item_by_window(self, inWindow);
	if(!item)
	{
		ESDASHBOARD_DEBUG(self, APPLICATIONS,
	...
	_esdashboard_application_tracker_item_remove_window(item, inWindow);
		...
		g_list_length(item->windows)==0)
	{
		...
		_esdashboard_application_tracker_item_free(item);

		/* Emit signal as this application is not running anymore */
		ESDASHBOARD_DEBUG(self, APPLICATIONS,
							"Emitting signal 'state-changed' to stopped for desktop ID '%s'",
							desktopID);
		g_signal_emit(self, EsdashboardApplicationTrackerSignals[SIGNAL_STATE_CHANGED], g_quark_from_string(desktopID), desktopID, FALSE);
...
static void _esdashboard_application_tracker_on_active_window_changed(EsdashboardApplicationTracker *self,
																		EsdashboardWindowTrackerWindow *inOldActiveWindow,
																		EsdashboardWindowTrackerWindow *inNewActiveWindow,
																		gpointer inUserData)
{
	EsdashboardApplicationTrackerItem		*item;

	g_return_if_fail(ESDASHBOARD_IS_APPLICATION_TRACKER(self));
	g_return_if_fail(!inNewActiveWindow || ESDASHBOARD_IS_WINDOW_TRACKER_WINDOW(inNewActiveWindow));
	...
	{
		ESDASHBOARD_DEBUG(self, APPLICATIONS, "No new active window to check for running application.");
		return;
	}
	 ...
	 */
	item=_esdashboard_application_tracker_find_item_by_window(self, inNewActiveWindow);
	if(!item)
	{
		ESDASHBOARD_DEBUG(self, APPLICATIONS,
							"Could not find running application for new active window '%s'",
							esdashboard_window_tracker_window_get_name(inNewActiveWindow));
...
static GObject* _esdashboard_application_tracker_constructor(GType inType,
																guint inNumberConstructParams,
																GObjectConstructParam *inConstructParams)
{
	GObject									*object;

	if(!_esdashboard_application_tracker)
	{
		object=G_OBJECT_CLASS(esdashboard_application_tracker_parent_class)->constructor(inType, inNumberConstructParams, inConstructParams);
		_esdashboard_application_tracker=ESDASHBOARD_APPLICATION_TRACKER(object);
	}
		else
		{
			object=g_object_ref(G_OBJECT(_esdashboard_application_tracker));
		}
...
/* Dispose this object */
static void _esdashboard_application_tracker_dispose(GObject *inObject)
{
	EsdashboardApplicationTracker							*self=ESDASHBOARD_APPLICATION_TRACKER(inObject);
	EsdashboardApplicationTrackerPrivate					*priv=self->priv;

	/* Release allocated resources */
	if(priv->runningApps)
	{
		g_list_free_full(priv->runningApps, (GDestroyNotify)_esdashboard_application_tracker_item_free);
	...
	G_OBJECT_CLASS(esdashboard_application_tracker_parent_class)->dispose(inObject);
}

/* Finalize this object */
static void _esdashboard_application_tracker_finalize(GObject *inObject)
{
	/* Release allocated resources finally, e.g. unset singleton */
	if(G_LIKELY(G_OBJECT(_esdashboard_application_tracker)==inObject))
	{
		_esdashboard_application_tracker=NULL;
	}

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_application_tracker_parent_class)->finalize(inObject);
}
 ...
 * and signals
 */
static void esdashboard_application_tracker_class_init(EsdashboardApplicationTrackerClass *klass)
{
	GObjectClass		*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->constructor=_esdashboard_application_tracker_constructor;
	gobjectClass->dispose=_esdashboard_application_tracker_dispose;
	gobjectClass->finalize=_esdashboard_application_tracker_finalize;

	/* Define signals */
	EsdashboardApplicationTrackerSignals[SIGNAL_STATE_CHANGED]=
		g_signal_new("state-changed",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_NO_HOOKS | G_SIGNAL_DETAILED,
						G_STRUCT_OFFSET(EsdashboardApplicationTrackerClass, state_changed),
						NULL,
						NULL,
						_esdashboard_marshal_VOID__STRING_BOOLEAN,
						G_TYPE_NONE,
						2,
						G_TYPE_STRING,
						G_TYPE_BOOLEAN);
 ...
 */
static void esdashboard_application_tracker_init(EsdashboardApplicationTracker *self)
{
	EsdashboardApplicationTrackerPrivate	*priv;

	priv=self->priv=esdashboard_application_tracker_get_instance_private(self);
	...
	if(!esdashboard_application_database_is_loaded(priv->appDatabase))
	{
	...
	g_signal_connect_swapped(priv->windowTracker,
								"window-opened",
								G_CALLBACK(_esdashboard_application_tracker_on_window_opened),
								self);
	g_signal_connect_swapped(priv->windowTracker,
								"window-closed",
								G_CALLBACK(_esdashboard_application_tracker_on_window_closed),
								self);
	g_signal_connect_swapped(priv->windowTracker,
								"active-window-changed",
								G_CALLBACK(_esdashboard_application_tracker_on_active_window_changed),
								self);
}
...
EsdashboardApplicationTracker* esdashboard_application_tracker_get_default(void)
{
	...
	singleton=g_object_new(ESDASHBOARD_TYPE_APPLICATION_TRACKER, NULL);
	return(ESDASHBOARD_APPLICATION_TRACKER(singleton));
}
...
gboolean esdashboard_application_tracker_is_running_by_desktop_id(EsdashboardApplicationTracker *self,
																	const gchar *inDesktopID)
{
	g_return_val_if_fail(ESDASHBOARD_IS_APPLICATION_TRACKER(self), FALSE);
	g_return_val_if_fail(inDesktopID && *inDesktopID, FALSE);

	return(_esdashboard_application_tracker_find_item_by_desktop_id(self, inDesktopID) ? TRUE : FALSE);
}
																...
																GAppInfo *inAppInfo)
{
	...
	return(_esdashboard_application_tracker_find_item_by_app_info(self, inAppInfo) ? TRUE : FALSE);
}
																			...
																			const gchar *inDesktopID)
{
	...
	item=_esdashboard_application_tracker_find_item_by_desktop_id(self, inDesktopID);
	if(!item) return(NULL);
																			...
																			GAppInfo *inAppInfo)
{
	...
	/* Get application tracker item for requested desktop ID */
	item=_esdashboard_application_tracker_find_item_by_app_info(self, inAppInfo);
	if(!item) return(NULL);
