...
struct _EsdashboardScrollbarPrivate
{
	/* Properties related */
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardScrollbar,
							esdashboard_scrollbar,
...
static GParamSpec* EsdashboardScrollbarProperties[PROP_LAST]={ 0, };
...
static guint EsdashboardScrollbarSignals[SIGNAL_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Get value from coord */
static gfloat _esdashboard_scrollbar_get_value_from_coord(EsdashboardScrollbar *self,
															gfloat inX,
															gfloat inY,
															gfloat inAlignment)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_SCROLLBAR(self), 0.0f);
...
static gboolean _esdashboard_scrollbar_on_motion_event(ClutterActor *inActor,
														ClutterEvent *inEvent,
														gpointer inUserData)
{
	EsdashboardScrollbar			*self;
	EsdashboardScrollbarPrivate		*priv;
	gfloat							eventX, eventY;
	gfloat							x, y;
	gfloat							value;

	g_return_val_if_fail(ESDASHBOARD_IS_SCROLLBAR(inActor), FALSE);
	g_return_val_if_fail(inEvent, FALSE);

	self=ESDASHBOARD_SCROLLBAR(inActor);
	priv=self->priv;
	...
	value=_esdashboard_scrollbar_get_value_from_coord(self, x, y, priv->dragAlignment);
	esdashboard_scrollbar_set_value(self, value);

	return(CLUTTER_EVENT_STOP);
}

/* User released button in scroll bar */
static gboolean _esdashboard_scrollbar_on_button_released(ClutterActor *inActor,
															ClutterEvent *inEvent,
															gpointer inUserData)
{
	EsdashboardScrollbar			*self;
	...
	/* Remove style */
	esdashboard_stylable_remove_pseudo_class(ESDASHBOARD_STYLABLE(self), "pressed");
...
static gboolean _esdashboard_scrollbar_on_button_pressed(ClutterActor *inActor,
															ClutterEvent *inEvent,
															gpointer inUserData)
{
	EsdashboardScrollbar			*self;
	...
	value=_esdashboard_scrollbar_get_value_from_coord(self, x, y, 0.0f);
	if(value<priv->value || value>=(priv->value+priv->valueRange))
	{
		/* Set new value */
		value=_esdashboard_scrollbar_get_value_from_coord(self, x, y, 0.5f);
		esdashboard_scrollbar_set_value(self, value);
		return(FALSE);
	}
	...
	priv->signalMotionEventID=g_signal_connect_after(inActor,
														"motion-event",
														G_CALLBACK(_esdashboard_scrollbar_on_motion_event),
														NULL);
	priv->signalButtonReleasedID=g_signal_connect_after(inActor,
															"button-release-event",
															G_CALLBACK(_esdashboard_scrollbar_on_button_released),
															NULL);

	/* Add style */
	esdashboard_stylable_add_pseudo_class(ESDASHBOARD_STYLABLE(self), "pressed");
...
static gboolean _esdashboard_scrollbar_on_scroll_event(ClutterActor *inActor,
														ClutterEvent *inEvent,
														gpointer inUserData)
{
	EsdashboardScrollbar			*self;
...
static gboolean _esdashboard_scrollbar_on_draw_slider(EsdashboardScrollbar *self,
														cairo_t *inContext,
														int inWidth,
														int inHeight,
														gpointer inUserData)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_SCROLLBAR(self), TRUE);
	...
	if(barValueRange!=priv->valueRange)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_VALUE_RANGE]);

		/* Adjust value to fit into range (respecting value-range) if needed */
		if(priv->value+priv->valueRange>priv->range)
		{
			esdashboard_scrollbar_set_value(self, priv->range-priv->valueRange);
		}
	}
...
static void _esdashboard_scrollbar_get_preferred_height(ClutterActor *self,
														gfloat inForWidth,
														gfloat *outMinHeight,
														gfloat *outNaturalHeight)
{
	EsdashboardScrollbarPrivate		*priv=ESDASHBOARD_SCROLLBAR(self)->priv;
	...
	if(priv->orientation==CLUTTER_ORIENTATION_HORIZONTAL)
	{
		...
		else
		{
			...
			ClutterActorClass		*actorClass=CLUTTER_ACTOR_CLASS(esdashboard_scrollbar_parent_class);
...
static void _esdashboard_scrollbar_get_preferred_width(ClutterActor *self,
														gfloat inForHeight,
														gfloat *outMinWidth,
...
static void _esdashboard_scrollbar_allocate(ClutterActor *self,
												...
												ClutterAllocationFlags inFlags)
{
	...
	CLUTTER_ACTOR_CLASS(esdashboard_scrollbar_parent_class)->allocate(self, inBox, inFlags);
...
/* Dispose this object */
static void _esdashboard_scrollbar_dispose(GObject *inObject)
{
	EsdashboardScrollbar			*self=ESDASHBOARD_SCROLLBAR(inObject);
	EsdashboardScrollbarPrivate		*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_scrollbar_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_scrollbar_set_property(GObject *inObject,
												guint inPropID,
												...
												GParamSpec *inSpec)
{
	...
	switch(inPropID)
	{
		case PROP_ORIENTATION:
			esdashboard_scrollbar_set_orientation(self, g_value_get_enum(inValue));
			break;

		case PROP_VALUE:
			esdashboard_scrollbar_set_value(self, g_value_get_float(inValue));
			break;

		case PROP_RANGE:
			esdashboard_scrollbar_set_range(self, g_value_get_float(inValue));
			break;

		case PROP_PAGE_SIZE_FACTOR:
			esdashboard_scrollbar_set_page_size_factor(self, g_value_get_float(inValue));
			break;

		case PROP_SPACING:
			esdashboard_scrollbar_set_spacing(self, g_value_get_float(inValue));
			break;

		case PROP_SLIDER_WIDTH:
			esdashboard_scrollbar_set_slider_width(self, g_value_get_float(inValue));
			break;

		case PROP_SLIDER_RADIUS:
			esdashboard_scrollbar_set_slider_radius(self, g_value_get_float(inValue));
			break;

		case PROP_SLIDER_COLOR:
			esdashboard_scrollbar_set_slider_color(self, clutter_value_get_color(inValue));
			break;
...
static void _esdashboard_scrollbar_get_property(GObject *inObject,
												guint inPropID,
												GValue *outValue,
												GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_scrollbar_class_init(EsdashboardScrollbarClass *klass)
{
	...
	/* Override functions */
	gobjectClass->set_property=_esdashboard_scrollbar_set_property;
	gobjectClass->get_property=_esdashboard_scrollbar_get_property;
	gobjectClass->dispose=_esdashboard_scrollbar_dispose;

	clutterActorClass->get_preferred_width=_esdashboard_scrollbar_get_preferred_width;
	clutterActorClass->get_preferred_height=_esdashboard_scrollbar_get_preferred_height;
	clutterActorClass->allocate=_esdashboard_scrollbar_allocate;

	/* Define properties */
	EsdashboardScrollbarProperties[PROP_ORIENTATION]=
	...
	EsdashboardScrollbarProperties[PROP_VALUE]=
		g_param_spec_float("value",
							"Value",
							"Current value of scroll bar within range",
							0.0f, G_MAXFLOAT,
							0.0f,
							G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

	EsdashboardScrollbarProperties[PROP_VALUE_RANGE]=
	...
	EsdashboardScrollbarProperties[PROP_RANGE]=
	...
	EsdashboardScrollbarProperties[PROP_PAGE_SIZE_FACTOR]=
	...
	EsdashboardScrollbarProperties[PROP_SPACING]=
	...
	EsdashboardScrollbarProperties[PROP_SLIDER_WIDTH]=
	...
	EsdashboardScrollbarProperties[PROP_SLIDER_RADIUS]=
	...
	EsdashboardScrollbarProperties[PROP_SLIDER_COLOR]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardScrollbarProperties);

	/* Define stylable properties */
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardScrollbarProperties[PROP_ORIENTATION]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardScrollbarProperties[PROP_PAGE_SIZE_FACTOR]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardScrollbarProperties[PROP_SPACING]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardScrollbarProperties[PROP_SLIDER_WIDTH]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardScrollbarProperties[PROP_SLIDER_RADIUS]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardScrollbarProperties[PROP_SLIDER_COLOR]);

	/* Define signals */
	EsdashboardScrollbarSignals[SIGNAL_VALUE_CHANGED]=
		g_signal_new("value-changed",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardScrollbarClass, value_changed),
						NULL,
						NULL,
 ...
 */
static void esdashboard_scrollbar_init(EsdashboardScrollbar *self)
{
	EsdashboardScrollbarPrivate		*priv;

	priv=self->priv=esdashboard_scrollbar_get_instance_private(self);
	...
	/* Connect signals */
	g_signal_connect_swapped(priv->slider, "draw", G_CALLBACK(_esdashboard_scrollbar_on_draw_slider), self);
	g_signal_connect(self, "button-press-event", G_CALLBACK(_esdashboard_scrollbar_on_button_pressed), NULL);
	g_signal_connect(self, "scroll-event", G_CALLBACK(_esdashboard_scrollbar_on_scroll_event), NULL);
}
...
{
	return(g_object_new(ESDASHBOARD_TYPE_SCROLLBAR,
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_SCROLLBAR(self), CLUTTER_ORIENTATION_HORIZONTAL);
...
void esdashboard_scrollbar_set_orientation(EsdashboardScrollbar *self, ClutterOrientation inOrientation)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_SCROLLBAR(self));
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_ORIENTATION]);
}
...
void esdashboard_scrollbar_set_value(EsdashboardScrollbar *self, gfloat inValue)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_VALUE]);
	g_signal_emit(self, EsdashboardScrollbarSignals[SIGNAL_VALUE_CHANGED], 0, priv->value);
}
...
void esdashboard_scrollbar_set_range(EsdashboardScrollbar *self, gfloat inRange)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_RANGE]);
	...
	if(priv->value>priv->range)
	{
		...
		ESDASHBOARD_DEBUG(self, ACTOR,
							...
							priv->range);
		oldValue=priv->value;
		esdashboard_scrollbar_set_value(self, priv->range);
		ESDASHBOARD_DEBUG(self, ACTOR,
...
void esdashboard_scrollbar_set_page_size_factor(EsdashboardScrollbar *self, gfloat inFactor)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_PAGE_SIZE_FACTOR]);
}
...
void esdashboard_scrollbar_set_spacing(EsdashboardScrollbar *self, gfloat inSpacing)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_SPACING]);
}
...
void esdashboard_scrollbar_set_slider_width(EsdashboardScrollbar *self, gfloat inWidth)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_SLIDER_WIDTH]);
}
...
void esdashboard_scrollbar_set_slider_radius(EsdashboardScrollbar *self, gfloat inRadius)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_SLIDER_RADIUS]);
}

/* Get/set color of slider */
const ClutterColor* esdashboard_scrollbar_get_slider_color(EsdashboardScrollbar *self)
{
	g_return_val_if_fail(ESDASHBOARD_IS_SCROLLBAR(self), NULL);
...
void esdashboard_scrollbar_set_slider_color(EsdashboardScrollbar *self, const ClutterColor *inColor)
{
	...
	if(priv->sliderColor==NULL || clutter_color_equal(inColor, priv->sliderColor)==FALSE)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScrollbarProperties[PROP_SLIDER_COLOR]);
	}
}
