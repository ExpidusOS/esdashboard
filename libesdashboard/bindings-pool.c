...
struct _EsdashboardBindingsPoolPrivate
{
	/* Instance related */
	GHashTable		*bindings;
};

G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardBindingsPool,
							esdashboard_bindings_pool,
							G_TYPE_OBJECT)
...
typedef struct _EsdashboardBindingsPoolParserData	EsdashboardBindingsPoolParserData;
struct _EsdashboardBindingsPoolParserData
{
	EsdashboardBindingsPool		*self;
	...
	EsdashboardBinding			*lastBinding;
};

typedef struct _EsdashboardBindingsPoolModifierMap	EsdashboardBindingsPoolModifierMap;
struct _EsdashboardBindingsPoolModifierMap
{
	const gchar							*name;
...
static void _esdashboard_bindings_pool_parse_set_error(EsdashboardBindingsPoolParserData *inParserData,
														GMarkupParseContext *inContext,
														GError **outError,
														EsdashboardBindingsPoolErrorEnum inCode,
														const gchar *inFormat,
														...) G_GNUC_PRINTF (5, 6);

/* Single instance of bindings */
static EsdashboardBindingsPool*				_esdashboard_bindings_pool=NULL;

/* Modifier map for conversion from string to integer used by parser */
static EsdashboardBindingsPoolModifierMap	_esdashboard_bindings_pool_modifier_map[]=
											{
														...
														...)
{
	...
	/* Create error object */
	tempError=g_error_new_literal(ESDASHBOARD_BINDINGS_POOL_ERROR, inCode, message);
	if(inParserData)
	{
...
static gint _esdashboard_bindings_pool_get_tag_by_name(const gchar *inTag)
{
...
static const gchar* _esdashboard_bindings_pool_get_tag_by_id(guint inTagType)
{
...
static gboolean _esdashboard_bindings_pool_parse_keycode(const gchar *inText,
															guint *outKey,
															ClutterModifierType *outModifiers)
{
	...
	while(*iter)
	{
		EsdashboardBindingsPoolModifierMap	*mapIter;
		...
		if(*iter[0]=='<')
		{
			...
			for(mapIter=_esdashboard_bindings_pool_modifier_map; !wasModifier && mapIter->name; mapIter++)
			{
...
static void _esdashboard_bindings_pool_parse_general_no_text_nodes(GMarkupParseContext *inContext,
																	const gchar *inText,
																	gsize inTextLength,
																	gpointer inUserData,
																	GError **outError)
{
	EsdashboardBindingsPoolParserData			*data=(EsdashboardBindingsPoolParserData*)inUserData;
	gchar										*realText;
	...
	if(*realText)
	{
		...
		_esdashboard_bindings_pool_parse_set_error(data,
												inContext,
												outError,
...
static void _esdashboard_bindings_pool_parse_general_action_text_node(GMarkupParseContext *inContext,
																		const gchar *inText,
																		gsize inTextLength,
																		gpointer inUserData,
																		GError **outError)
{
	...
	esdashboard_binding_set_action(data->lastBinding, action);
...
static void _esdashboard_bindings_pool_parse_bindings_start(GMarkupParseContext *inContext,
															...
															GError **outError)
{
	...
	nextTag=_esdashboard_bindings_pool_get_tag_by_name(inElementName);
	if(nextTag==-1)
	{
		_esdashboard_bindings_pool_parse_set_error(data,
												inContext,
	...
	if(nextTag==TAG_KEY)
	{
		static GMarkupParser					keyParser=
												{
													NULL,
													NULL,
													_esdashboard_bindings_pool_parse_general_action_text_node,
													NULL,
													NULL,
												};
		gchar									*keycode=NULL;
		...
		EsdashboardBindingFlags					flags=0;
		...
		/* Parse keycode */
		if(!_esdashboard_bindings_pool_parse_keycode(keycode, &key, &modifiers))
		{
			/* Set error */
			_esdashboard_bindings_pool_parse_set_error(data,
														inContext,
		...
		/* Parse keycode */
		data->lastBinding=esdashboard_binding_new();
		if(!data->lastBinding)
		{
			/* Set error */
			_esdashboard_bindings_pool_parse_set_error(data,
		...
		if(allowUnfocusableTargets)
		{
			...
			{
				flags|=ESDASHBOARD_BINDING_FLAGS_ALLOW_UNFOCUSABLE_TARGET;
			}
		...
		esdashboard_binding_set_event_type(data->lastBinding, eventType);
		esdashboard_binding_set_class_name(data->lastBinding, source);
		esdashboard_binding_set_key(data->lastBinding, key);
		esdashboard_binding_set_modifiers(data->lastBinding, modifiers);
		if(target) esdashboard_binding_set_target(data->lastBinding, target);
		esdashboard_binding_set_flags(data->lastBinding, flags);
	...
	_esdashboard_bindings_pool_parse_set_error(data,
											...
											_esdashboard_bindings_pool_get_tag_by_id(currentTag),
											inElementName);
}

static void _esdashboard_bindings_pool_parse_bindings_end(GMarkupParseContext *inContext,
															const gchar *inElementName,
															gpointer inUserData,
															GError **outError)
{
...
/* Parser callbacks for document root node */
static void _esdashboard_bindings_pool_parse_document_start(GMarkupParseContext *inContext,
														const gchar *inElementName,
														const gchar **inAttributeNames,
														const gchar **inAttributeValues,
														gpointer inUserData,
														GError **outError)
{
	...
	if(nextTag==TAG_BINDINGS_POOL)
	{
		static GMarkupParser					propertyParser=
												{
													_esdashboard_bindings_pool_parse_bindings_start,
													_esdashboard_bindings_pool_parse_bindings_end,
													_esdashboard_bindings_pool_parse_general_no_text_nodes,
													NULL,
													NULL,
												};
...
static void _esdashboard_bindings_pool_parse_document_end(GMarkupParseContext *inContext,
														const gchar *inElementName,
														gpointer inUserData,
														GError **outError)
{
...
static void _esdashboard_bindings_pool_merge_bindings(gpointer inKey,
														gpointer inValue,
														gpointer inUserData)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_BINDING(inKey));
	g_return_if_fail(inUserData);

	targetBindingsHashTable=(GHashTable*)inUserData;
	binding=ESDASHBOARD_BINDING(inKey);
...
static gboolean _esdashboard_bindings_pool_load_bindings_from_file(EsdashboardBindingsPool *self,
																	const gchar *inPath,
																	GError **outError)
{
	static GMarkupParser				parser=
										{
											_esdashboard_bindings_pool_parse_document_start,
											_esdashboard_bindings_pool_parse_document_end,
											_esdashboard_bindings_pool_parse_general_no_text_nodes,
											NULL,
											NULL,
										};
	...
	EsdashboardBindingsPoolParserData	*data;
	GError								*error;
	gboolean							success;

	g_return_val_if_fail(ESDASHBOARD_IS_BINDINGS_POOL(self), FALSE);
	...
	data=g_new0(EsdashboardBindingsPoolParserData, 1);
	if(!data)
	{
		/* Set error */
		g_set_error(outError,
					ESDASHBOARD_BINDINGS_POOL_ERROR,
					ESDASHBOARD_BINDINGS_POOL_ERROR_PARSER_INTERNAL_ERROR,
	...
	data->bindings=g_hash_table_new_full(esdashboard_binding_hash,
											esdashboard_binding_compare,
	...
	if(success)
	{
		...
		g_hash_table_foreach(data->bindings, _esdashboard_bindings_pool_merge_bindings, priv->bindings);
		ESDASHBOARD_DEBUG(self, MISC,
...
static GObject* _esdashboard_bindings_pool_constructor(GType inType,
														guint inNumberConstructParams,
														GObjectConstructParam *inConstructParams)
{
	GObject									*object;

	if(!_esdashboard_bindings_pool)
	{
		object=G_OBJECT_CLASS(esdashboard_bindings_pool_parent_class)->constructor(inType, inNumberConstructParams, inConstructParams);
		_esdashboard_bindings_pool=ESDASHBOARD_BINDINGS_POOL(object);
	}
		else
		{
			object=g_object_ref(G_OBJECT(_esdashboard_bindings_pool));
		}
...
/* Dispose this object */
static void _esdashboard_bindings_pool_dispose(GObject *inObject)
{
	/* Release allocated variables */
	EsdashboardBindingsPool				*self=ESDASHBOARD_BINDINGS_POOL(inObject);
	EsdashboardBindingsPoolPrivate		*priv=self->priv;
	...
	G_OBJECT_CLASS(esdashboard_bindings_pool_parent_class)->dispose(inObject);
}

/* Finalize this object */
static void _esdashboard_bindings_pool_finalize(GObject *inObject)
{
	/* Release allocated resources finally, e.g. unset singleton */
	if(G_LIKELY(G_OBJECT(_esdashboard_bindings_pool)==inObject))
	{
		_esdashboard_bindings_pool=NULL;
	}

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_bindings_pool_parent_class)->finalize(inObject);
}
 ...
 * and signals
 */
static void esdashboard_bindings_pool_class_init(EsdashboardBindingsPoolClass *klass)
{
	GObjectClass					*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->constructor=_esdashboard_bindings_pool_constructor;
	gobjectClass->dispose=_esdashboard_bindings_pool_dispose;
	gobjectClass->finalize=_esdashboard_bindings_pool_finalize;
}

/* Object initialization
 * Create private structure and set up default values
 */
static void esdashboard_bindings_pool_init(EsdashboardBindingsPool *self)
{
	EsdashboardBindingsPoolPrivate		*priv;

	priv=self->priv=esdashboard_bindings_pool_get_instance_private(self);
...
{
	return(g_quark_from_static_string("esdashboard-bindings-error-quark"));
}
...
EsdashboardBindingsPool* esdashboard_bindings_pool_get_default(void)
{
	...
	singleton=g_object_new(ESDASHBOARD_TYPE_BINDINGS_POOL, NULL);
	return(ESDASHBOARD_BINDINGS_POOL(singleton));
}
...
gboolean esdashboard_bindings_pool_load(EsdashboardBindingsPool *self, GError **outError)
{
	...
	if(priv->bindings)
	{
		...
		ESDASHBOARD_DEBUG(self, MISC, "Removed current bindings because of reloading bindings configuration files");
	}

	/* Create hash-table to store bindings at */
	priv->bindings=g_hash_table_new_full(esdashboard_binding_hash,
											esdashboard_binding_compare,
											(GDestroyNotify)g_object_unref,
											NULL);
	if(!priv->bindings)
	 ...
	 * usually located at /usr/share/esdashboard. The file is called "bindings.xml".
	 */
	if(success)
	{
		configFile=g_build_filename(PACKAGE_DATADIR, "esdashboard", "bindings.xml", NULL);
		ESDASHBOARD_DEBUG(self, MISC,
							"Trying system bindings configuration file: %s",
							configFile);
		if(g_file_test(configFile, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_REGULAR))
		{
			if(!_esdashboard_bindings_pool_load_bindings_from_file(self, configFile, &error))
			{
	 ...
	 * the folder 'esdashboard' at configuration directory in user's home
	 * path (usually ~/.config/esdashboard). The file is called "bindings.xml".
	 */
	if(success)
	{
		configFile=g_build_filename(g_get_user_config_dir(), "esdashboard", "bindings.xml", NULL);
		ESDASHBOARD_DEBUG(self, MISC,
	...
	if(success)
	{
		...
		envFile=g_getenv("ESDASHBOARD_BINDINGS_POOL_FILE");
		if(envFile)
		{
...
{
	EsdashboardBindingsPoolPrivate		*priv;
	EsdashboardBinding					*lookupBinding;
	const EsdashboardBinding			*foundBinding;
	GType								classType;
	GSList								*interfaces;

	g_return_val_if_fail(ESDASHBOARD_IS_BINDINGS_POOL(self), NULL);
	...
	lookupBinding=esdashboard_binding_new_for_event(inEvent);
	...
	while(classType)
	{
		...
		esdashboard_binding_set_class_name(lookupBinding, g_type_name(classType));
		...
		if(g_hash_table_lookup_extended(priv->bindings, lookupBinding, (gpointer*)&foundBinding, NULL))
		{
			ESDASHBOARD_DEBUG(self, MISC,
								...
								esdashboard_binding_get_class_name(lookupBinding),
								esdashboard_binding_get_key(lookupBinding),
								esdashboard_binding_get_modifiers(lookupBinding));
	...
	if(interfaces)
	{
		...
		do
		{
			...
			esdashboard_binding_set_class_name(lookupBinding, g_type_name(classInterface));
