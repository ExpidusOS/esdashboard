...
/* Callback function for esdashboard_traverse_actor() to find stage interface
 * used in esdashboard_notify().
 */
static gboolean _esdashboard_notify_traverse_callback(ClutterActor *inActor, gpointer inUserData)
{
	EsdashboardStage					**outStageInterface;
	EsdashboardWindowTrackerMonitor		*stageMonitor;

	g_return_val_if_fail(CLUTTER_IS_ACTOR(inActor), ESDASHBOARD_TRAVERSAL_CONTINUE);
	g_return_val_if_fail(inUserData, ESDASHBOARD_TRAVERSAL_CONTINUE);

	outStageInterface=(EsdashboardStage**)inUserData;

	/* If actor currently traverse is a stage interface then store
	 * the actor in user-data and stop further traversal.
	 */
	if(ESDASHBOARD_IS_STAGE_INTERFACE(inActor))
	{
		stageMonitor=esdashboard_stage_interface_get_monitor(ESDASHBOARD_STAGE_INTERFACE(inActor));
		if(esdashboard_window_tracker_monitor_is_primary(stageMonitor))
		{
			*outStageInterface=ESDASHBOARD_STAGE(clutter_actor_get_stage(inActor));
			return(ESDASHBOARD_TRAVERSAL_STOP);
		}
	}
							...
							const gchar *inFormat, ...)
{
	...
	if(inSender) stage=ESDASHBOARD_STAGE(clutter_actor_get_stage(inSender));
	...
	if(!stage)
	{
		...
		selector=esdashboard_css_selector_new_from_string("EsdashboardStageInterface");
		esdashboard_traverse_actor(NULL, selector, _esdashboard_notify_traverse_callback, &stage);
	...
	if(stage) esdashboard_stage_show_notification(stage, inIconName, text);
...
/**
 * esdashboard_create_app_context:
...
GAppLaunchContext* esdashboard_create_app_context(EsdashboardWindowTrackerWorkspace *inWorkspace)
{
	...
	g_return_val_if_fail(inWorkspace==NULL || ESDASHBOARD_IS_WINDOW_TRACKER_WORKSPACE(inWorkspace), NULL);
	...
	if(!inWorkspace)
	{
		tracker=esdashboard_window_tracker_get_default();
		inWorkspace=esdashboard_window_tracker_get_active_workspace(tracker);
	...
	gdk_app_launch_context_set_desktop(context, esdashboard_window_tracker_workspace_get_number(inWorkspace));
...
static void _esdashboard_gvalue_transform_string_int(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_int=g_ascii_strtoll(inSourceValue->data[0].v_pointer, NULL, 10);
}

static void _esdashboard_gvalue_transform_string_uint(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_uint=g_ascii_strtoull(inSourceValue->data[0].v_pointer, NULL, 10);
}

static void _esdashboard_gvalue_transform_string_long(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_long=g_ascii_strtoll(inSourceValue->data[0].v_pointer, NULL, 10);
}

static void _esdashboard_gvalue_transform_string_ulong(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_ulong=g_ascii_strtoull(inSourceValue->data[0].v_pointer, NULL, 10);
}

static void _esdashboard_gvalue_transform_string_int64(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_int64=g_ascii_strtoll(inSourceValue->data[0].v_pointer, NULL, 10);
}

static void _esdashboard_gvalue_transform_string_uint64(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_uint64=g_ascii_strtoull(inSourceValue->data[0].v_pointer, NULL, 10);
}

static void _esdashboard_gvalue_transform_string_float(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_float=g_ascii_strtod(inSourceValue->data[0].v_pointer, NULL);
}

static void _esdashboard_gvalue_transform_string_double(const GValue *inSourceValue, GValue *ioDestValue)
{
	ioDestValue->data[0].v_double=g_ascii_strtod(inSourceValue->data[0].v_pointer, NULL);
}

static void _esdashboard_gvalue_transform_string_boolean(const GValue *inSourceValue, GValue *ioDestValue)
{
...
static void _esdashboard_gvalue_transform_string_enum(const GValue *inSourceValue, GValue *ioDestValue)
{
...
static void _esdashboard_gvalue_transform_string_flags(const GValue *inSourceValue, GValue *ioDestValue)
{
...
void esdashboard_register_gvalue_transformation_funcs(void)
{
	...
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_INT, _esdashboard_gvalue_transform_string_int);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_UINT, _esdashboard_gvalue_transform_string_uint);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_LONG, _esdashboard_gvalue_transform_string_long);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_ULONG, _esdashboard_gvalue_transform_string_ulong);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_INT64, _esdashboard_gvalue_transform_string_int64);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_UINT64, _esdashboard_gvalue_transform_string_uint64);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_FLOAT, _esdashboard_gvalue_transform_string_float);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_DOUBLE, _esdashboard_gvalue_transform_string_double);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_BOOLEAN, _esdashboard_gvalue_transform_string_boolean);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_FLAGS, _esdashboard_gvalue_transform_string_flags);
	g_value_register_transform_func(G_TYPE_STRING, G_TYPE_ENUM, _esdashboard_gvalue_transform_string_enum);
}

/**
 * esdashboard_find_actor_by_name:
...
ClutterActor* esdashboard_find_actor_by_name(ClutterActor *inActor, const gchar *inName)
{
	...
	{
		result=esdashboard_find_actor_by_name(child, inName);
...
static gboolean _esdashboard_traverse_actor_internal(ClutterActor *inActor,
														...
														gpointer inUserData)
{
	...
	g_return_val_if_fail(CLUTTER_IS_ACTOR(inActor), ESDASHBOARD_TRAVERSAL_CONTINUE);
	g_return_val_if_fail(ESDASHBOARD_IS_CSS_SELECTOR(inSelector), ESDASHBOARD_TRAVERSAL_CONTINUE);
	g_return_val_if_fail(inCallback, ESDASHBOARD_TRAVERSAL_CONTINUE);
	...
	{
		score=esdashboard_css_selector_score(inSelector, ESDASHBOARD_STYLABLE(inActor));
		if(score>=0)
		{
	...
	while(clutter_actor_iter_next(&iter, &child))
	{
		doContinueTraversal=_esdashboard_traverse_actor_internal(child, inSelector, inCallback, inUserData);
		if(!doContinueTraversal) return(doContinueTraversal);
	}
...
/**
 * esdashboard_traverse_actor:
 * @inRootActor: The root #ClutterActor where to begin traversing
 * @inSelector: A #EsdashboardCssSelector to filter actors while traversing or
								...
								gpointer inUserData)
{
	...
	g_return_if_fail(!inSelector || ESDASHBOARD_IS_CSS_SELECTOR(inSelector));
	g_return_if_fail(inCallback);

	/* If root actor where begin traversal is NULL then begin at stage */
	if(!inRootActor)
	{
		inRootActor=CLUTTER_ACTOR(esdashboard_application_get_stage(NULL));
		...
		if(!inRootActor)
		{
			ESDASHBOARD_DEBUG(NULL, MISC, "No root actor to begin traversal at was provided and no stage available");
			return;
		}
	}
	 ...
	 */
	if(!inSelector) inSelector=esdashboard_css_selector_new_from_string("*");
		else g_object_ref(inSelector);

	/* Do traversal */
	_esdashboard_traverse_actor_internal(inRootActor, inSelector, inCallback, inUserData);
...
/**
 * esdashboard_get_stage_of_actor:
 * @inActor: The #ClutterActor for which to find the stage
 *
 * Gets the #EsdashboardStageInterface of the monitor where
 * @inActor belongs to.
 *
 * Return value: (transfer none): The #EsdashboardStageInterface
...
EsdashboardStageInterface* esdashboard_get_stage_of_actor(ClutterActor *inActor)
{
	...
	/* Iterate through parents and return first EsdashboardStageInterface
	...
	while(parent)
	{
		/* Check if current iterated parent is a EsdashboardStageInterface.
		 * If it is return it.
		 */
		if(ESDASHBOARD_IS_STAGE_INTERFACE(parent)) return(ESDASHBOARD_STAGE_INTERFACE(parent));
...
/**
 * esdashboard_get_enum_value_name:
...
/**
 * esdashboard_get_enum_value_from_nickname:
 * @inEnumClass: The #GType of enum class
...
/* Dump actors */
static void _esdashboard_dump_actor_print(ClutterActor *inActor, gint inLevel)
{
	EsdashboardStylable		*stylable;
	...
	stylable=NULL;
	if(ESDASHBOARD_IS_STYLABLE(inActor)) stylable=ESDASHBOARD_STYLABLE(inActor);
	...
	g_print("+- %s@%p [%s%s%s%s%s%s] - geometry: %.2f,%.2f [%.2fx%.2f], mapped: %s, visible: %s, layout: %s, children: %d\n",
				...
				stylable && esdashboard_stylable_get_classes(stylable) ? "." : "",
				stylable && esdashboard_stylable_get_classes(stylable) ? esdashboard_stylable_get_classes(stylable) : "",
				stylable && esdashboard_stylable_get_pseudo_classes(stylable) ? ":" : "",
				stylable && esdashboard_stylable_get_pseudo_classes(stylable) ? esdashboard_stylable_get_pseudo_classes(stylable) : "",
				allocation.x1,
				allocation.y1,
...
static void _esdashboard_dump_actor_internal(ClutterActor *inActor, gint inLevel)
{
	ClutterActorIter		iter;
	ClutterActor			*child;
	...
	while(clutter_actor_iter_next(&iter, &child))
	{
		_esdashboard_dump_actor_print(child, inLevel);
		if(clutter_actor_get_n_children(child)>0) _esdashboard_dump_actor_internal(child, inLevel+1);
	}
}

/**
 * esdashboard_dump_actor:
