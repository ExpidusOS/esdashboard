...
struct _EsdashboardApplicationDatabasePrivate
{
	/* Properties related */
	gboolean			isLoaded;
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardApplicationDatabase,
							esdashboard_application_database,
							G_TYPE_OBJECT)
...
static GParamSpec* EsdashboardApplicationDatabaseProperties[PROP_LAST]={ 0, };
...
static guint EsdashboardApplicationDatabaseSignals[SIGNAL_LAST]={ 0, };

/* IMPLEMENTATION: Private variables and methods */

/* Single instance of application database */
static EsdashboardApplicationDatabase*		_esdashboard_application_database=NULL;

typedef struct _EsdashboardApplicationDatabaseFileMonitorData	EsdashboardApplicationDatabaseFileMonitorData;
struct _EsdashboardApplicationDatabaseFileMonitorData
{
	GFile				*path;
...
static void _esdashboard_application_database_add_hashtable_item_to_list(gpointer inKey,
																			gpointer inValue,
																			gpointer inUserData)
{
	GList									**applicationsList;

	g_return_if_fail(ESDASHBOARD_DESKTOP_APP_INFO(inValue));

	applicationsList=(GList**)inUserData;

	/* Add a reference of value (a EsdashboardDesktopAppInfo) to list */
...
static EsdashboardApplicationDatabaseFileMonitorData* _esdashboard_application_database_monitor_data_new(GFile *inPath)
{
	EsdashboardApplicationDatabaseFileMonitorData	*monitorData;

	g_return_val_if_fail(G_IS_FILE(inPath), NULL);

	/* Allocate memory for file monitor data structure */
	monitorData=g_new0(EsdashboardApplicationDatabaseFileMonitorData, 1);
...
static void _esdashboard_application_database_monitor_data_free(EsdashboardApplicationDatabaseFileMonitorData *inData)
{
...
static EsdashboardApplicationDatabaseFileMonitorData* _esdashboard_application_database_monitor_data_find_by_monitor(EsdashboardApplicationDatabase *self,
																														GFileMonitor *inMonitor)
{
	EsdashboardApplicationDatabasePrivate			*priv;
	GList											*iter;
	EsdashboardApplicationDatabaseFileMonitorData	*iterData;
	...
	{
		iterData=(EsdashboardApplicationDatabaseFileMonitorData*)iter->data;
		if(iterData &&
			iterData->monitor &&
...
static EsdashboardApplicationDatabaseFileMonitorData* _esdashboard_application_database_monitor_data_find_by_file(EsdashboardApplicationDatabase *self,
																													GFile *inPath)
{
	EsdashboardApplicationDatabasePrivate			*priv;
	GList											*iter;
...
static void _esdashboard_application_database_on_file_monitor_changed(EsdashboardApplicationDatabase *self,
																		GFile *inFile,
																		GFile *inOtherFile,
																		...
																		gpointer inUserData)
{
	...
	monitorData=_esdashboard_application_database_monitor_data_find_by_monitor(self, monitor);
	if(!monitorData)
	{
	...
	{
		EsdashboardApplicationDatabaseFileMonitorData	*fileMonitorData;
		GError											*error;
		 ...
		 */
		fileMonitorData=_esdashboard_application_database_monitor_data_new(inFile);
		if(!fileMonitorData)
		{
		...
		if(!fileMonitorData->monitor)
		{
			...
			/* Release allocated resources */
			if(fileMonitorData) _esdashboard_application_database_monitor_data_free(fileMonitorData);
			if(filePath) g_free(filePath);
		...
		fileMonitorData->changedID=g_signal_connect_swapped(fileMonitorData->monitor,
															"changed",
															G_CALLBACK(_esdashboard_application_database_on_file_monitor_changed),
															self);
		...
		g_file_query_file_type(inFile, G_FILE_QUERY_INFO_NONE, NULL)==G_FILE_TYPE_REGULAR)
	{
			...
			priv->applications)
		{
			...
			desktopID=esdashboard_application_database_get_desktop_id_from_file(inFile);
			...
			if(desktopID)
			{
				EsdashboardDesktopAppInfo				*currentDesktopAppInfo;
				...
				if(g_hash_table_lookup_extended(priv->applications, desktopID, NULL, (gpointer*)&currentDesktopAppInfo))
				{
					 ...
					 */
					newDesktopFilename=esdashboard_application_database_get_file_from_desktop_id(desktopID);
					...
					{
						EsdashboardDesktopAppInfo		*newDesktopAppInfo;
						 ...
						 */
						newDesktopAppInfo=ESDASHBOARD_DESKTOP_APP_INFO(g_object_new(ESDASHBOARD_TYPE_DESKTOP_APP_INFO,
																					"desktop-id", desktopID,
																					"file", inFile,
																					NULL));

						if(esdashboard_desktop_app_info_is_valid(newDesktopAppInfo))
						{
							...
							g_signal_emit(self, EsdashboardApplicationDatabaseSignals[SIGNAL_APPLICATION_ADDED], 0, newDesktopAppInfo);
			...
			priv->applications)
		{
			...
			if(desktopID)
			{
				...
				if(g_hash_table_lookup_extended(priv->applications, desktopID, NULL, (gpointer*)&appInfo))
				{
					...
					appInfoFile=esdashboard_desktop_app_info_get_file(appInfo);
					...
					if(g_file_equal(appInfoFile, inFile))
					{
						 ...
						 */
						if(!esdashboard_desktop_app_info_reload(appInfo) ||
							!esdashboard_desktop_app_info_is_valid(appInfo))
						{
							...
							g_signal_emit(self, EsdashboardApplicationDatabaseSignals[SIGNAL_APPLICATION_REMOVED], 0, appInfo);
	...
	if(inEventType==G_FILE_MONITOR_EVENT_DELETED)
	{
		 ...
		 */
		fileMonitorData=_esdashboard_application_database_monitor_data_find_by_file(self, inFile);
		if(fileMonitorData)
		{
			ESDASHBOARD_DEBUG(self, APPLICATIONS,
			...
			_esdashboard_application_database_monitor_data_free(fileMonitorData);
				...
				if(g_hash_table_lookup_extended(priv->applications, desktopID, NULL, (gpointer*)&currentDesktopAppInfo))
				{
					...
					if(newDesktopFilename)
					{
						...
						else
						{
							...
							/* Emit signal that an application has been removed */
							g_signal_emit(self, EsdashboardApplicationDatabaseSignals[SIGNAL_APPLICATION_REMOVED], 0, currentDesktopAppInfo);
...
static gboolean _esdashboard_application_database_load_applications_recursive(EsdashboardApplicationDatabase *self,
																				...
																				GError **outError)
{
	EsdashboardApplicationDatabasePrivate			*priv G_GNUC_UNUSED;
	gchar											*topLevelPath;
	gchar											*path;
	...
	while((info=g_file_enumerator_next_file(enumerator, NULL, &error)))
	{
		...
		if(g_file_info_get_file_type(info)==G_FILE_TYPE_DIRECTORY)
		{
			...
			childSuccess=_esdashboard_application_database_load_applications_recursive(self,
																						inTopLevelPath,
																						childPath,
																						ioDesktopAppInfos,
																						ioFileMonitors,
			...
			g_str_has_suffix(g_file_info_get_name(info), ".desktop"))
		{
												...
												NULL))
			{
				...
				appInfo=ESDASHBOARD_DESKTOP_APP_INFO(g_object_new(ESDASHBOARD_TYPE_DESKTOP_APP_INFO,
																	"desktop-id", desktopID,
																	"file", childFile,
																	NULL));
				if(esdashboard_desktop_app_info_is_valid(appInfo))
				{
	 ...
	 * for this path.
	 */
	monitorData=_esdashboard_application_database_monitor_data_new(inCurrentPath);
	if(!monitorData)
	{
		ESDASHBOARD_DEBUG(self, APPLICATIONS,
	...
	if(!monitorData->monitor && error)
	{
		...
		/* Release allocated resources */
		if(monitorData) _esdashboard_application_database_monitor_data_free(monitorData);
...
static gboolean _esdashboard_application_database_load_applications(EsdashboardApplicationDatabase *self, GError **outError)
{
	EsdashboardApplicationDatabasePrivate			*priv;
	GHashTable										*apps;
	GList											*fileMonitors;
	...
	for(iter=priv->searchPaths; iter; iter=g_list_next(iter))
	{
		...
		if(g_file_query_file_type(directory, G_FILE_QUERY_INFO_NONE, NULL)==G_FILE_TYPE_DIRECTORY &&
			!_esdashboard_application_database_load_applications_recursive(self, directory, directory, &apps, &fileMonitors, &error))
		{
			/* Propagate error */
			g_propagate_error(outError, error);
	...
	if(priv->appDirMonitors)
	{
		for(iter=priv->appDirMonitors; iter; iter=g_list_next(iter))
		{
			_esdashboard_application_database_monitor_data_free((EsdashboardApplicationDatabaseFileMonitorData*)iter->data);
		}
	...
	for(iter=priv->appDirMonitors; iter; iter=g_list_next(iter))
	{
		monitorData=(EsdashboardApplicationDatabaseFileMonitorData*)iter->data;
...
static gboolean _esdashboard_application_database_load_application_menu(EsdashboardApplicationDatabase *self, GError **outError)
{
	...
	priv->appsMenuReloadRequiredID=g_signal_connect_swapped(priv->appsMenu,
																	"reload-required",
																	G_CALLBACK(_esdashboard_application_database_on_application_menu_reload_required),
																	self);

	/* Emit signal 'menu-reload-required' */
	g_signal_emit(self, EsdashboardApplicationDatabaseSignals[SIGNAL_MENU_RELOAD_REQUIRED], 0);
...
static void _esdashboard_application_database_clean(EsdashboardApplicationDatabase *self)
{
	EsdashboardApplicationDatabasePrivate					*priv;
	...
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardApplicationDatabaseProperties[PROP_IS_LOADED]);
}

/* Callback function to check for duplicate search path entries used in object instance initialization */
static void _esdashboard_application_database_check_search_path_duplicate(gpointer inData, gpointer inUserData)
{
...
static GObject* _esdashboard_application_database_constructor(GType inType,
																guint inNumberConstructParams,
																GObjectConstructParam *inConstructParams)
{
	GObject									*object;

	if(!_esdashboard_application_database)
	{
		object=G_OBJECT_CLASS(esdashboard_application_database_parent_class)->constructor(inType, inNumberConstructParams, inConstructParams);
		_esdashboard_application_database=ESDASHBOARD_APPLICATION_DATABASE(object);
	}
		else
		{
			object=g_object_ref(G_OBJECT(_esdashboard_application_database));
		}
...
/* Dispose this object */
static void _esdashboard_application_database_dispose(GObject *inObject)
{
	EsdashboardApplicationDatabase			*self=ESDASHBOARD_APPLICATION_DATABASE(inObject);
	EsdashboardApplicationDatabasePrivate	*priv=self->priv;

	/* Release allocated resources */
	_esdashboard_application_database_clean(self);
	...
	G_OBJECT_CLASS(esdashboard_application_database_parent_class)->dispose(inObject);
}

/* Finalize this object */
static void _esdashboard_application_database_finalize(GObject *inObject)
{
	/* Release allocated resources finally, e.g. unset singleton */
	if(G_LIKELY(G_OBJECT(_esdashboard_application_database)==inObject))
	{
		_esdashboard_application_database=NULL;
	}

	/* Call parent's class dispose method */
	G_OBJECT_CLASS(esdashboard_application_database_parent_class)->finalize(inObject);
}

/* Set/get properties */
static void _esdashboard_application_database_get_property(GObject *inObject,
															guint inPropID,
															GValue *outValue,
															GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_application_database_class_init(EsdashboardApplicationDatabaseClass *klass)
{
	GObjectClass		*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	gobjectClass->constructor=_esdashboard_application_database_constructor;
	gobjectClass->dispose=_esdashboard_application_database_dispose;
	gobjectClass->finalize=_esdashboard_application_database_finalize;
	gobjectClass->get_property=_esdashboard_application_database_get_property;

	/* Define properties */
	EsdashboardApplicationDatabaseProperties[PROP_IS_LOADED]=
	...
	/* Define signals */
	EsdashboardApplicationDatabaseSignals[SIGNAL_MENU_RELOAD_REQUIRED]=
		g_signal_new("menu-reload-required",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_NO_HOOKS,
						G_STRUCT_OFFSET(EsdashboardApplicationDatabaseClass, menu_reload_required),
						NULL,
						NULL,
	...
	EsdashboardApplicationDatabaseSignals[SIGNAL_APPLICATION_ADDED]=
		g_signal_new("application-added",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_NO_HOOKS,
						G_STRUCT_OFFSET(EsdashboardApplicationDatabaseClass, application_added),
						NULL,
						NULL,
	...
	EsdashboardApplicationDatabaseSignals[SIGNAL_APPLICATION_REMOVED]=
		g_signal_new("application-removed",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST | G_SIGNAL_NO_HOOKS,
						G_STRUCT_OFFSET(EsdashboardApplicationDatabaseClass, application_removed),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
 ...
 */
static void esdashboard_application_database_init(EsdashboardApplicationDatabase *self)
{
	EsdashboardApplicationDatabasePrivate	*priv;
	const gchar* const						*systemPaths;
	gchar									*path;

	priv=self->priv=esdashboard_application_database_get_instance_private(self);
	...
	while(*systemPaths)
	{
		...
		g_list_foreach(priv->searchPaths, _esdashboard_application_database_check_search_path_duplicate, &searchPathUnique);
		if(searchPathUnique)
		{
...
EsdashboardApplicationDatabase* esdashboard_application_database_get_default(void)
{
	...
	singleton=g_object_new(ESDASHBOARD_TYPE_APPLICATION_DATABASE, NULL);
	return(ESDASHBOARD_APPLICATION_DATABASE(singleton));
}
...
gboolean esdashboard_application_database_load(EsdashboardApplicationDatabase *self, GError **outError)
{
	...
	/* Load installed and user-overidden application desktop files */
	if(!_esdashboard_application_database_load_applications(self, &error))
	{
		/* Propagate error */
		g_propagate_error(outError, error);
...
GList* esdashboard_application_database_get_all_applications(EsdashboardApplicationDatabase *self)
{
	...
	if(priv->applications)
	{
		g_hash_table_foreach(priv->applications,
								_esdashboard_application_database_add_hashtable_item_to_list,
								&applicationsList);
...
gchar* esdashboard_application_database_get_file_from_desktop_id(const gchar *inDesktopID)
{
	...
	{
		ESDASHBOARD_DEBUG(appDB, APPLICATIONS,
	...
	/* Get search paths */
	searchPaths=esdashboard_application_database_get_application_search_paths(appDB);
	if(!searchPaths)
	{
		/* Release allocated resources */
		g_object_unref(appDB);
...
gchar* esdashboard_application_database_get_desktop_id_from_file(GFile *inFile)
{
	...
	foundDesktopID=esdashboard_application_database_get_desktop_id_from_path(path);
