...
#include <libesdashboard/scrollbar.h>
...
static void _esdashboard_viewpad_focusable_iface_init(EsdashboardFocusableInterface *iface);

struct _EsdashboardViewpadPrivate
{
	/* Properties related */
	gfloat							spacing;
	EsdashboardView					*activeView;
	EsdashboardVisibilityPolicy		hScrollbarPolicy;
	gboolean						hScrollbarVisible;
	EsdashboardVisibilityPolicy		vScrollbarPolicy;
...
G_DEFINE_TYPE_WITH_CODE(EsdashboardViewpad,
						esdashboard_viewpad,
						ESDASHBOARD_TYPE_BACKGROUND,
						G_ADD_PRIVATE(EsdashboardViewpad)
						G_IMPLEMENT_INTERFACE(ESDASHBOARD_TYPE_FOCUSABLE, _esdashboard_viewpad_focusable_iface_init))
...
static GParamSpec* EsdashboardViewpadProperties[PROP_LAST]={ 0, };
...
static guint EsdashboardViewpadSignals[SIGNAL_LAST]={ 0, };

/* Forward declaration */
static void _esdashboard_viewpad_allocate(ClutterActor *self, const ClutterActorBox *inBox, ClutterAllocationFlags inFlags);

/* IMPLEMENTATION: Private variables and methods */

/* Update view depending on scrollbar values */
static void _esdashboard_viewpad_update_view_viewport(EsdashboardViewpad *self)
{
	EsdashboardViewpadPrivate	*priv;
	ClutterMatrix				transform;
	gfloat						x, y, w, h;

	g_return_if_fail(ESDASHBOARD_IS_VIEWPAD(self));
...
static void _esdashboard_viewpad_on_scrollbar_value_changed(EsdashboardViewpad *self,
															gfloat inValue,
															gpointer inUserData)
{
	EsdashboardViewpadPrivate	*priv;
	...
	g_return_if_fail(ESDASHBOARD_IS_VIEWPAD(self));
	g_return_if_fail(ESDASHBOARD_IS_SCROLLBAR(inUserData));
	...
	/* Update viewport */
	_esdashboard_viewpad_update_view_viewport(self);
}

/* Allocation of a view changed */
static void _esdashboard_viewpad_update_scrollbars(EsdashboardViewpad *self)
{
	EsdashboardViewpadPrivate	*priv;
	gfloat						w, h;
	...
	esdashboard_scrollbar_set_range(ESDASHBOARD_SCROLLBAR(priv->vScrollbar), isnan(h)==0 ? h : 0.0f);
	...
	if(clutter_actor_is_visible(CLUTTER_ACTOR(self)) &&
		(priv->hScrollbarPolicy==ESDASHBOARD_VISIBILITY_POLICY_AUTOMATIC ||
			priv->vScrollbarPolicy==ESDASHBOARD_VISIBILITY_POLICY_AUTOMATIC))
	{
		ClutterActorBox			box;

		clutter_actor_get_allocation_box(CLUTTER_ACTOR(self), &box);
		_esdashboard_viewpad_allocate(CLUTTER_ACTOR(self), &box, CLUTTER_DELEGATE_LAYOUT);
	}
}

/* Set new active view and deactive current one */
static void _esdashboard_viewpad_activate_view(EsdashboardViewpad *self, EsdashboardView *inView)
{
	EsdashboardViewpadPrivate	*priv;
	gfloat						x, y;
	...
	g_return_if_fail(ESDASHBOARD_IS_VIEWPAD(self));
	g_return_if_fail(inView==NULL || ESDASHBOARD_IS_VIEW(inView));
	...
	if(inView && !esdashboard_view_get_enabled(inView))
	{
	...
	focusManager=esdashboard_focus_manager_get_default();

	/* Deactivate current view */
	if(priv->activeView)
	{
		/* Unset focus at current active view if this view has the focus */
		hasFocus=esdashboard_focus_manager_has_focus(focusManager, ESDASHBOARD_FOCUSABLE(priv->activeView));
		if(hasFocus)
		{
			esdashboard_focusable_unset_focus(ESDASHBOARD_FOCUSABLE(priv->activeView));
			ESDASHBOARD_DEBUG(self, ACTOR,
								"Unset focus from view '%s' because it is the active view at viewpad",
								esdashboard_view_get_name(priv->activeView));
		}

		/* Hide current view and emit signal before and after deactivation */
		g_signal_emit(self, EsdashboardViewpadSignals[SIGNAL_VIEW_DEACTIVATING], 0, priv->activeView);
		...
		g_signal_emit(self, EsdashboardViewpadSignals[SIGNAL_VIEW_DEACTIVATED], 0, priv->activeView);
	...
	if(inView)
	{
		...
		g_signal_emit(self, EsdashboardViewpadSignals[SIGNAL_VIEW_ACTIVATING], 0, priv->activeView);
		g_signal_emit_by_name(priv->activeView, "activating");

		g_signal_handlers_block_by_func(priv->hScrollbar, _esdashboard_viewpad_on_scrollbar_value_changed, self);
		g_signal_handlers_block_by_func(priv->vScrollbar, _esdashboard_viewpad_on_scrollbar_value_changed, self);

		x=y=0.0f;
		clutter_actor_get_clip(CLUTTER_ACTOR(priv->activeView), &x, &y, NULL, NULL);
		_esdashboard_viewpad_update_scrollbars(self);
		esdashboard_scrollbar_set_value(ESDASHBOARD_SCROLLBAR(priv->hScrollbar), x);
		esdashboard_scrollbar_set_value(ESDASHBOARD_SCROLLBAR(priv->vScrollbar), y);
		_esdashboard_viewpad_update_view_viewport(self);
		...
		g_signal_handlers_unblock_by_func(priv->hScrollbar, _esdashboard_viewpad_on_scrollbar_value_changed, self);
		g_signal_handlers_unblock_by_func(priv->vScrollbar, _esdashboard_viewpad_on_scrollbar_value_changed, self);

		g_signal_emit_by_name(priv->activeView, "activated");
		g_signal_emit(self, EsdashboardViewpadSignals[SIGNAL_VIEW_ACTIVATED], 0, priv->activeView);

		/* Set focus to new active view if this viewpad has the focus */
		if(hasFocus)
		{
			esdashboard_focus_manager_set_focus(focusManager, ESDASHBOARD_FOCUSABLE(priv->activeView));
			ESDASHBOARD_DEBUG(self, ACTOR,
	...
	{
		EsdashboardFocusable	*newFocusable;

		newFocusable=esdashboard_focus_manager_get_next_focusable(focusManager, ESDASHBOARD_FOCUSABLE(self));
		if(newFocusable)
		{
			esdashboard_focus_manager_set_focus(focusManager, newFocusable);
			ESDASHBOARD_DEBUG(self, ACTOR,
	...
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardViewpadProperties[PROP_ACTIVE_VIEW]);
}

/* A view was disabled */
static void _esdashboard_viewpad_on_view_disabled(EsdashboardViewpad *self, EsdashboardView *inView)
{
	EsdashboardViewpadPrivate	*priv;
	ClutterActorIter			iter;
	ClutterActor				*child;
	EsdashboardView				*firstActivatableView;
	...
	if(inView==priv->activeView)
	{
		...
		while(clutter_actor_iter_next(&iter, &child))
		{
			...
			if(ESDASHBOARD_IS_VIEW(child)!=TRUE) continue;
			 ...
			 */
			if(ESDASHBOARD_VIEW(child)!=inView &&
				esdashboard_view_get_enabled(ESDASHBOARD_VIEW(child)))
			{
				firstActivatableView=ESDASHBOARD_VIEW(child);
			}
		}
		...
		_esdashboard_viewpad_activate_view(self, firstActivatableView);
	}
}

/* A view was enabled */
static void _esdashboard_viewpad_on_view_enabled(EsdashboardViewpad *self, EsdashboardView *inView)
{
	EsdashboardViewpadPrivate	*priv;
	...
	if(!priv->activeView) _esdashboard_viewpad_activate_view(self, inView);
}

/* Allocation of a view changed */
static gboolean _esdashboard_viewpad_on_allocation_changed_repaint_callback(gpointer inUserData)
{
	EsdashboardViewpad			*self;
	EsdashboardViewpadPrivate	*priv;

	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(inUserData), G_SOURCE_REMOVE);

	self=ESDASHBOARD_VIEWPAD(inUserData);
	priv=self->priv;
	...
	_esdashboard_viewpad_on_scrollbar_value_changed(self,
													esdashboard_scrollbar_get_value(ESDASHBOARD_SCROLLBAR(priv->hScrollbar)),
													priv->hScrollbar);
	_esdashboard_viewpad_on_scrollbar_value_changed(self,
													esdashboard_scrollbar_get_value(ESDASHBOARD_SCROLLBAR(priv->vScrollbar)),
													priv->vScrollbar);
...
static void _esdashboard_viewpad_on_allocation_changed(ClutterActor *inActor,
														...
														gpointer inUserData)
{
	EsdashboardViewpad			*self;
	EsdashboardViewpadPrivate	*priv;
	EsdashboardView				*view G_GNUC_UNUSED;

	g_return_if_fail(ESDASHBOARD_IS_VIEWPAD(inActor));
	g_return_if_fail(ESDASHBOARD_IS_VIEW(inUserData));

	self=ESDASHBOARD_VIEWPAD(inActor);
	priv=self->priv;
	view=ESDASHBOARD_VIEW(inUserData);
	...
	if(priv->scrollbarUpdateID==0)
	{
		priv->scrollbarUpdateID=
			clutter_threads_add_repaint_func_full(CLUTTER_REPAINT_FLAGS_QUEUE_REDRAW_ON_ADD | CLUTTER_REPAINT_FLAGS_POST_PAINT,
													_esdashboard_viewpad_on_allocation_changed_repaint_callback,
													self,
													NULL);
	}
}
...
static void _esdashboard_viewpad_on_view_scroll_to(EsdashboardViewpad *self,
													gfloat inX,
													gfloat inY,
													gpointer inUserData)
{
	...
	{
		if(inX>=0.0f) esdashboard_scrollbar_set_value(ESDASHBOARD_SCROLLBAR(priv->hScrollbar), inX);
		if(inY>=0.0f) esdashboard_scrollbar_set_value(ESDASHBOARD_SCROLLBAR(priv->vScrollbar), inY);
	}
 ...
 */
static gboolean _esdashboard_viewpad_view_needs_scrolling_for_child(EsdashboardViewpad *self,
																	...
																	gfloat *outScrollY)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(self), FALSE);
	g_return_val_if_fail(ESDASHBOARD_IS_VIEW(inView), FALSE);
	...
	if(inView==priv->activeView)
	{
		x=esdashboard_scrollbar_get_value(ESDASHBOARD_SCROLLBAR(priv->hScrollbar));
		y=esdashboard_scrollbar_get_value(ESDASHBOARD_SCROLLBAR(priv->vScrollbar));
		w=viewpadWidth;
		h=viewpadHeight;
	}
		else
...
static gboolean _esdashboard_viewpad_on_view_child_needs_scroll(EsdashboardViewpad *self,
																...
																gpointer inUserData)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_VIEW(inUserData), FALSE);

	view=ESDASHBOARD_VIEW(inUserData);

	/* Determine if scrolling is needed and return result */
	return(_esdashboard_viewpad_view_needs_scrolling_for_child(self, view, inActor, NULL, NULL));
}

/* Ensure that a child of a view is visible by scrolling if needed */
static void _esdashboard_viewpad_on_view_child_ensure_visible(EsdashboardViewpad *self,
														ClutterActor *inActor,
														gpointer inUserData)
{
	EsdashboardView				*view;
	...
	if(_esdashboard_viewpad_view_needs_scrolling_for_child(self, view, inActor, &scrollX, &scrollY))
	{
		_esdashboard_viewpad_on_view_scroll_to(self, scrollX, scrollY, view);
	}
}

/* Create view of given type and add to this actor */
static void _esdashboard_viewpad_add_view(EsdashboardViewpad *self, const gchar *inID)
{
	EsdashboardViewpadPrivate	*priv;
	GObject						*view;
	...
	view=esdashboard_view_manager_create_view(priv->viewManager, inID);
	if(view==NULL)
	{
	...
	if(ESDASHBOARD_IS_VIEW(view)!=TRUE)
	{
	...
	g_signal_connect_swapped(view, "allocation-changed", G_CALLBACK(_esdashboard_viewpad_on_allocation_changed), self);
	g_signal_connect_swapped(view, "scroll-to", G_CALLBACK(_esdashboard_viewpad_on_view_scroll_to), self);
	g_signal_connect_swapped(view, "child-needs-scroll", G_CALLBACK(_esdashboard_viewpad_on_view_child_needs_scroll), self);
	g_signal_connect_swapped(view, "child-ensure-visible", G_CALLBACK(_esdashboard_viewpad_on_view_child_ensure_visible), self);
	g_signal_connect_swapped(view, "disabled", G_CALLBACK(_esdashboard_viewpad_on_view_disabled), self);
	g_signal_connect_swapped(view, "enabled", G_CALLBACK(_esdashboard_viewpad_on_view_enabled), self);
	g_signal_emit(self, EsdashboardViewpadSignals[SIGNAL_VIEW_ADDED], 0, view);

	/* Set active view if none active (usually it is the first view created) */
	if(priv->activeView==NULL &&
		esdashboard_view_get_enabled(ESDASHBOARD_VIEW(view)))
	{
		_esdashboard_viewpad_activate_view(self, ESDASHBOARD_VIEW(view));
	}

	/* If newly added view is focusable register it to focus manager */
	if(priv->doRegisterFocusableViews &&
		ESDASHBOARD_IS_FOCUSABLE(view))
	{
		EsdashboardFocusManager	*focusManager;

		focusManager=esdashboard_focus_manager_get_default();
		esdashboard_focus_manager_register_after(focusManager,
													ESDASHBOARD_FOCUSABLE(view),
...
static void _esdashboard_viewpad_on_view_registered(EsdashboardViewpad *self,
													const gchar *inID,
													gpointer inUserData)
{
	g_return_if_fail(ESDASHBOARD_IS_VIEWPAD(self));
	...
	_esdashboard_viewpad_add_view(self, inID);
}

/* Called when a view type was unregistered */
static void _esdashboard_viewpad_on_view_unregistered(EsdashboardViewpad *self,
														const gchar *inID,
														gpointer inUserData)
{
	EsdashboardViewpadPrivate	*priv;
	...
	while(clutter_actor_iter_next(&iter, &child))
	{
		 ...
		 */
		if(!esdashboard_view_has_id(ESDASHBOARD_VIEW(child), inID))
		{
			if(esdashboard_view_get_enabled(ESDASHBOARD_VIEW(child))) firstActivatableView=child;
		}
			else
			{
				if(G_OBJECT(child)==G_OBJECT(priv->activeView)) _esdashboard_viewpad_activate_view(self, NULL);
				g_signal_emit(self, EsdashboardViewpadSignals[SIGNAL_VIEW_REMOVED], 0, child);
				clutter_actor_destroy(child);
			}
	}

	/* Now activate the first activatable view we found during iteration */
	if(firstActivatableView) _esdashboard_viewpad_activate_view(self, ESDASHBOARD_VIEW(firstActivatableView));
}

/* Scroll event happened at this actor because it was not handled
 * with any child actor
 */
static gboolean _esdashboard_viewpad_on_scroll_event(ClutterActor *inActor,
														...
														gpointer inUserData)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(inActor), FALSE);
...
static void _esdashboard_viewpad_show(ClutterActor *self)
{
	EsdashboardViewpadPrivate	*priv=ESDASHBOARD_VIEWPAD(self)->priv;
	ClutterActorClass			*actorClass=CLUTTER_ACTOR_CLASS(esdashboard_viewpad_parent_class);
...
static void _esdashboard_viewpad_get_preferred_height(ClutterActor *self,
														gfloat inForWidth,
														gfloat *outMinHeight,
...
static void _esdashboard_viewpad_get_preferred_width(ClutterActor *self,
														gfloat inForHeight,
														gfloat *outMinWidth,
...
static void _esdashboard_viewpad_allocate(ClutterActor *self,
											...
											ClutterAllocationFlags inFlags)
{
	...
	if(priv->hScrollbarPolicy==ESDASHBOARD_VISIBILITY_POLICY_ALWAYS ||
		(priv->hScrollbarPolicy==ESDASHBOARD_VISIBILITY_POLICY_AUTOMATIC &&
			esdashboard_scrollbar_get_range(ESDASHBOARD_SCROLLBAR(priv->hScrollbar))>viewWidth))
	{
		hScrollbarVisible=TRUE;
	}
	if(esdashboard_view_get_view_fit_mode(ESDASHBOARD_VIEW(priv->activeView))==ESDASHBOARD_VIEW_FIT_MODE_HORIZONTAL ||
		esdashboard_view_get_view_fit_mode(ESDASHBOARD_VIEW(priv->activeView))==ESDASHBOARD_VIEW_FIT_MODE_BOTH)
	{
		hScrollbarVisible=FALSE;
	}

	vScrollbarVisible=FALSE;
	if(priv->vScrollbarPolicy==ESDASHBOARD_VISIBILITY_POLICY_ALWAYS ||
		(priv->vScrollbarPolicy==ESDASHBOARD_VISIBILITY_POLICY_AUTOMATIC &&
			esdashboard_scrollbar_get_range(ESDASHBOARD_SCROLLBAR(priv->vScrollbar))>viewHeight))
	{
		vScrollbarVisible=TRUE;
	}
	if(esdashboard_view_get_view_fit_mode(ESDASHBOARD_VIEW(priv->activeView))==ESDASHBOARD_VIEW_FIT_MODE_VERTICAL ||
		esdashboard_view_get_view_fit_mode(ESDASHBOARD_VIEW(priv->activeView))==ESDASHBOARD_VIEW_FIT_MODE_BOTH)
	{
		vScrollbarVisible=FALSE;
	}
	...
	if(priv->activeView)
	{
		...
		switch(esdashboard_view_get_view_fit_mode(ESDASHBOARD_VIEW(priv->activeView)))
		{
			case ESDASHBOARD_VIEW_FIT_MODE_BOTH:
				w=viewWidth;
				h=viewHeight;
				break;

			case ESDASHBOARD_VIEW_FIT_MODE_HORIZONTAL:
				w=viewWidth;
				clutter_actor_get_preferred_height(CLUTTER_ACTOR(priv->activeView), w, NULL, &h);
				break;

			case ESDASHBOARD_VIEW_FIT_MODE_VERTICAL:
				h=viewHeight;
	...
	if(priv->hScrollbarVisible!=hScrollbarVisible)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardViewpadProperties[PROP_HSCROLLBAR_VISIBLE]);
	}
	...
	if(priv->vScrollbarVisible!=vScrollbarVisible)
	{
		...
		/* Notify about property change */
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardViewpadProperties[PROP_VSCROLLBAR_VISIBLE]);
	}
...
static gboolean _esdashboard_viewpad_focusable_can_focus(EsdashboardFocusable *inFocusable)
{
	EsdashboardViewpad			*self;
	EsdashboardViewpadPrivate	*priv;

	g_return_val_if_fail(ESDASHBOARD_IS_FOCUSABLE(inFocusable), FALSE);
	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(inFocusable), FALSE);

	self=ESDASHBOARD_VIEWPAD(inFocusable);
	priv=self->priv;
	...
	if(!priv->doRegisterFocusableViews)
	{
		...
		while(clutter_actor_iter_next(&iter, &child))
		{
			...
			if(ESDASHBOARD_IS_FOCUSABLE(child))
			{
				esdashboard_focus_manager_register_after(focusManager,
															ESDASHBOARD_FOCUSABLE(child),
															ESDASHBOARD_FOCUSABLE(self));
			}
		}
 ...
 */
void _esdashboard_viewpad_focusable_iface_init(EsdashboardFocusableInterface *iface)
{
	iface->can_focus=_esdashboard_viewpad_focusable_can_focus;
}

/* IMPLEMENTATION: GObject */

/* Dispose this object */
static void _esdashboard_viewpad_dispose(GObject *inObject)
{
	EsdashboardViewpad			*self=ESDASHBOARD_VIEWPAD(inObject);
	EsdashboardViewpadPrivate	*priv=self->priv;

	/* Prevent further registers of views */
	priv->doRegisterFocusableViews=FALSE;

	/* Deactivate current view */
	if(priv->activeView) _esdashboard_viewpad_activate_view(self, NULL);
	...
	G_OBJECT_CLASS(esdashboard_viewpad_parent_class)->dispose(inObject);
}

/* Set/get properties */
static void _esdashboard_viewpad_set_property(GObject *inObject,
												guint inPropID,
												...
												GParamSpec *inSpec)
{
	...
	{
		case PROP_SPACING:
			esdashboard_viewpad_set_spacing(self, g_value_get_float(inValue));
			break;

		case PROP_HSCROLLBAR_POLICY:
			esdashboard_viewpad_set_horizontal_scrollbar_policy(self, (EsdashboardVisibilityPolicy)g_value_get_enum(inValue));
			break;

		case PROP_VSCROLLBAR_POLICY:
			esdashboard_viewpad_set_vertical_scrollbar_policy(self, (EsdashboardVisibilityPolicy)g_value_get_enum(inValue));
			break;
...
static void _esdashboard_viewpad_get_property(GObject *inObject,
												guint inPropID,
												GValue *outValue,
												GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_viewpad_class_init(EsdashboardViewpadClass *klass)
{
	...
	/* Override functions */
	gobjectClass->set_property=_esdashboard_viewpad_set_property;
	gobjectClass->get_property=_esdashboard_viewpad_get_property;
	gobjectClass->dispose=_esdashboard_viewpad_dispose;

	clutterActorClass->show=_esdashboard_viewpad_show;
	clutterActorClass->get_preferred_width=_esdashboard_viewpad_get_preferred_width;
	clutterActorClass->get_preferred_height=_esdashboard_viewpad_get_preferred_height;
	clutterActorClass->allocate=_esdashboard_viewpad_allocate;

	/* Define properties */
	EsdashboardViewpadProperties[PROP_SPACING]=
	...
	EsdashboardViewpadProperties[PROP_ACTIVE_VIEW]=
	...
	EsdashboardViewpadProperties[PROP_HSCROLLBAR_VISIBLE]=
	...
	EsdashboardViewpadProperties[PROP_VSCROLLBAR_POLICY]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardViewpadProperties);

	/* Define stylable properties */
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardViewpadProperties[PROP_SPACING]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardViewpadProperties[PROP_HSCROLLBAR_POLICY]);
	esdashboard_actor_install_stylable_property(actorClass, EsdashboardViewpadProperties[PROP_VSCROLLBAR_POLICY]);

	/* Define signals */
	EsdashboardViewpadSignals[SIGNAL_VIEW_ADDED]=
		g_signal_new("view-added",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardViewpadClass, view_added),
						NULL,
						NULL,
	...
	EsdashboardViewpadSignals[SIGNAL_VIEW_REMOVED]=
		g_signal_new("view-removed",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardViewpadClass, view_removed),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
	...
	EsdashboardViewpadSignals[SIGNAL_VIEW_ACTIVATING]=
		g_signal_new("view-activating",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardViewpadClass, view_activating),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
	...
	EsdashboardViewpadSignals[SIGNAL_VIEW_ACTIVATED]=
		g_signal_new("view-activated",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardViewpadClass, view_activated),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
	...
	EsdashboardViewpadSignals[SIGNAL_VIEW_DEACTIVATING]=
		g_signal_new("view-deactivating",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardViewpadClass, view_deactivating),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
	...
	EsdashboardViewpadSignals[SIGNAL_VIEW_DEACTIVATED]=
		g_signal_new("view-deactivated",
						G_TYPE_FROM_CLASS(klass),
						G_SIGNAL_RUN_LAST,
						G_STRUCT_OFFSET(EsdashboardViewpadClass, view_deactivated),
						NULL,
						NULL,
						g_cclosure_marshal_VOID__OBJECT,
						G_TYPE_NONE,
 ...
 */
static void esdashboard_viewpad_init(EsdashboardViewpad *self)
{
	EsdashboardViewpadPrivate	*priv;
	GList						*views, *viewEntry;

	priv=self->priv=esdashboard_viewpad_get_instance_private(self);
	...
	g_signal_connect(self, "scroll-event", G_CALLBACK(_esdashboard_viewpad_on_scroll_event), NULL);

	/* Set up child actors */
	priv->hScrollbar=esdashboard_scrollbar_new(CLUTTER_ORIENTATION_HORIZONTAL);
	clutter_actor_add_child(CLUTTER_ACTOR(self), priv->hScrollbar);
	g_signal_connect_swapped(priv->hScrollbar, "value-changed", G_CALLBACK(_esdashboard_viewpad_on_scrollbar_value_changed), self);

	priv->vScrollbar=esdashboard_scrollbar_new(CLUTTER_ORIENTATION_VERTICAL);
	clutter_actor_add_child(CLUTTER_ACTOR(self), priv->vScrollbar);
	g_signal_connect_swapped(priv->vScrollbar, "value-changed", G_CALLBACK(_esdashboard_viewpad_on_scrollbar_value_changed), self);

	/* Create instance of each registered view type and add it to this actor
	 * and connect signals
	 */
	views=viewEntry=esdashboard_view_manager_get_registered(priv->viewManager);
	for(; viewEntry; viewEntry=g_list_next(viewEntry))
	{
		const gchar				*viewID;

		viewID=(const gchar*)viewEntry->data;
		_esdashboard_viewpad_add_view(self, viewID);
	}
	g_list_free_full(views, g_free);

	g_signal_connect_swapped(priv->viewManager, "registered", G_CALLBACK(_esdashboard_viewpad_on_view_registered), self);
	g_signal_connect_swapped(priv->viewManager, "unregistered", G_CALLBACK(_esdashboard_viewpad_on_view_unregistered), self);
}
...
{
	return(CLUTTER_ACTOR(g_object_new(ESDASHBOARD_TYPE_VIEWPAD, NULL)));
}

/* Get/set spacing */
gfloat esdashboard_viewpad_get_spacing(EsdashboardViewpad *self)
{
	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(self), 0.0f);
...
void esdashboard_viewpad_set_spacing(EsdashboardViewpad *self, gfloat inSpacing)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardViewpadProperties[PROP_SPACING]);
}
...
GList* esdashboard_viewpad_get_views(EsdashboardViewpad *self)
{
	...
	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(self), NULL);
	...
	while(clutter_actor_iter_next(&iter, &child))
	{
		...
		if(ESDASHBOARD_IS_VIEW(child)==TRUE) list=g_list_prepend(list, child);
	}
...
gboolean esdashboard_viewpad_has_view(EsdashboardViewpad *self, EsdashboardView *inView)
{
	...
	while(clutter_actor_iter_next(&iter, &child))
	{
		if(ESDASHBOARD_IS_VIEW(child) &&
			ESDASHBOARD_VIEW(child)==inView)
		{
			return(TRUE);
		}
	}
...
EsdashboardView* esdashboard_viewpad_find_view_by_type(EsdashboardViewpad *self, GType inType)
{
	...
	while(!view && clutter_actor_iter_next(&iter, &child))
	{
		...
		if(ESDASHBOARD_IS_VIEW(child)==TRUE &&
			G_OBJECT_TYPE(child)==inType)
		{
			view=ESDASHBOARD_VIEW(child);
		}
	}
...
EsdashboardView* esdashboard_viewpad_find_view_by_id(EsdashboardViewpad *self, const gchar *inID)
{
	...
	while(!view && clutter_actor_iter_next(&iter, &child))
	{
		...
		if(ESDASHBOARD_IS_VIEW(child)==TRUE &&
			esdashboard_view_has_id(ESDASHBOARD_VIEW(child), inID))
		{
			view=ESDASHBOARD_VIEW(child);
		}
	}
...
void esdashboard_viewpad_set_active_view(EsdashboardViewpad *self, EsdashboardView *inView)
{
	...
	if(priv->activeView!=inView) _esdashboard_viewpad_activate_view(self, inView);
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_VIEWPAD(self), ESDASHBOARD_VISIBILITY_POLICY_AUTOMATIC);
...
void esdashboard_viewpad_set_horizontal_scrollbar_policy(EsdashboardViewpad *self, EsdashboardVisibilityPolicy inPolicy)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardViewpadProperties[PROP_HSCROLLBAR_POLICY]);
}
...
void esdashboard_viewpad_set_vertical_scrollbar_policy(EsdashboardViewpad *self, EsdashboardVisibilityPolicy inPolicy)
{
	...
	/* Notify about property change */
	g_object_notify_by_pspec(G_OBJECT(self), EsdashboardViewpadProperties[PROP_VSCROLLBAR_POLICY]);
}
