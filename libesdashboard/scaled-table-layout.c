...
struct _EsdashboardScaledTableLayoutPrivate
{
	/* Properties related */
	gfloat		rowSpacing;
...
G_DEFINE_TYPE_WITH_PRIVATE(EsdashboardScaledTableLayout,
							esdashboard_scaled_table_layout,
...
static GParamSpec* EsdashboardScaledTableLayoutProperties[PROP_LAST]={ 0, };


/* IMPLEMENTATION: Private variables and methods */

/* Updates the minimum number of rows and columns needed for layout */
static void _esdashboard_scaled_table_layout_update_rows_and_columns(EsdashboardScaledTableLayout *self,
																		ClutterContainer *inContainer)
{
	...
	g_return_if_fail(ESDASHBOARD_IS_SCALED_TABLE_LAYOUT(self));
	...
	if(numberChildren!=priv->numberChildren)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_NUMBER_CHILDREN]);
	}
	...
	{
		priv->rows=rows;
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_ROWS]);
	}

	if(columns!=priv->columns)
	{
		priv->columns=columns;
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_COLUMNS]);
	}
...
static void _esdashboard_scaled_table_layout_get_preferred_width(ClutterLayoutManager *self,
																	ClutterContainer *inContainer,
																	gfloat inForHeight,
																	gfloat *outMinWidth,
																	gfloat *outNaturalWidth)
{
	...
	priv=ESDASHBOARD_SCALED_TABLE_LAYOUT(self)->priv;
	...
	_esdashboard_scaled_table_layout_update_rows_and_columns(ESDASHBOARD_SCALED_TABLE_LAYOUT(self), inContainer);
...
static void _esdashboard_scaled_table_layout_get_preferred_height(ClutterLayoutManager *self,
																	ClutterContainer *inContainer,
																	gfloat inForWidth,
																	gfloat *outMinHeight,
...
static void _esdashboard_scaled_table_layout_allocate(ClutterLayoutManager *self,
														ClutterContainer *inContainer,
...
/* Set/get properties */
static void _esdashboard_scaled_table_layout_set_property(GObject *inObject,
															guint inPropID,
															const GValue *inValue,
															GParamSpec *inSpec)
{
	EsdashboardScaledTableLayout			*self=ESDASHBOARD_SCALED_TABLE_LAYOUT(inObject);
	
	switch(inPropID)
	{
		case PROP_ROW_SPACING:
			esdashboard_scaled_table_layout_set_row_spacing(self, g_value_get_float(inValue));
			break;

		case PROP_COLUMN_SPACING:
			esdashboard_scaled_table_layout_set_column_spacing(self, g_value_get_float(inValue));
			break;

		case PROP_RELATIVE_SCALE:
			esdashboard_scaled_table_layout_set_relative_scale(self, g_value_get_boolean(inValue));
			break;

		case PROP_PREVENT_UPSCALING:
			esdashboard_scaled_table_layout_set_prevent_upscaling(self, g_value_get_boolean(inValue));
			break;
...
static void _esdashboard_scaled_table_layout_get_property(GObject *inObject,
															guint inPropID,
															GValue *outValue,
															GParamSpec *inSpec)
{
 ...
 * and signals
 */
static void esdashboard_scaled_table_layout_class_init(EsdashboardScaledTableLayoutClass *klass)
{
	ClutterLayoutManagerClass	*layoutClass=CLUTTER_LAYOUT_MANAGER_CLASS(klass);
	GObjectClass				*gobjectClass=G_OBJECT_CLASS(klass);

	/* Override functions */
	layoutClass->get_preferred_width=_esdashboard_scaled_table_layout_get_preferred_width;
	layoutClass->get_preferred_height=_esdashboard_scaled_table_layout_get_preferred_height;
	layoutClass->allocate=_esdashboard_scaled_table_layout_allocate;

	gobjectClass->set_property=_esdashboard_scaled_table_layout_set_property;
	gobjectClass->get_property=_esdashboard_scaled_table_layout_get_property;

	/* Define properties */
	EsdashboardScaledTableLayoutProperties[PROP_ROW_SPACING]=
	...
	EsdashboardScaledTableLayoutProperties[PROP_COLUMN_SPACING]=
	...
	EsdashboardScaledTableLayoutProperties[PROP_RELATIVE_SCALE]=
	...
	EsdashboardScaledTableLayoutProperties[PROP_PREVENT_UPSCALING]=
	...
	EsdashboardScaledTableLayoutProperties[PROP_NUMBER_CHILDREN]=
	...
	EsdashboardScaledTableLayoutProperties[PROP_ROWS]=
	...
	EsdashboardScaledTableLayoutProperties[PROP_COLUMNS]=
	...
	g_object_class_install_properties(gobjectClass, PROP_LAST, EsdashboardScaledTableLayoutProperties);
}

/* Object initialization
 * Create private structure and set up default values
 */
static void esdashboard_scaled_table_layout_init(EsdashboardScaledTableLayout *self)
{
	EsdashboardScaledTableLayoutPrivate	*priv;

	priv=self->priv=esdashboard_scaled_table_layout_get_instance_private(self);
...
{
	return(CLUTTER_LAYOUT_MANAGER(g_object_new(ESDASHBOARD_TYPE_SCALED_TABLE_LAYOUT, NULL)));
}
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_SCALED_TABLE_LAYOUT(self), 0);
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_SCALED_TABLE_LAYOUT(self), FALSE);
...
void esdashboard_scaled_table_layout_set_relative_scale(EsdashboardScaledTableLayout *self, gboolean inScaling)
{
	...
	if(priv->relativeScale!=inScaling)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_RELATIVE_SCALE]);
...
void esdashboard_scaled_table_layout_set_prevent_upscaling(EsdashboardScaledTableLayout *self, gboolean inPreventUpscaling)
{
	...
	if(priv->preventUpscaling!=inPreventUpscaling)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_PREVENT_UPSCALING]);
...
void esdashboard_scaled_table_layout_set_spacing(EsdashboardScaledTableLayout *self, gfloat inSpacing)
{
	...
	if(priv->rowSpacing!=inSpacing || priv->columnSpacing!=inSpacing)
	{
		...
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_ROW_SPACING]);

		priv->columnSpacing=inSpacing;
		g_object_notify_by_pspec(G_OBJECT(self), EsdashboardScaledTableLayoutProperties[PROP_COLUMN_SPACING]);
...
{
	g_return_val_if_fail(ESDASHBOARD_IS_SCALED_TABLE_LAYOUT(self), 0.0f);
